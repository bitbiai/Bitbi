    ...
    Object.freeze({
        ...
        items: [
            ...
            { id: 'canvas-input-cost', title: { en: 'Inputs and credits', de: 'Eingaben und Credits' },
                ...
                detail: { en: 'For image nodes, Additional prompt supplements connected text or works alone. For other nodes, a nonempty Prompt takes precedence over connected text. Run missing upstream outputs first. Estimates include the effective purpose instructions where text pricing depends on them. Reference images and media settings can affect cost. The server checks the applicable personal or organization credits before execution. The estimate describes consumption, including Admin runs. Admin platform-budget runs do not debit personal credits; their separate platform budget is still enforced. A dash means no reliable estimate is available. Outputs remain saved with the project for downstream nodes.', de: 'Bei Bild-Nodes ergänzt der zusätzliche Prompt den verbundenen Text oder funktioniert allein. Bei anderen Nodes hat ein ausgefüllter Prompt Vorrang vor verbundenem Text. Fehlende vorgeschaltete Ergebnisse zuerst ausführen. Wenn die Textpreise davon abhängen, umfasst die Schätzung auch die wirksame Zweckvorgabe. Referenzbilder und Medieneinstellungen können die Kosten verändern. Der Server prüft vor der Ausführung die geltenden persönlichen oder Organisationscredits. Die Schätzung beschreibt den Verbrauch, auch bei Admin-Ausführungen. Diese buchen keine persönlichen Credits ab; das separate Plattformbudget wird weiterhin geprüft. Ein Strich bedeutet, dass keine verlässliche Schätzung verfügbar ist. Ergebnisse bleiben im Projekt für nachfolgende Nodes gespeichert.' } },
            { id:'canvas-media',title:{en:'Canvas media and full video',de:'Canvas-Medien und Gesamtvideo'},
                summary:{en:'Keep results in your project or save them to Assets.',de:'Ergebnisse im Projekt behalten oder in Assets speichern.'},
                detail:{en:'New images, video clips and audio stay private in Canvas across reloads. Save to Assets keeps the existing bytes permanently without another generation or credit charge. Deleting a producer removes unsaved output once consumers finish. Create full video joins existing clips: up to 400 MB of sources, 10 minutes and an 80 MB result. Combined videos are saved automatically to Assets and survive source deletion. For Grok video connections, Last frame remains available. Edit and Extend are temporarily unavailable pending Cloudflare operation billing verification; existing saved settings and results are retained. Size is a provider setting, not a guarantee of original-file resolution.',de:'Neue Bilder, Videoclips und Audios bleiben auch nach erneutem Öffnen privat in Canvas. In Assets speichern übernimmt dieselben Dateien dauerhaft, ohne erneute Generierung oder Creditsabbuchung. Beim Löschen eines erzeugenden Nodes werden ungespeicherte Ausgaben entfernt, sobald sie nicht mehr benötigt werden. Gesamtes Video erstellen verbindet vorhandene Clips: bis zu 400 MB Quellen, 10 Minuten und 80 MB Ergebnis. Gesamtvideos werden automatisch in Assets gespeichert und bleiben nach dem Löschen der Quellen erhalten. Bei Grok-Videoverbindungen bleibt Letztes Bild verfügbar. Bearbeiten und Verlängern sind bis zur Prüfung der Cloudflare-Abrechnung vorübergehend nicht verfügbar; gespeicherte Einstellungen und Ergebnisse bleiben erhalten. Größe ist eine Anbietereinstellung, keine Garantie für die Auflösung der Originaldatei.'}},
        ],
    }),
    Object.freeze({
        id: 'start',
