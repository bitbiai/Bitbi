...
export function renderWorkspaceModelHelp() {
    ...
    for (const mode of GENERATE_LAB_MEDIA_TYPES) {
        ...
        for (const model of models) {
            ...
            if(controls.referenceRoles?.length) {
                paragraph(copy('Use a first frame, a last frame, or both with adaptive ratio. Alternatively use up to nine images, three video clips and three audio clips, at most twelve files in order. Frame inputs and reference mode cannot be mixed. Video/audio clips must each be 2–15 seconds and total at most 15 seconds per type. Only authorized saved originals are used; unsupported files are rejected before acceptance.',
                    'Nutze ein Anfangsbild, ein Endbild oder beide mit adaptivem Format. Alternativ sind bis zu neun Bilder, drei Videos und drei Audioclips möglich, insgesamt höchstens zwölf Dateien in Reihenfolge. Frame-Eingaben und Referenzmodus sind getrennt. Video-/Audioclips müssen jeweils 2–15 Sekunden und je Medientyp zusammen höchstens 15 Sekunden lang sein. Nur berechtigte gespeicherte Originale werden verwendet; ungeeignete Dateien werden vor der Annahme abgelehnt.'));
                paragraph(copy('H3 is priced per output second: 768P $0.08, 2K $0.13 before the existing BITBI credit conversion. Requested settings are not a guarantee of exact output dimensions.','H3 wird pro Ausgabesekunde berechnet: 768P 0,08 USD, 2K 0,13 USD vor der bestehenden BITBI-Creditumrechnung. Angeforderte Einstellungen garantieren keine exakten Ausgabeabmessungen.'));
            } else if (controls.supportsVideoInput) {
