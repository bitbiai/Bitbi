...
function setupExperimentCard(revealObserver) {
    ...
    const ytStar = createStarButton('experiments', 'youtube-exclusives', { title: 'YouTube Exclusives', thumb_url: '' });
    ytStar.style.cssText = 'position:absolute;top:8px;right:8px;z-index:3';
    const ytContent = content.querySelector('div[style*="padding:20px"]');
    if (ytContent) { ytContent.style.position = 'relative'; ytContent.appendChild(ytStar); }
...
function setupSoundLabCard(revealObserver) {
    ...
    exclusiveTracks.forEach((tr, idx) => {
        ...
        const exclStar = createStarButton('soundlab', tr.slug, { title: tr.t, thumb_url: `/api/soundlab-thumbs/${tr.thumb}` });
    ...
    function ensureAudio(idx) {
        ...
        a.addEventListener('ended', () => {
            ...
            if (idx < exclusiveTracks.length - 1) {
                playExclTrack(idx + 1);
                ctn.dispatchEvent(new CustomEvent('snd:excl-deck-sync', { detail: idx + 1 }));
            } else {
                activeExclIdx = null;
            }
        });
    ...
    ctn.addEventListener('snd:excl-swipe', (e) => {
        ...
        const wasPlaying = activeExclIdx !== null && audios[activeExclIdx] && !audios[activeExclIdx].paused;
