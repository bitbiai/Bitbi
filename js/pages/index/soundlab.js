...
export function initSoundLab(revealObserver) {
    ...
    tracks.forEach((tr, idx) => {
        ...
        const slug = tr.file.split('/').pop().replace('.mp3', '');
        const star = createStarButton('soundlab', slug, { title: tr.t, thumb_url: trackImages[idx] });
        d.querySelector('.snd-player-row').appendChild(star);
        ctn.appendChild(d);
    });
