...
export const ROUTE_POLICIES = Object.freeze([
  ...
  safeRead("gallery.comments.list", "GET", "/api/gallery/:collection/:id/comments", "homepage", {
    auth: "anonymous",
    sensitivity: "low",
    config: ["DB"],
    rateLimit: { noneReason: "Read-only public comments list is bounded and only returns comments for currently public media." },
    notes: "Verifies Mempic, Memvid, or Memtrack public visibility before returning sanitized comments. Author user ids, emails, roles, and R2 keys are never returned.",
  }),
  policy({
    id: "gallery.comments.create",
    method: "POST",
    path: "/api/gallery/:collection/:id/comments",
    auth: "user",
    csrf: "same-origin-required",
    body: { kind: "json", maxBytesName: "smallJson", contentType: "application/json" },
    rateLimit: { id: "public-media-comment-create-user-and-ip", failClosed: true },
    config: REQUIRED_CONFIG.authPublicLimiter,
    audit: { event: "public_media_comment_created" },
    owner: "homepage",
    sensitivity: "medium",
    notes: "Authenticated comment creation for currently public Mempics, Memvids, and Memtracks only. Body text is normalized, bounded, and stored without exposing private media keys.",
  }),
  safeRead("gallery.comments.avatar", "GET", "/api/gallery/comments/:commentId/:version/avatar", "homepage", {
    auth: "anonymous",
    sensitivity: "low",
    config: ["DB", "PRIVATE_MEDIA"],
    rateLimit: { noneReason: "Read-only public comment avatar route resolves only from a comment whose parent media is still public." },
    notes: "Comment avatar URLs are versioned by profile avatar timestamp and re-check parent public visibility before serving PRIVATE_MEDIA avatar bytes.",
  }),
  safeRead("gallery.media.interactions", "GET", "/api/gallery/:collection/:id/interactions", "homepage", {
    auth: "optional-user",
    sensitivity: "low",
    config: ["DB"],
    rateLimit: { noneReason: "Read-only public interaction state is bounded and verifies current public media visibility." },
    notes: "Returns like/comment/follow counts plus viewer state when a valid session exists. It never returns raw user ids or private media keys.",
  }),
  policy({
    id: "gallery.media.like.create",
    method: "POST",
    path: "/api/gallery/:collection/:id/like",
    auth: "user",
    csrf: "same-origin-required",
    body: { kind: "none", noneReason: "Like toggles use only the path media type/id and authenticated session." },
    rateLimit: { id: "public-media-like-create-user", failClosed: true },
    config: REQUIRED_CONFIG.authPublicLimiter,
  ...
  policy({
    ...
    auth: "user",
    csrf: "same-origin-required",
    body: { kind: "none", noneReason: "Unlike uses only the path media type/id and authenticated session." },
    rateLimit: { id: "public-media-like-delete-user", failClosed: true },
    config: REQUIRED_CONFIG.authPublicLimiter,
    audit: { event: "public_media_like_deleted" },
    owner: "homepage",
    sensitivity: "medium",
    notes: "Authenticated, idempotent unlike for public media. Media cleanup hard-deletes likes when a medium is unpublished or deleted.",
  }),
  policy({
    id: "gallery.media.follow.create",
    method: "POST",
    path: "/api/gallery/:collection/:id/follow",
    auth: "user",
    csrf: "same-origin-required",
    body: { kind: "none", noneReason: "Follow resolves the public media owner from the path target and authenticated session." },
    rateLimit: { id: "profile-follow-create-user", failClosed: true },
    config: REQUIRED_CONFIG.authPublicLimiter,
    audit: { event: "profile_follow_created" },
    owner: "homepage",
    sensitivity: "medium",
    notes: "Authenticated follow of a public media owner. The route rejects self-follow and does not expose private profile fields.",
  }),
  policy({
    id: "gallery.media.follow.delete",
    method: "DELETE",
    path: "/api/gallery/:collection/:id/follow",
    auth: "user",
    csrf: "same-origin-required",
    body: { kind: "none", noneReason: "Unfollow resolves the public media owner from the path target and authenticated session." },
    rateLimit: { id: "profile-follow-delete-user", failClosed: true },
    config: REQUIRED_CONFIG.authPublicLimiter,
  ...
  safeRead("profile.social.summary", "GET", "/api/profile/social/summary", "profile", {
    ...
    sensitivity: "medium",
    config: ["DB"],
    rateLimit: { noneReason: "Authenticated profile summary uses bounded aggregate counts for the current user only." },
    notes: "Returns follower, following, received-like, published-media, and liked-media counts for the signed-in member dashboard.",
  }),
  safeRead("profile.social.followers", "GET", "/api/profile/social/followers", "profile", {
    auth: "user",
    sensitivity: "medium",
    config: ["DB"],
    rateLimit: { noneReason: "Authenticated, bounded follower list for the current user only." },
    notes: "Returns sanitized display names for users following the signed-in member.",
  }),
  safeRead("profile.social.following", "GET", "/api/profile/social/following", "profile", {
    auth: "user",
    sensitivity: "medium",
    config: ["DB"],
    rateLimit: { noneReason: "Authenticated, bounded following list for the current user only." },
    notes: "Returns sanitized display names for users followed by the signed-in member.",
  }),
  safeRead("profile.social.likes", "GET", "/api/profile/social/likes", "profile", {
    auth: "user",
    sensitivity: "medium",
    config: ["DB"],
  ...
  safeRead("profile.media.published", "GET", "/api/profile/media/published", "profile", {
    ...
    sensitivity: "medium",
    config: ["DB"],
    rateLimit: { noneReason: "Authenticated, bounded list of the current user's own public media." },
    notes: "Returns current user's public Mempics, Memvids, and Memtracks with safe public media URLs.",
  }),
  safeRead("profile.media.liked", "GET", "/api/profile/media/liked", "profile", {
    auth: "user",
    sensitivity: "medium",
    config: ["DB"],
  ...
  adminRead("admin.r2.buckets.list", "/api/admin/r2/buckets", "admin", {
    config: REQUIRED_CONFIG.adminR2,
    rateLimit: { id: "admin-r2-read-ip", failClosed: true },
    notes: "Admin-only configured R2 bucket discovery. Returns only allowlisted Worker-bound buckets and never accepts arbitrary bucket names or exposes Cloudflare account IDs, secrets, tokens, or object payloads.",
  }),
  adminRead("admin.r2.objects.list", "/api/admin/r2/objects", "admin", {
    config: REQUIRED_CONFIG.adminR2,
    rateLimit: { id: "admin-r2-read-ip", failClosed: true },
    notes: "Admin-only bounded R2 object listing for allowlisted buckets with prefix, delimiter, cursor, limit, owner-label enrichment, and app-link risk metadata. It does not recursively dump whole buckets or expose secrets.",
  }),
  adminRead("admin.r2.objects.detail", "/api/admin/r2/objects/detail", "admin", {
    config: REQUIRED_CONFIG.adminR2,
    rateLimit: { id: "admin-r2-read-ip", failClosed: true },
    notes: "Admin-only object metadata/detail lookup. Raw R2 keys are visible only inside the admin session; logs and audit events use key hashes and redacted summaries.",
  }),
  adminRead("admin.r2.objects.file", "/api/admin/r2/objects/file", "admin", {
    config: REQUIRED_CONFIG.adminR2,
    rateLimit: { id: "admin-r2-read-ip", failClosed: true },
    notes: "Admin-only R2 object preview/download stream for allowlisted buckets. Supports private no-store responses, Range requests, nosniff, and never grants public or member access.",
  }),
  policy({
    id: "admin.r2.objects.upload",
    method: "POST",
    path: "/api/admin/r2/objects/upload",
    auth: "admin",
    mfa: "admin-production-required",
    csrf: "same-origin-required",
    body: { kind: "multipart", maxBytesName: "adminR2Upload", contentType: "multipart/form-data" },
    rateLimit: { id: "admin-r2-write-ip", failClosed: true },
    audit: { event: "admin_r2_object_uploaded" },
    owner: "admin",
    sensitivity: "high",
    config: REQUIRED_CONFIG.adminR2,
    notes: "High-risk admin R2 upload. Requires Admin/MFA, same-origin multipart, Idempotency-Key, bounded operator reason, allowlisted bucket, bounded object key, size limit, overwrite guard, fail-closed rate limiting, and audit logging with key hashes only.",
  }),
  adminJsonWrite("admin.r2.folders.create", "POST", "/api/admin/r2/folders", "admin", "smallJson", "admin-r2-write-ip", {
    audit: { event: "admin_r2_folder_created" },
    config: REQUIRED_CONFIG.adminR2,
    notes: "Admin-only R2 prefix creation using a hidden .keep sentinel when needed. Requires Admin/MFA, same-origin JSON, Idempotency-Key, bounded operator reason, allowlisted bucket, normalized prefix, fail-closed rate limiting, and audit logging with prefix hashes only.",
  }),
  adminJsonWrite("admin.r2.objects.copy", "POST", "/api/admin/r2/objects/copy", "admin", "adminJson", "admin-r2-write-ip", {
    audit: { event: "admin_r2_objects_copied" },
    config: REQUIRED_CONFIG.adminR2,
    notes: "Admin-only bounded R2 object copy across allowlisted buckets. Requires Admin/MFA, same-origin JSON, Idempotency-Key, bounded operator reason, batch size limit, normalized keys, fail-closed rate limiting, and audit logging with counts only.",
  }),
  adminJsonWrite("admin.r2.objects.move", "POST", "/api/admin/r2/objects/move", "admin", "adminJson", "admin-r2-write-ip", {
    audit: { event: "admin_r2_objects_moved" },
    config: REQUIRED_CONFIG.adminR2,
    notes: "High-risk admin R2 move/rename. Requires Admin/MFA, same-origin JSON, Idempotency-Key, bounded operator reason, copy-before-delete, allowlisted buckets, normalized keys, fail-closed rate limiting, and audit logging; DB-linked/app-managed R2 keys are blocked from raw key changes to avoid broken D1 references.",
  }),
  adminJsonWrite("admin.r2.objects.delete", "POST", "/api/admin/r2/objects/delete", "admin", "adminJson", "admin-r2-write-ip", {
    audit: { event: "admin_r2_objects_deleted" },
    config: REQUIRED_CONFIG.adminR2,
    notes: "High-risk admin R2 delete. Requires Admin/MFA, same-origin JSON, Idempotency-Key, confirm=true with confirmation=DELETE R2 OBJECTS, bounded operator reason, explicit bucket/key echoes, batch size limit, fail-closed rate limiting, and audit logging; DB-linked/app-managed R2 objects are blocked from raw deletion to avoid dangling D1 references.",
  }),
  ...
  safeRead("ai.generation-jobs.list", "GET", "/api/ai/generation-jobs", "ai-studio"),
  safeRead("ai.generation-jobs.detail", "GET", "/api/ai/generation-jobs/:id", "ai-studio"),
  userJsonWrite("ai.generation-preview.retry", "POST", "/api/ai/generation-jobs/:id/retry-preview", "ai-studio", "smallJson", "member-preview-retry"),
