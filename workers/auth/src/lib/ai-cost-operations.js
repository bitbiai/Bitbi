...
const BUDGET_POLICY_BY_OPERATION_ID = Object.freeze({
  ...
  "admin.fable_chat.send": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    notes: "The private admin Fable chat checks the existing Admin Text master/runtime switch, persists a pending attempt, and atomically admits effort- and estimated-context-weighted daily/monthly platform-cap units before calling the AI Worker. Provider failures and unknown outcomes retain the admitted units; successful sends store the assistant result for replay.",
    temporaryBaselineAllowed: false,
    killSwitchTarget: "ENABLE_ADMIN_AI_TEXT_BUDGET",
    liveBudgetCapStatus: "cap_enforced",
    liveBudgetCapReadiness: "countable_now",
    liveBudgetCapFuturePhase: "Weighted admission implemented with migration 0070",
    liveBudgetCapEvidence: {
  ...
  "admin.fable_chat.compact_memory": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    currentBudgetScope: AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET,
    targetFuturePhase: "Private admin Fable rolling-memory compaction",
    targetEnforcementStatus: "implemented",
    targetEnforcement: { idempotency: "durable_checkpoint_fingerprint", budgetLedger: "atomic_preprovider_platform_budget_usage_event", replay: "stored_hidden_checkpoint", killSwitch: "ENABLE_ADMIN_AI_TEXT_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "Server-owned Standard/Lite memory compaction stores an immutable checkpoint attempt and atomically admits Qwen input/output-price-weighted platform-cap units before provider execution. Failed or unknown attempts never replace the prior valid checkpoint.",
    temporaryBaselineAllowed: false,
    killSwitchTarget: "ENABLE_ADMIN_AI_TEXT_BUDGET",
    liveBudgetCapStatus: "cap_enforced",
    liveBudgetCapReadiness: "countable_now",
    liveBudgetCapFuturePhase: "Rolling-memory compaction admission implemented with migration 0073",
    liveBudgetCapEvidence: {
      dataSources: ["fable_chat_memory_checkpoints", "platform_budget_usage_events", "platform_budget_limits"],
      durableCompletionTimestamp: true,
      estimatedCostUnitsAvailable: true,
      requiresCentralUsageLedger: false,
    },
    reconciliationStatus: "partial",
    reconciliationFuturePhase: "Add explicit memory-compaction usage repair only if operator evidence requires it",
    reconciliationEvidence: {
      budgetScope: AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET,
      sourceTables: ["fable_chat_memory_checkpoints", "platform_budget_usage_events"],
      sourceIdField: "source_attempt_id",
      repairExecutor: false,
      executableActions: [],
      reviewOnlyActions: ["review_missing_usage_event", "review_orphan_usage_event"],
  ...
  "admin.music.test": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    notes: "Phase 4.17 enforces platform_admin_lab_budget daily/monthly caps after runtime switches and before durable admin_ai_usage_attempts rows or provider execution. Metadata-only duplicate suppression, conflict detection, cleanup, and admin-only inspection exclude raw prompts, lyrics, composition plans, audio, and customer-credit claims; model-specific provider USD estimates remain separate from abstract cap units.",
    temporaryBaselineAllowed: false,
...
export const AI_COST_OPERATION_REGISTRY = Object.freeze([
  ...
  operation({
    operationConfig: {
      ...
      creditCost: 1,
      costPolicy: "admin_fable_chat_weighted_platform_budget",
      quantity: 1,
      idempotencyPolicy: "required",
      reservationPolicy: "platform_budget_only",
      replayPolicy: "durable_result",
      ...
      notes: "Private platform-admin Fable chat with server-loaded native roles, hashed idempotency keys, serialized conversations, effort/context-weighted atomic pre-provider platform-cap admission, durable pending/running/unknown state, and stored assistant replay.",
    },
    sourceFiles: [
      "workers/auth/src/routes/admin-fable-chat.js",
      "workers/auth/src/lib/fable-chat.js",
      "workers/auth/src/lib/fable-chat-context.js",
      "workers/auth/src/lib/fable-chat-budget.js",
      "workers/auth/src/lib/fable-chat-stream.js",
      "workers/auth/src/lib/admin-ai-proxy.js",
      "workers/shared/fable-chat-contract.mjs",
      "workers/ai/src/routes/fable-chat.js",
      "workers/ai/src/lib/anthropic-stream.js",
  ...
  operation({
    operationConfig: {
      ...
      featureKey: "admin.ai.fable_chat_memory",
      actorType: "admin",
      billingScope: AI_COST_GATEWAY_SCOPES.UNMETERED_ADMIN,
      providerFamily: "workers_ai",
      modelResolverKey: "admin.fable_chat.memory.fixed_qwen",
      creditCost: 1,
      costPolicy: "admin_fable_chat_memory_qwen_weighted_platform_budget",
      quantity: 1,
      idempotencyPolicy: "required",
      reservationPolicy: "platform_budget_only",
      replayPolicy: "durable_result",
      failurePolicy: "manual_review",
      storagePolicy: "none",
      observabilityEventPrefix: "admin.fable_chat.compact_memory",
      routeId: "internal.ai.fable-chat.memory",
      routePath: "/internal/ai/fable-chat/memory",
      notes: "Internal-only Qwen rolling-memory compaction with immutable input fingerprints, exact model pricing metadata, atomic platform budget admission, and no browser-visible summary content.",
    },
    sourceFiles: [
      "workers/auth/src/lib/fable-chat-memory.js",
      "workers/auth/src/lib/fable-chat-memory-budget.js",
      "workers/shared/fable-chat-memory-contract.mjs",
      "workers/ai/src/routes/fable-chat-memory.js",
      "workers/ai/src/lib/invoke-ai.js",
    ],
    currentStatus: "implemented",
    currentEnforcement: {
    ...
    routePolicy: null,
    currentGaps: [
      "An interrupted provider attempt remains unknown and never replaces or advances the prior valid memory checkpoint.",
    ],
    gapSeverity: "P3",
    nextMigrationPhase: "No further memory budget migration planned",
  }),
  operation({
    operationConfig: {
  ...
  operation({
    operationConfig: {
      ...
      notes: "ENABLE_ADMIN_AI_MUSIC_BUDGET is enforced before attempts or provider execution for registry-allowlisted Admin Music models. MiniMax and ElevenLabs share the existing fixed 160 abstract platform-budget-unit rule; ElevenLabs provider USD estimates are recorded separately without treating USD as platform units or customer billing. Idempotency-Key remains required with metadata-only replay and no raw prompts, lyrics, composition plans, or audio in durable attempt metadata.",
    },
  ...
  operation({
    operationConfig: {
      ...
      notes: "The internal music route is service-only, dispatches only registry-allowlisted models, and requires caller-side policy metadata before provider execution.",
    },
