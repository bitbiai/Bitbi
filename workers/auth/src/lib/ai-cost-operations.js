...
const BUDGET_POLICY_BY_OPERATION_ID = Object.freeze({
  "admin.text.test": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    notes: "Phase 4.8.1 requires Idempotency-Key, creates admin_ai_usage_attempts rows, suppresses same-key duplicate provider execution after pending/completed/failed states, conflicts same-key/different-request retries, and stores metadata-only replay state without credits or live budget caps.",
    temporaryBaselineAllowed: false,
...
export const AI_COST_OPERATION_REGISTRY = Object.freeze([
  ...
  operation({
    operationConfig: {
      ...
      notes: "Phase 4.8.1 admin-only provider-cost text test. Requires Idempotency-Key, creates an admin_ai_usage_attempts row before provider execution, suppresses duplicate same-key provider calls once pending/completed/failed state exists, and stores metadata-only replay state without raw prompt or generated text.",
    },
