...
const BUDGET_POLICY_BY_OPERATION_ID = Object.freeze({
  ...
  "admin.image.test.charged": budgetPolicy(AI_COST_BUDGET_SCOPES.ADMIN_ORG_CREDIT_ACCOUNT, {
    ...
    targetEnforcement: { idempotency: "required", budgetLedger: "selected_org_credit_account", replay: "metadata_only", killSwitch: "model-specific charged image budget metadata target" },
    notes: "Phase 4.3 hardens the charged Admin image-test branch with admin_org_credit_account budget policy metadata while preserving selected organization credit debits and no provider replay. Phase 4.14 keeps this charged branch unchanged while classifying every non-charged Admin Image branch.",
  }),
  "admin.image.test.unmetered": budgetPolicy(AI_COST_BUDGET_SCOPES.EXPLICIT_UNMETERED_ADMIN, {
    ...
    targetFuturePhase: "Phase 4.14 unmetered/unsupported Admin Image branch handling",
    targetEnforcementStatus: "implemented",
    targetEnforcement: { idempotency: "explicit_exception_optional", budgetLedger: "explicit_unmetered_admin_metadata", replay: "metadata_only_no_result_replay", killSwitch: "ENABLE_ADMIN_AI_UNMETERED_IMAGE_TESTS metadata target" },
    notes: "Phase 4.14 classifies FLUX.2 Dev as the only explicit unmetered admin image lab exception, adds safe budget/caller-policy metadata before AI Worker execution, and blocks future unclassified image branches before provider calls.",
    temporaryBaselineAllowed: false,
  ...
  "admin.compare": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    currentBudgetScope: AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET,
    targetFuturePhase: "Phase 4.10 admin compare budget enforcement",
    targetEnforcementStatus: "partial",
    targetEnforcement: { idempotency: "durable_metadata_only", budgetLedger: "platform_admin_lab_budget_metadata", replay: "metadata_only_no_compare_results", killSwitch: "ENABLE_ADMIN_AI_COMPARE_BUDGET metadata target" },
    notes: "Phase 4.10 requires Idempotency-Key, creates admin_ai_usage_attempts rows, suppresses same-key duplicate provider fanout after pending/completed/failed states, conflicts same-key/different-request retries, and stores metadata-only replay state without raw prompts, compare outputs, credits, or live budget caps. Phase 4.8.2 cleanup/inspection applies to these rows.",
    temporaryBaselineAllowed: false,
  ...
  "admin.live_agent": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    notes: "Phase 4.12 requires Idempotency-Key, creates admin_ai_usage_attempts parent stream-session rows, suppresses same-key duplicate provider streams after pending/completed/failed states, conflicts same-key/different-request retries, propagates signed caller-policy metadata, and finalizes metadata-only stream status without raw messages, streamed output, provider bodies, credits, or live platform caps.",
    temporaryBaselineAllowed: false,
...
export const AI_COST_OPERATION_REGISTRY = Object.freeze([
  ...
  operation({
    operationConfig: {
      ...
      notes: "Priced Admin AI image test path charges selected organization credits. Phase 4.3 adds admin_org_credit_account budget-policy plan/audit metadata for the charged branch; Phase 4.14 preserves that behavior while separately classifying explicit unmetered or blocked Admin Image branches.",
    },
  ...
  operation({
    operationConfig: {
      ...
      routeId: "admin.ai.test-image",
      routePath: "/api/admin/ai/test-image",
      notes: "Phase 4.14 makes FLUX.2 Dev the only explicit unmetered Admin Image lab exception, with safe budget-policy and caller-policy metadata. It does not debit credits, require durable idempotency, or claim live platform budget caps.",
    },
    sourceFiles: ["workers/auth/src/routes/admin-ai.js", "workers/ai/src/routes/image.js", "workers/ai/src/lib/invoke-ai.js"],
    currentStatus: "implemented",
    currentEnforcement: {
    ...
    currentGaps: [
      "Runtime env kill-switch enforcement and live platform budget caps remain future work for this explicit unmetered exception.",
    ...
    gapSeverity: "P3",
    nextMigrationPhase: "Phase 4.15 runtime kill-switch enforcement and live platform budget caps",
  }),
  operation({
    operationConfig: {
  ...
  operation({
    operationConfig: {
      ...
      notes: "Phase 4.10 Admin Compare requires durable metadata-only idempotency before multi-model text provider fanout and propagates signed caller-policy metadata to the internal compare route.",
    },
    ...
    currentGaps: ["Runtime env kill-switch enforcement and live platform budget caps remain future work; full compare result replay is intentionally unavailable."],
    gapSeverity: "P3",
    nextMigrationPhase: "Keep metadata-only; add live platform lab caps only with a later route-specific migration",
  }),
  operation({
    operationConfig: {
      ...
      notes: "Phase 4.12 covers the Admin Live-Agent streaming route with required request idempotency, durable metadata-only stream-session attempts, signed caller-policy propagation, and safe stream completion/failure tracking. Runtime env kill-switch enforcement and live platform caps remain future work.",
    },
    ...
    currentGaps: ["Runtime env kill-switch enforcement, explicit output-token/duration caps, and live platform budget caps remain future work; full stream result replay is intentionally unavailable."],
    gapSeverity: "P2",
    nextMigrationPhase: "Keep metadata-only; add route-specific runtime kill switch and live platform lab caps in a later phase",
  }),
  operation({
    operationConfig: {
