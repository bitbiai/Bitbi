...
const BUDGET_POLICY_BY_OPERATION_ID = Object.freeze({
  "admin.text.test": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    targetFuturePhase: "Phase 4.8 admin text/embeddings budget enforcement",
    targetEnforcementStatus: "partial",
    targetEnforcement: { idempotency: "required", budgetLedger: "platform_admin_lab_budget_metadata", replay: "disabled", killSwitch: "ENABLE_ADMIN_AI_TEXT_BUDGET metadata target" },
    notes: "Phase 4.8 requires Idempotency-Key, builds platform_admin_lab_budget metadata, and propagates signed caller-policy metadata for admin text tests; durable replay/conflict storage and live budget caps remain future work.",
    temporaryBaselineAllowed: false,
  ...
  "admin.embeddings.test": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    targetFuturePhase: "Phase 4.8 admin text/embeddings budget enforcement",
    targetEnforcementStatus: "partial",
    targetEnforcement: { idempotency: "required", budgetLedger: "platform_admin_lab_budget_metadata", replay: "disabled", killSwitch: "ENABLE_ADMIN_AI_EMBEDDINGS_BUDGET metadata target" },
    notes: "Phase 4.8 requires Idempotency-Key, builds platform_admin_lab_budget metadata, and propagates signed caller-policy metadata for admin embeddings tests; durable replay/conflict storage and live budget caps remain future work.",
    temporaryBaselineAllowed: false,
...
export const AI_COST_OPERATION_REGISTRY = Object.freeze([
  ...
  operation({
    operationConfig: {
      ...
      notes: "Phase 4.8 admin-only provider-cost text test. Requires Idempotency-Key, emits platform_admin_lab_budget metadata, and propagates signed caller-policy metadata; no credits are debited and no durable replay is created.",
    },
    ...
    currentGaps: [
      "Idempotency-Key is required and included in safe metadata, but there is no durable replay/conflict persistence for same-key requests yet.",
      "Runtime env kill-switch enforcement and live platform budget caps remain future work.",
    ],
    gapSeverity: "P2",
    nextMigrationPhase: "Phase 4.9 remaining admin provider-cost budget migrations",
  }),
  operation({
    operationConfig: {
  ...
  operation({
    operationConfig: {
      ...
      notes: "Phase 4.8 admin-only provider-cost embeddings test. Requires Idempotency-Key, emits platform_admin_lab_budget metadata, and propagates signed caller-policy metadata; no credits are debited and no durable replay is created.",
    },
  ...
  operation({
    operationConfig: {
      ...
      notes: "Internal MiniMax music route relies on caller-side policy. Phase 3.6 covers the member caller; admin/provider-test callers remain outside the member gateway.",
    },
    ...
    routePolicy: null,
    currentGaps: ["Admin/provider-test callers still rely on explicit admin/platform budget policy rather than direct internal route enforcement."],
    gapSeverity: "P2",
