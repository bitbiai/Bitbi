...
const BUDGET_POLICY_BY_OPERATION_ID = Object.freeze({
  "admin.text.test": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    targetFuturePhase: "Phase 4.15 runtime budget kill-switch enforcement",
    targetEnforcementStatus: "partial",
    targetEnforcement: { idempotency: "durable_metadata_only", budgetLedger: "platform_admin_lab_budget_metadata", replay: "metadata_only_no_result_replay", killSwitch: "ENABLE_ADMIN_AI_TEXT_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "Phase 4.17 enforces platform_admin_lab_budget daily/monthly caps after runtime switches and before durable admin_ai_usage_attempts rows or provider execution. Phase 4.8.1/4.8.2 still provide metadata-only duplicate suppression, conflict detection, cleanup, and admin-only sanitized inspection without credits.",
    temporaryBaselineAllowed: false,
    killSwitchTarget: "ENABLE_ADMIN_AI_TEXT_BUDGET",
    liveBudgetCapStatus: "cap_enforced",
    liveBudgetCapReadiness: "countable_now",
    liveBudgetCapFuturePhase: "Phase 4.17 platform_admin_lab_budget cap foundation implemented",
    liveBudgetCapEvidence: {
      dataSources: ["admin_ai_usage_attempts", "platform_budget_usage_events", "platform_budget_limits"],
  ...
  "admin.fable_chat.send": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    currentBudgetScope: AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET,
    targetFuturePhase: "Private admin Fable chat durable persistence",
    targetEnforcementStatus: "implemented",
    targetEnforcement: { idempotency: "durable_result_replay", budgetLedger: "atomic_preprovider_platform_budget_usage_event", replay: "stored_assistant_result", killSwitch: "ENABLE_ADMIN_AI_TEXT_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "The private admin Fable chat checks the existing Admin Text master/runtime switch, persists a pending attempt, and atomically admits one conservative daily/monthly platform-cap usage unit before calling the AI Worker. Provider failures and unknown outcomes retain that admitted unit; successful sends store the assistant result for replay.",
    temporaryBaselineAllowed: false,
    killSwitchTarget: "ENABLE_ADMIN_AI_TEXT_BUDGET",
    liveBudgetCapStatus: "cap_enforced",
    liveBudgetCapReadiness: "countable_now",
    liveBudgetCapFuturePhase: "Implemented with migration 0069",
    liveBudgetCapEvidence: {
      dataSources: ["fable_chat_turns", "fable_chat_messages", "platform_budget_usage_events", "platform_budget_limits"],
      durableCompletionTimestamp: true,
      estimatedCostUnitsAvailable: true,
      requiresCentralUsageLedger: false,
    },
    reconciliationStatus: "partial",
    reconciliationFuturePhase: "Add explicit Fable chat usage reconciliation only if operator evidence requires it",
    reconciliationEvidence: {
      budgetScope: AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET,
      sourceTables: ["fable_chat_turns", "platform_budget_usage_events"],
      sourceIdField: "source_attempt_id",
      repairExecutor: false,
      executableActions: [],
      reviewOnlyActions: ["review_missing_usage_event", "review_orphan_usage_event"],
      automaticRepair: false,
      scheduledRepair: false,
    },
  }),
  ...
  "admin.image.test.unmetered": budgetPolicy(AI_COST_BUDGET_SCOPES.EXPLICIT_UNMETERED_ADMIN, {
    ...
    targetFuturePhase: "Phase 4.15 runtime budget kill-switch enforcement",
    targetEnforcementStatus: "implemented",
    targetEnforcement: { idempotency: "explicit_exception_optional", budgetLedger: "explicit_unmetered_admin_metadata", replay: "metadata_only_no_result_replay", killSwitch: "ENABLE_ADMIN_AI_UNMETERED_IMAGE_TESTS runtime_enforced", runtimeKillSwitch: "implemented" },
    notes: "Phase 4.15 enforces ENABLE_ADMIN_AI_UNMETERED_IMAGE_TESTS before explicit unmetered FLUX.2 Dev provider execution. Phase 4.14 keeps FLUX.2 Dev as the only explicit unmetered admin image lab exception and blocks future unclassified image branches before provider calls.",
    temporaryBaselineAllowed: false,
  ...
  "admin.embeddings.test": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    targetFuturePhase: "Phase 4.15 runtime budget kill-switch enforcement",
    targetEnforcementStatus: "partial",
    targetEnforcement: { idempotency: "durable_metadata_only", budgetLedger: "platform_admin_lab_budget_metadata", replay: "metadata_only_no_vectors", killSwitch: "ENABLE_ADMIN_AI_EMBEDDINGS_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "Phase 4.17 enforces platform_admin_lab_budget daily/monthly caps after runtime switches and before durable admin_ai_usage_attempts rows or provider execution. Phase 4.8.1/4.8.2 still provide metadata-only duplicate suppression, conflict detection, cleanup, and admin-only sanitized inspection without raw embedding input, vectors, or credits.",
    temporaryBaselineAllowed: false,
  ...
  "admin.music.test": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    targetFuturePhase: "Phase 4.15 runtime budget kill-switch enforcement",
    targetEnforcementStatus: "partial",
    targetEnforcement: { idempotency: "durable_metadata_only", budgetLedger: "platform_admin_lab_budget_metadata", replay: "metadata_only_no_audio", killSwitch: "ENABLE_ADMIN_AI_MUSIC_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "Phase 4.17 enforces platform_admin_lab_budget daily/monthly caps after runtime switches and before durable admin_ai_usage_attempts rows or provider execution. Phase 4.9/4.8.2 still provide metadata-only duplicate suppression, conflict detection, cleanup, and admin-only sanitized inspection without raw prompts, lyrics, audio, or credits.",
    temporaryBaselineAllowed: false,
  ...
  "admin.video.sync_debug": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    currentBudgetScope: AI_COST_BUDGET_SCOPES.EXPLICIT_UNMETERED_ADMIN,
    targetFuturePhase: "Phase 4.13 sync video debug retirement decision",
    targetEnforcementStatus: "partial",
    targetEnforcement: { idempotency: "not_required_while_disabled", budgetLedger: "not_applicable_retired_debug_path", replay: "disabled", killSwitch: "ALLOW_SYNC_VIDEO_DEBUG emergency flag only" },
    notes: "Phase 4.13 retires sync video debug as a normal provider-cost path. The route remains disabled-by-default; the existing ALLOW_SYNC_VIDEO_DEBUG=true emergency compatibility path must not be treated as supported budgeted generation.",
    temporaryBaselineAllowed: false,
  ...
  "admin.video.job.create": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    targetEnforcementStatus: "implemented",
    targetEnforcement: { idempotency: "required", budgetLedger: "platform_admin_lab_budget_metadata", replay: "job_metadata", killSwitch: "ENABLE_ADMIN_AI_VIDEO_JOB_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "Phase 4.17 enforces platform_admin_lab_budget daily/monthly caps after runtime switches and before admin async video job rows are created or provider-cost queue work is enqueued. Successful job completion records a de-duplicated platform budget usage event; no credits are debited.",
    temporaryBaselineAllowed: false,
    liveBudgetCapStatus: "cap_enforced",
    liveBudgetCapReadiness: "countable_now",
    liveBudgetCapFuturePhase: "Phase 4.17 platform_admin_lab_budget cap foundation implemented",
    liveBudgetCapEvidence: {
      dataSources: ["ai_video_jobs", "platform_budget_usage_events", "platform_budget_limits"],
      durableCompletionTimestamp: true,
      estimatedCostUnitsAvailable: true,
      requiresCentralUsageLedger: false,
    },
  ...
  "admin.compare": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    targetFuturePhase: "Phase 4.15 runtime budget kill-switch enforcement",
    targetEnforcementStatus: "partial",
    targetEnforcement: { idempotency: "durable_metadata_only", budgetLedger: "platform_admin_lab_budget_metadata", replay: "metadata_only_no_compare_results", killSwitch: "ENABLE_ADMIN_AI_COMPARE_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "Phase 4.17 enforces platform_admin_lab_budget daily/monthly caps after runtime switches and before durable admin_ai_usage_attempts rows or provider fanout. Phase 4.10/4.8.2 still provide metadata-only duplicate suppression, conflict detection, cleanup, and admin-only sanitized inspection without raw prompts, compare outputs, or credits.",
    temporaryBaselineAllowed: false,
  ...
  "admin.live_agent": budgetPolicy(AI_COST_BUDGET_SCOPES.PLATFORM_ADMIN_LAB_BUDGET, {
    ...
    targetEnforcementStatus: "partial",
    targetEnforcement: {
      idempotency: "durable_metadata_only_stream_session",
      budgetLedger: "platform_admin_lab_budget_metadata",
      replay: "metadata_only_no_stream_output",
      killSwitch: "ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET runtime_enforced",
      runtimeKillSwitch: "implemented",
      liveBudgetCap: "implemented",
    },
    notes: "Phase 4.17 enforces platform_admin_lab_budget daily/monthly caps after runtime switches and before durable stream-session attempts or provider streams. Phase 4.12 still requires Idempotency-Key, suppresses same-key duplicate streams, propagates signed caller-policy metadata, and finalizes metadata-only stream status without raw messages, streamed output, provider bodies, or credits.",
    temporaryBaselineAllowed: false,
  ...
  "internal.live_agent": budgetPolicy(AI_COST_BUDGET_SCOPES.INTERNAL_AI_WORKER_CALLER_ENFORCED, {
    ...
    targetEnforcementStatus: "implemented",
    targetEnforcement: { idempotency: "caller_stream_session_required", budgetLedger: "caller_enforced", replay: "metadata_only", killSwitch: "service_binding_only plus ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET caller metadata" },
    notes: "Phase 4.12 requires signed caller-policy metadata on /internal/ai/live-agent and delegates stream-session idempotency/budget metadata to the Admin Live-Agent auth route.",
    temporaryBaselineAllowed: false,
  }),
  "platform.news_pulse.visual.ingest": budgetPolicy(AI_COST_BUDGET_SCOPES.OPENCLAW_NEWS_PULSE_BUDGET, {
    targetFuturePhase: "A1 Wave 2 OpenClaw/News Pulse aggregate cap enforcement",
    targetEnforcementStatus: "implemented",
    targetEnforcement: { idempotency: "deterministic_item_key", budgetLedger: "openclaw_news_pulse_budget_usage_events", replay: "durable_thumbnail", killSwitch: "ENABLE_NEWS_PULSE_VISUAL_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "A1 Wave 2 enforces openclaw_news_pulse_budget daily/monthly caps after the runtime switch and before ingest-triggered visual provider calls. Duplicate suppression remains on deterministic item/status rows.",
    temporaryBaselineAllowed: false,
    killSwitchTarget: "ENABLE_NEWS_PULSE_VISUAL_BUDGET",
    liveBudgetCapStatus: "cap_enforced",
    liveBudgetCapReadiness: "countable_now",
    liveBudgetCapFuturePhase: "A1 Wave 2 openclaw_news_pulse_budget cap enforcement implemented",
    liveBudgetCapEvidence: {
      dataSources: ["news_pulse_items", "platform_budget_limits", "platform_budget_usage_events"],
      durableCompletionTimestamp: true,
      estimatedCostUnitsAvailable: true,
      requiresCentralUsageLedger: false,
    },
    reconciliationStatus: "partial",
    reconciliationFuturePhase: "A1 Wave 2 usage event evidence; full repair executor remains platform_admin_lab_budget-only",
    reconciliationEvidence: {
      budgetScope: AI_COST_BUDGET_SCOPES.OPENCLAW_NEWS_PULSE_BUDGET,
      sourceTables: ["news_pulse_items", "platform_budget_usage_events"],
      sourceIdField: "source_job_id",
      repairExecutor: false,
      executableActions: Object.freeze([]),
      reviewOnlyActions: Object.freeze(["review_missing_news_pulse_usage_event"]),
      automaticRepair: false,
      scheduledRepair: false,
    },
  }),
  "platform.news_pulse.visual.scheduled": budgetPolicy(AI_COST_BUDGET_SCOPES.OPENCLAW_NEWS_PULSE_BUDGET, {
    ...
    targetEnforcementStatus: "implemented",
    targetEnforcement: { idempotency: "deterministic_item_key", budgetLedger: "openclaw_news_pulse_budget_usage_events", replay: "durable_thumbnail", killSwitch: "ENABLE_NEWS_PULSE_VISUAL_BUDGET runtime_enforced", runtimeKillSwitch: "implemented", liveBudgetCap: "implemented" },
    notes: "A1 Wave 2 enforces openclaw_news_pulse_budget daily/monthly caps after the runtime switch and before scheduled/backfill visual provider calls. Bounded batch/status guards remain unchanged.",
    temporaryBaselineAllowed: false,
    killSwitchTarget: "ENABLE_NEWS_PULSE_VISUAL_BUDGET",
    liveBudgetCapStatus: "cap_enforced",
    liveBudgetCapReadiness: "countable_now",
...
export const AI_COST_OPERATION_REGISTRY = Object.freeze([
  ...
  operation({
    operationConfig: {
      ...
      providerCost: true,
      modelResolverKey: "member.text.model_catalog",
      creditCost: 1,
      quantity: 1,
      idempotencyPolicy: "required",
      reservationPolicy: "required",
      ...
      notes: "Personal member text generation reserves model-priced member credits before AI_LAB execution and stores bounded replay metadata. Fable 5 uses a disclosed safe upper-bound estimate.",
    },
    ...
    currentGaps: [],
    gapSeverity: "P3",
    nextMigrationPhase: "Actual-usage reconciliation when all providers expose stable token usage",
  }),
  operation({
    operationConfig: {
  ...
  operation({
    operationConfig: {
      ...
      notes: "Phase 4.15 enforces ENABLE_ADMIN_AI_TEXT_BUDGET before attempts or provider execution. Phase 4.8.1 admin-only provider-cost text test still requires Idempotency-Key, stores metadata-only replay state, and suppresses duplicate same-key provider calls without raw prompt or generated text.",
    },
    ...
    currentGaps: [
      ...
      "Other budget scopes remain outside the Phase 4.17 platform_admin_lab_budget cap foundation.",
    ],
    gapSeverity: "P2",
    nextMigrationPhase: "Phase 4.17 platform_admin_lab_budget cap foundation implemented; future cap hardening only",
  }),
  operation({
    operationConfig: {
      operationId: "admin.fable_chat.send",
      featureKey: "admin.ai.fable_chat",
      actorType: "admin",
      billingScope: AI_COST_GATEWAY_SCOPES.UNMETERED_ADMIN,
      providerFamily: "ai_worker",
      modelResolverKey: "admin.fable_chat.fixed_model",
      creditCost: 1,
      costPolicy: "admin_fable_chat_platform_budget",
      quantity: 1,
      idempotencyPolicy: "required",
      reservationPolicy: "platform_budget_only",
      replayPolicy: "durable_result",
      failurePolicy: "manual_review",
      storagePolicy: "none",
      observabilityEventPrefix: "admin.fable_chat.send",
      routeId: "admin.fable-chat.messages.send",
      routePath: "/api/admin/fable-chat/conversations/:id/messages",
      notes: "Private platform-admin Fable chat with server-loaded native roles, hashed idempotency keys, serialized conversations, atomic pre-provider platform-cap admission, durable pending/running/unknown state, and stored assistant replay.",
    },
    sourceFiles: [
      "workers/auth/src/routes/admin-fable-chat.js",
      "workers/auth/src/lib/fable-chat.js",
      "workers/auth/src/lib/fable-chat-budget.js",
      "workers/auth/src/lib/admin-ai-proxy.js",
      "workers/ai/src/routes/fable-chat.js",
    ...
    routePolicy: {
      id: "admin.fable-chat.messages.send",
      path: "/api/admin/fable-chat/conversations/:id/messages",
      expectedIdempotency: "required",
    },
    currentGaps: [
      "A provider acceptance followed by Worker termination before D1 finalization has an unknown outcome; stale attempts become non-retryable unknown records and never trigger automatic provider re-execution.",
    ],
    gapSeverity: "P2",
    nextMigrationPhase: "Operator reconciliation only if production evidence demonstrates a need",
  }),
  operation({
    operationConfig: {
      ...
      notes: "Priced Admin AI image test path charges selected organization credits only when its model-specific budget switch is enabled. Phase 4.15 blocks disabled requests before provider calls or credit debits.",
    },
  ...
  operation({
    operationConfig: {
      ...
      creditCost: 0,
      costPolicy: "explicit_unmetered_admin_exception",
      quantity: 1,
      ...
      routeId: "admin.ai.test-image",
      routePath: "/api/admin/ai/test-image",
      notes: "Phase 4.15 enforces ENABLE_ADMIN_AI_UNMETERED_IMAGE_TESTS before FLUX.2 Dev provider execution. Phase 4.14 keeps FLUX.2 Dev as the only explicit unmetered Admin Image lab exception with safe budget-policy and caller-policy metadata.",
    },
    sourceFiles: ["workers/auth/src/routes/admin-ai.js", "workers/ai/src/routes/image.js", "workers/ai/src/lib/invoke-ai.js"],
    currentStatus: "implemented",
    currentEnforcement: {
    ...
    currentGaps: [
      "Live platform budget caps remain future work for this explicit unmetered exception.",
  ...
  operation({
    operationConfig: {
      ...
      notes: "Phase 4.15 enforces ENABLE_ADMIN_AI_EMBEDDINGS_BUDGET before attempts or provider execution. Phase 4.8.1 admin-only provider-cost embeddings test still requires Idempotency-Key, stores metadata-only replay state, and suppresses duplicate same-key provider calls without raw embedding input or vectors.",
    },
  ...
  operation({
    operationConfig: {
      ...
      notes: "Phase 4.15 enforces ENABLE_ADMIN_AI_MUSIC_BUDGET before attempts or provider execution. Phase 4.9 admin-only MiniMax music test still requires Idempotency-Key, stores metadata-only replay state, and suppresses duplicate same-key provider calls without raw prompts, lyrics, or audio.",
    },
  ...
  operation({
    operationConfig: {
      ...
      creditCost: 0,
      costPolicy: "admin_debug_retired_disabled_by_default",
      quantity: 1,
      idempotencyPolicy: "optional",
      reservationPolicy: "not_supported",
      ...
      notes: "Phase 4.13 retires synchronous admin debug video from normal provider-cost operations. It is disabled-by-default; ALLOW_SYNC_VIDEO_DEBUG=true is retained only for emergency compatibility, while async admin video jobs are the supported budgeted admin video path.",
    },
    ...
    routePolicy: {
      ...
      expectedIdempotency: "disabled-by-default",
    },
    currentGaps: [
      "Normal provider-cost execution is retired/disabled-by-default; emergency flag execution remains compatibility-only and is not a supported budgeted path.",
      "If this sync path is ever retained beyond emergency debugging, a later phase must add required idempotency, durable budget metadata, and explicit budget kill-switch enforcement before normal use.",
    ],
    gapSeverity: "P3",
    nextMigrationPhase: "Retired by Phase 4.13; use admin async video jobs for supported budgeted video generation",
  }),
  operation({
    operationConfig: {
      ...
      notes: "Admin async video job creation requires Idempotency-Key and Phase 4.15 ENABLE_ADMIN_AI_VIDEO_JOB_BUDGET before writing job rows or queueing provider task work.",
    },
    ...
    currentGaps: ["Async video job usage recording is tied to successful queue completion; provider-success/usage-record failure reconciliation remains operator-reviewed future hardening."],
    gapSeverity: "P3",
    nextMigrationPhase: "Phase 4.17 platform_admin_lab_budget cap foundation implemented; future async reconciliation hardening only",
  }),
  operation({
    operationConfig: {
  ...
  operation({
    operationConfig: {
      ...
      creditCost: 0,
      costPolicy: "admin_platform_lab_budget_metadata_only_multi_model",
      quantity: 1,
      idempotencyPolicy: "required",
      reservationPolicy: "platform_budget_only",
      replayPolicy: "metadata_only",
      ...
      notes: "Phase 4.15 enforces ENABLE_ADMIN_AI_COMPARE_BUDGET before durable idempotency attempts or multi-model text provider fanout; Phase 4.10 still propagates signed caller-policy metadata to the internal compare route.",
    },
    ...
    currentGaps: ["Full compare result replay is intentionally unavailable; other budget scopes remain outside the Phase 4.17 platform_admin_lab_budget cap foundation."],
    gapSeverity: "P3",
    nextMigrationPhase: "Phase 4.17 platform_admin_lab_budget cap foundation implemented; future cap hardening only",
  }),
  operation({
    operationConfig: {
      ...
      creditCost: 0,
      costPolicy: "platform_admin_lab_budget_metadata",
      quantity: 1,
      idempotencyPolicy: "required",
      reservationPolicy: "platform_budget_only",
      replayPolicy: "metadata_only",
      ...
      notes: "Phase 4.15 enforces ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET before durable stream-session attempts or provider streams. Phase 4.12 covers required request idempotency, signed caller-policy propagation, and safe stream completion/failure tracking.",
    },
    ...
    currentGaps: ["Explicit output-token/duration caps remain future work; full stream result replay is intentionally unavailable."],
    gapSeverity: "P2",
    nextMigrationPhase: "Phase 4.17 platform_admin_lab_budget cap foundation implemented; future stream cap hardening only",
  }),
  operation({
    operationConfig: {
  ...
  operation({
    operationConfig: {
      ...
      notes: "Internal MiniMax music route is service-only and requires caller-side policy metadata before provider execution.",
    },
    ...
    routePolicy: null,
    currentGaps: ["Internal route must stay service-only and requires caller-side gateway enforcement; missing caller policy fails closed."],
    gapSeverity: "P2",
  ...
  operation({
    operationConfig: {
      ...
      notes: "Internal sync video route remains service-only and requires caller policy; the Auth Worker sync debug caller is disabled-by-default and caller-gated when explicitly enabled.",
    },
    ...
    routePolicy: null,
    currentGaps: ["Internal route must stay service-only and caller-gated; the Auth Worker sync debug caller is retired/disabled-by-default and should not be treated as normal budgeted generation."],
    gapSeverity: "P2",
    nextMigrationPhase: "Later targeted internal caller migration only if another supported sync-video caller is retained",
  }),
  operation({
    operationConfig: {
  ...
  operation({
    ...
    routePolicy: null,
    currentGaps: ["Admin Compare now supplies caller-policy metadata, but the shared internal compare route still allows baseline-missing metadata for other known callers to avoid a global internal AI migration."],
    gapSeverity: "P3",
    nextMigrationPhase: "Keep caller-bound; require policy for additional compare callers only after targeted caller migrations",
  }),
  operation({
    operationConfig: {
      ...
      notes: "Internal streaming route remains service-auth protected and now requires signed caller-policy metadata for the Admin Live-Agent caller added in Phase 4.12.",
    },
    ...
    routePolicy: null,
    currentGaps: ["Provider-cost execution remains delegated to caller routes; no global internal AI Worker budget ledger is introduced in Phase 4.12."],
    gapSeverity: "P2",
    nextMigrationPhase: "Keep caller-bound; add any future Live-Agent callers only through targeted caller migrations",
  }),
  operation({
    operationConfig: {
      ...
      notes: "OpenClaw ingest can schedule provider-cost News Pulse visual generation; runtime switch and openclaw_news_pulse_budget caps are enforced before provider execution.",
    },
  ...
  operation({
    operationConfig: {
      ...
      notes: "Scheduled News Pulse visual backfill can call the image provider for missing/failed thumbnails; runtime switch and openclaw_news_pulse_budget caps are enforced before provider execution.",
    },
