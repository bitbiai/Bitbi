...
export const ADMIN_IMAGE_TEST_FLUX_1_SCHNELL_MODEL_ID = "@cf/black-forest-labs/flux-1-schnell";
export const ADMIN_IMAGE_TEST_FLUX_2_KLEIN_MODEL_IDS = Object.freeze([
...
export function isChargeableAdminImageTestModel(modelId) {
  const id = String(modelId || "").trim();
  return id === ADMIN_IMAGE_TEST_FLUX_1_SCHNELL_MODEL_ID
    || ADMIN_IMAGE_TEST_FLUX_2_KLEIN_MODEL_IDS.includes(id);
}

export function calculateAdminImageTestCreditCost(modelId, params = {}) {
  const id = String(modelId || "").trim();
  let pricing = null;
  if (id === ADMIN_IMAGE_TEST_FLUX_1_SCHNELL_MODEL_ID) {
    pricing = flux1SchnellCost(params);
  } else if (ADMIN_IMAGE_TEST_FLUX_2_KLEIN_MODEL_IDS.includes(id)) {
