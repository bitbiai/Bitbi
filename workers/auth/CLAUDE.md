...
This gate is local-only and non-mutating. It does not deploy, run remote migrations, call Stripe APIs, mutate Cloudflare/GitHub settings, change secrets, enable live billing, or prove production readiness. It should be used with `docs/production-readiness/MAIN_ONLY_RELEASE_RUNBOOK.md` and `docs/production-readiness/MAIN_ONLY_RELEASE_CHECKLIST.md` when the owner intentionally deploys directly from `main`. For the Phase 3.4 member personal image AI Cost Gateway pilot, also use `docs/production-readiness/PHASE3_MEMBER_IMAGE_GATEWAY_MAIN_CHECKLIST.md`; apply and verify remote auth migration `0048_add_member_ai_usage_attempts.sql` before deploying auth Worker code that depends on `member_ai_usage_attempts`. For Phase 4.5 admin async video job budget metadata, apply and verify remote auth migration `0049_add_admin_video_job_budget_metadata.sql` before deploying auth Worker code that writes `ai_video_jobs` budget columns. For Phase 4.6 OpenClaw/News Pulse visual budget metadata, apply and verify remote auth migration `0050_add_news_pulse_visual_budget_metadata.sql` before deploying auth Worker code that writes `news_pulse_items` visual budget columns. Phase 4.7 adds no migration, but it changes both auth Worker caller propagation and AI Worker internal caller-policy validation; deploy those Worker changes together after local validation if this branch is released. Phase 4.8 adds no migration; it changes auth Worker admin text/embeddings request handling plus signed caller-policy propagation and relies on the Phase 4.7 AI Worker stripping/validation path. Phase 4.8.1 adds `0051_add_admin_ai_usage_attempts.sql`; apply and verify that migration before deploying auth Worker code that persists admin text/embeddings idempotency attempts. Phase 4.8.2 adds no migration; it adds auth Worker admin-only inspection/cleanup plus scheduled cleanup for those rows and still depends on migration `0051`. Phase 4.9 adds no migration; it extends the same `admin_ai_usage_attempts` metadata-only idempotency foundation only to Admin Music test generation and still depends on migration `0051`. Phase 4.10 adds no migration; it extends the same foundation only to Admin Compare and still depends on migration `0051`. Phase 4.11 adds no migration and audits/designs Admin Live-Agent only. Phase 4.12 adds no migration; it extends the same foundation only to Admin Live-Agent stream sessions, changes both auth Worker Live-Agent request handling and AI Worker `/internal/ai/live-agent` caller-policy validation, and still depends on migration `0051`. Phase 4.13 adds no migration; it classifies the synchronous Admin Video Debug route as retired/disabled-by-default and leaves `ALLOW_SYNC_VIDEO_DEBUG=true` as emergency-only compatibility while async admin video jobs remain the supported budgeted admin video path. Phase 4.14 adds no migration; it classifies Admin Image branches in auth Worker only, keeps charged priced models on the existing selected-organization credit path, treats FLUX.2 Dev as the sole `explicit_unmetered_admin` exception with safe metadata, and blocks unknown/unclassified Admin Image models before AI Worker/provider execution.
...
**Queue** `bitbi-ai-video-jobs` bound as `AI_VIDEO_JOBS_QUEUE` — carries async admin video jobs from `/api/admin/ai/video-jobs`. The auth worker consumes the queue, leases `ai_video_jobs` rows, verifies Phase 4.5 budget metadata for admin video jobs before signed internal AI worker task create/poll routes, ingests completed output into `USER_IMAGES`, and records success/failure/retry/poison state in D1. Phase 4.13 retires the synchronous `/api/admin/ai/test-video` route from normal provider-cost operations; it remains disabled by default and is available only for explicitly approved emergency compatibility when `ALLOW_SYNC_VIDEO_DEBUG=true`.
...
Migrations in `migrations/` are numbered sequentially from `0001_init` through `0051_add_admin_ai_usage_attempts`. Current release metadata declares latest auth D1 migration `0051_add_admin_ai_usage_attempts.sql`. Phase 4.7, Phase 4.8, and Phase 4.8.2 add no auth D1 migration; they are shared JS/auth Worker/AI Worker code plus docs/tests. Phase 4.8.1 adds only the `admin_ai_usage_attempts` table for admin text/embeddings durable metadata-only idempotency; Phase 4.8.2 reuses that table for bounded cleanup and admin-only sanitized inspection.
