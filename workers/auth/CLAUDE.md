...
- `POST /api/billing/webhooks/stripe/live` — live Stripe one-time credit-pack webhook route with separate raw-body `Stripe-Signature` verification using `STRIPE_LIVE_WEBHOOK_SECRET`; accepts only live checkout events and grants credits only for persisted live checkout rows created by an active member for their personal balance or by a currently authorized platform admin/active org owner for organization balance
