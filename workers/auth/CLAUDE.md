...
- `POST /api/admin/ai/live-agent` — proxy a streaming Admin Live-Agent request into `workers/ai` (requires admin). Phase 4.11 audits/designs this flow only: runtime behavior is unchanged, `Idempotency-Key` is not newly required, no durable Live-Agent attempts are added, and the route remains a baselined provider-cost gap for Phase 4.12 with target budget scope `platform_admin_lab_budget` and kill-switch metadata target `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`.
