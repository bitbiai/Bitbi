...
Migrations in `migrations/` are numbered sequentially from `0001_init` through `0050_add_news_pulse_visual_budget_metadata`. Current release metadata declares latest auth D1 migration `0050_add_news_pulse_visual_budget_metadata.sql`. Phase 4.7 adds no auth D1 migration; it is shared JS/auth Worker/AI Worker code plus docs/tests.
