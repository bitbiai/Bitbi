...
- `GET /api/admin/ai/admin-usage-attempts` — list sanitized admin text/embeddings/music idempotency attempts with bounded filters (requires admin; Phase 4.8.2, extended to music by Phase 4.9)
- `GET /api/admin/ai/admin-usage-attempts/:id` — inspect one sanitized admin text/embeddings/music idempotency attempt without raw prompts/input/output/vectors/audio/lyrics/idempotency keys/hashes/request fingerprints (requires admin; Phase 4.8.2, extended to music by Phase 4.9)
- `POST /api/admin/ai/admin-usage-attempts/cleanup-expired` — run a bounded dry-run/default cleanup batch for expired pending/running admin text/embeddings/music attempts; execution marks rows expired only and does not delete rows, call providers, mutate credits, or mutate billing (requires admin, same-origin, `Idempotency-Key`; Phase 4.8.2, extended to music by Phase 4.9)
