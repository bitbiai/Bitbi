...
Migrations in `migrations/` are numbered sequentially from `0001_init` through `0051_add_admin_ai_usage_attempts`. Current release metadata declares latest auth D1 migration `0051_add_admin_ai_usage_attempts.sql`. Phase 4.7 and Phase 4.8 add no auth D1 migration; they are shared JS/auth Worker/AI Worker code plus docs/tests. Phase 4.8.1 adds only the `admin_ai_usage_attempts` table for admin text/embeddings durable metadata-only idempotency.
