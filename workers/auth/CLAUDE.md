...
Migrations in `migrations/` are numbered sequentially from `0001_init` through `0048_add_member_ai_usage_attempts`. Current release metadata declares latest auth D1 migration `0048_add_member_ai_usage_attempts.sql`.
...
- `0048_add_member_ai_usage_attempts` — required before Phase 3.4 member personal image generation can require idempotency, reserve member credits before provider calls, suppress duplicate provider execution, replay safe temporary results, and release reservations on provider failure
