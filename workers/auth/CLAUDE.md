...
Cloudflare Worker providing auth API for bitbi.ai. Single-file worker (`src/index.js`) using Cloudflare D1 (SQLite) for persistence and cookie-based sessions. No framework — pure request/response handling with manual route matching.
...
- `POST /api/register` — create account (min 10-char password)
...
**Tables**: `users`, `sessions`, `password_reset_tokens`, `admin_audit_log`

Migrations in `migrations/` — numbered sequentially. Note: there are two `0002_*` migrations (admin role and password reset tokens) applied separately.
