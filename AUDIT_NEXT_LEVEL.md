...
| Tenant assets | Phase 6.1-6.25 add design/evidence, folder/image owner-map diagnostics, nullable new-write metadata, manual-review import/queue/status workflows, reset dry-run planning, reset executor design/action tracking, and pending reset dry-run evidence/confirmation-gate docs. | Collect Phase 6.26 reset executor dry-run evidence; do not broad-backfill, execute confirmed reset, or change runtime access behavior. |
...
4. Choose one engineering track only: Phase 6.26 legacy media reset operator dry-run evidence, manual-review idempotency evidence completion, billing remediation, next AI budget scope, internal caller-policy hardening, or privacy self-service.
