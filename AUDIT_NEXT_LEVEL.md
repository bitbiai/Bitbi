...
4. **Tenant ownership** - organization/RBAC exists, but existing assets are not fully tenant-owned.
...
4. Choose one engineering track only: billing remediation, next AI budget scope, internal caller-policy hardening, tenant migration, or privacy self-service.
