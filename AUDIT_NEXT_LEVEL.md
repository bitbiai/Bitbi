...
- Current aggregate validation: Phase 2-O validation is recorded in `PHASE2O_PRICING_HERO_LIVE_PACKS_AND_PROFILE_NAV_REPORT.md` and the final Codex response. Phase 2-N validation is recorded in `PHASE2N_ORGANIZATION_CONTEXT_AND_CREDIT_DEBIT_VISIBILITY_REPORT.md`. Phase 3.4.1 is documentation/checklist/check wording only. Phase 3.8 validation is local-only and must include focused member video gateway Worker tests plus the requested release/AI cost checks. Live Cloudflare validation, live Stripe setup, real production webhook tests, remote migrations, real AI provider calls, and production deploy were not run.
...
| Phase 3.3 AI Cost Operation Registry | Completed for current scope | Adds a central operation registry for known provider-cost/current-policy AI operations, current enforcement metadata, route-policy/source-file baselines, registry summary helper, and report-only check/test coverage. | `npm run test:ai-cost-operations` validates unique operation ids, normalizer compatibility, high-risk operation coverage, deterministic summary counts, duplicate rejection, and no external calls. `npm run check:ai-cost-policy` loads the registry and remains report-only by default. | Used by Phase 3.4 only for `member.image.generate`. It does not migrate music/video/admin/platform/internal routes, call providers, mutate billing, deploy, or prove production readiness. | `workers/auth/src/lib/ai-cost-operations.js`, `scripts/test-ai-cost-operations.mjs`, `scripts/check-ai-cost-policy.mjs` |
...
| Area | Status | Notes |
| --- | --- | --- |
| Merge readiness | Ready for review after Phase 3.8 validation | The current Phase 3.4 runtime/migration foundation, Phase 3.4.1 evidence guidance, Phase 3.5 decomposition, Phase 3.6 member music gateway migration, Phase 3.7 member image/music hardening, and Phase 3.8 member video gateway tests must pass the requested local validation suite; no commit was made. |
...
| Current recommended next phase | Verification evidence, billing operations, admin/platform AI cost telemetry, video entitlement, or tenant track | Because the owner deploys directly from `main`, collect main-only evidence for migration `0048`, member personal image gateway behavior, member music gateway behavior, Phase 3.7 replay/finalization/cleanup hardening, and member video gateway behavior before making live claims. Any bounded operator canary still needs immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: add admin/platform AI cost telemetry and a route-metadata guard, turn Phase 2.3 review queue UI/resolution metadata plus Phase 2.4 read-only reconciliation signals into an approved remediation workflow, wire a video route entitlement path, or migrate one tenant ownership domain. Do not combine all tracks into one broad rewrite. |
