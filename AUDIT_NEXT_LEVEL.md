...
| Pricing / Credit Purchase | Completed for current scope | Admin-only `/pricing.html`, admin-only desktop/mobile Pricing header link, and Phase 2-O runtime UI alignment with Free/5,000/12,000 live credit-pack tiers. Paid Pricing CTAs now route to `/account/credits.html`; direct checkout creation remains in the backend-gated Credits/Testmode routes, not the Pricing page. | Original validation is recorded in `PHASE_PRICING_PAGE_CREDIT_PACKS_REPORT.md`; Phase 2-O validation is recorded in `PHASE2O_PRICING_HERO_LIVE_PACKS_AND_PROFILE_NAV_REPORT.md`. No dependency, live Stripe activation, subscription, invoice, customer portal, or public rollout was added. | Staging/canary pricing link/access-gate verification, Credits dashboard checkout behavior, model status truthfulness, and no public billing side effects. | `PHASE_PRICING_PAGE_CREDIT_PACKS_REPORT.md`, `PHASE2O_PRICING_HERO_LIVE_PACKS_AND_PROFILE_NAV_REPORT.md` |
...
- Existing assets remain user-owned; full tenant isolation, public live payment activation, subscriptions, invoices, customer portal, production-trusted broad webhooks, refunds/chargebacks, broad AI route credit enforcement beyond explicitly wired member image/text and admin BFL test paths, permanent provider-result cache, full IaC/dashboard drift enforcement, full type/lint migration, full user self-service privacy flows, irreversible deletion execution, formal load budgets, and live SLO/alert evidence remain open.
...
| Billing/plans/entitlements foundation | Reduced | `0035_add_billing_entitlements.sql`, `0036_add_ai_usage_attempts.sql`, `0037_add_billing_event_ingestion.sql`, `0038_add_stripe_credit_pack_checkout.sql`, `0039_raise_credit_balance_cap_for_pricing_packs.sql`, `0040_add_live_stripe_credit_pack_scope.sql`, `workers/auth/src/lib/billing.js`, `workers/auth/src/lib/ai-usage-policy.js`, `workers/auth/src/lib/ai-usage-attempts.js`, `workers/auth/src/lib/billing-events.js`, `workers/auth/src/lib/stripe-billing.js`, `workers/auth/src/routes/orgs.js`, `workers/auth/src/routes/admin-billing.js`, `workers/auth/src/routes/billing-webhooks.js`, `workers/auth/src/routes/ai/text-generate.js`, route-policy/release updates, and Worker tests add default free plan resolution, org billing/entitlement reads, admin plan/org billing inspection, idempotent credit grants, idempotent credit consumption helpers, org-scoped image and text generation credit enforcement, credit reservations, duplicate provider-call suppression, temporary result replay, bounded cleanup, provider-neutral billing event ingestion, Stripe Testmode credit-pack checkout, server-side platform-admin-only Testmode checkout lockdown, verified Testmode checkout webhook credit grants only for admin-created persisted sessions, gated live one-time Stripe credit-pack checkout for platform admins/org owners, verified live webhook credit grants only for authorized persisted sessions, admin-only Pricing page checkout initiation, gated Credits dashboard, and sanitized admin billing/usage inspection. Subscriptions, invoices, customer portal, taxes, public pricing rollout, refund/chargeback automation, and broad/video route-level credit enforcement remain open. |
