...
- Latest implemented product/security phase: Phase 2.3 Admin Control Plane billing review queue UI on top of Phase 2.2 review APIs, Phase 2-O Pricing hero/live-pack cleanup, member credit buckets, BITBI Pro subscription scaffolding, and asset storage quota tracking.
...
| BITBI Pro member subscriptions and buckets | Completed for current scope | Adds member subscription checkout/session scaffolding, member subscription rows, member credit buckets/events, asset storage quota tracking, and release compatibility through auth migration `0047_add_member_subscriptions_and_credit_buckets.sql`. Phase 2.1 adds review-only live Stripe failure/refund/dispute/expired-checkout classification; Phase 2.2 adds admin-only review queue/detail/resolution metadata; Phase 2.3 adds Admin Control Plane UI for the review queue. | Validation evidence must be read from the implementation report/final response for that later phase; this Alpha Audit reconciliation only corrects current-state documentation. | Apply auth migrations through `0047`, configure optional live subscription price/status values only in an approved environment, verify Phase 2.3 review queue UI/resolution behavior, and implement/approve reconciliation/remediation behavior before any live billing readiness claim. | Current source docs plus `config/release-compat.json`; historical phase report needs verification if present. |
...
- Existing assets remain user-owned; full tenant isolation, production-ready subscription lifecycle, invoices, customer portal, Stripe Tax, production-trusted broad webhooks, refund/chargeback automation, approved billing reconciliation/remediation workflow, broad AI route credit enforcement beyond explicitly wired paths, permanent provider-result cache, full IaC/dashboard drift enforcement, full type/lint migration, full user self-service privacy flows, irreversible deletion execution, formal load budgets, and live SLO/alert evidence remain open. Phase 2.3 review queue UI/resolution metadata is not automated remediation and does not call Stripe or claw back credits.
...
| Current recommended next phase | Verification evidence, billing operations, video AI route, or tenant track | Prefer staging verification and deployment evidence first. If the operator intentionally skips staging, perform only bounded operator canaries with immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: turn Phase 2.3 review queue UI/resolution metadata into a reconciliation/admin remediation workflow, video route entitlement wiring, or domain-by-domain tenant ownership migration. Do not combine all tracks into one broad rewrite. |
