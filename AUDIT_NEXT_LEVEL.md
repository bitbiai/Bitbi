...
- The next implementation phase should be chosen between operator-run main-only evidence collection, billing lifecycle/reconciliation hardening, Phase 4.15 runtime budget kill-switch/live platform cap work, and domain-by-domain tenant ownership migration. Do not redo Phase 0/1/2-A/2-B/2-C/2-D/2-E/2-F/2-G/2-H/2-I/2-J or Admin Control Plane foundations.
...
| Current recommended next phase | Verification evidence, Phase 4.15 runtime caps work, billing operations, or tenant track | Because the owner deploys directly from `main`, collect main-only evidence for migration `0051`, member personal image gateway behavior, member music gateway behavior, Phase 3.7 replay/finalization/cleanup hardening, member video gateway behavior, Phase 3.9 `check:ai-cost-policy` baseline output, Phase 4.1/4.2 admin/platform budget policy design/helper output, Phase 4.3 charged Admin BFL image-test budget metadata, Phase 4.4/4.8.2/4.10/4.12/4.13/4.14 budget evidence output, Phase 4.5 admin video budget metadata/queue behavior, Phase 4.6 News Pulse visual budget metadata/status behavior, Phase 4.7 internal caller-policy guard behavior, Phase 4.8.1/4.8.2 admin text/embeddings durable idempotency plus cleanup/inspection behavior, Phase 4.9 Admin Music durable metadata-only idempotency behavior, Phase 4.10 Admin Compare durable metadata-only idempotency behavior, Phase 4.12 Admin Live-Agent stream-session idempotency/finalization evidence, Phase 4.13 sync video debug disabled-by-default evidence, and Phase 4.14 Admin Image branch classification evidence before making live claims. Any bounded operator canary still needs immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: Phase 4.15 runtime budget kill-switch/live platform caps, turn Phase 2.3 review queue UI/resolution metadata plus Phase 2.4 read-only reconciliation signals into an approved remediation workflow, or migrate one tenant ownership domain. Do not combine all tracks into one broad rewrite. |
