...
Historical audit constraint: the original 2026-04-24 audit did not change application code. This reconciled file now also summarizes later implementation phases; see phase reports for code-change evidence.

## Current Remediation Status After Phase 5.1 Admin Control Plane UX Consolidation

This section is the current implementation checkpoint as of 2026-05-17. Phase 5.1 consolidates the Admin Control Plane UX/navigation around existing implemented panels, adding budget-control wayfinding, keyboard-accessible help, status badges, responsive panel links, and deep-link aliases without backend behavior changes. Phase 4.18 adds read-only `platform_admin_lab_budget` usage reconciliation and repair evidence on top of the Phase 4.17 cap foundation. Phase 4.19 adds additive migration `0054_add_platform_budget_repair_actions.sql`, an explicit admin-approved repair executor, admin-only repair APIs, and Admin Control Plane repair controls. Phase 4.20 adds read-only bounded repair evidence report/export APIs and Admin Control Plane report/export UI over those repair action rows. Phase 4.21 adds additive migration `0055_add_platform_budget_evidence_archives.sql`, sanitized evidence archive metadata, admin-only archive create/list/detail/download/expire/cleanup APIs, and Admin Control Plane archive controls over Phase 4.20 report snapshots stored in private `AUDIT_ARCHIVE` under `platform-budget-evidence/`. The Phase 5.1 UI consolidation and the report/export/archive layers do not apply repairs, run automatic repair, mutate source attempts/jobs, mutate usage/repair rows, mutate credits, mutate member/org/customer billing, call providers, call Stripe, mutate Cloudflare, or change runtime provider route behavior. Phase 2.6 through Phase 5.1 do not deploy, run remote migrations, enable live billing, call real AI providers in tests, call Stripe, migrate new provider-cost routes, change member image/music/video billing behavior, change org-scoped member route behavior, or prove production readiness. This does not prove live billing readiness, full subscription lifecycle maturity, automated refund/dispute remediation, automatic credit clawback, broad approved reconciliation/remediation, full platform budget cap coverage, full runtime AI cost readiness, or full SaaS maturity. The original audit findings, risk ratings, score estimates, and command output below remain a historical pre-remediation baseline from 2026-04-24.

Current repository state at checkpoint:

- Branch: `main`
- Latest commit observed before this Phase 0 doc-currentness reconciliation: needs verification from local `git rev-parse HEAD` at audit time.
- Latest implemented product/security phase: Phase 5.1 Admin Control Plane UX/navigation consolidation, on top of Phase 4.21 sanitized `platform_admin_lab_budget` evidence archive/retention workflow, Phase 4.20 repair evidence report/export, Phase 4.19 explicit admin-approved usage repair execution, Phase 4.18 read-only reconciliation, Phase 4.17 cap foundation, and the prior AI Cost Gateway/admin/billing foundations.
- Working tree at start of this reconciliation: clean.
...
- Current aggregate validation: Phase 2-O validation is recorded in `PHASE2O_PRICING_HERO_LIVE_PACKS_AND_PROFILE_NAV_REPORT.md` and the final Codex response. Phase 2-N validation is recorded in `PHASE2N_ORGANIZATION_CONTEXT_AND_CREDIT_DEBIT_VISIBILITY_REPORT.md`. Phase 3.4.1 is documentation/checklist/check wording only. Phase 4.12 validation is local-only and must include focused Admin Live-Agent budget/idempotency/caller-policy Worker tests, deterministic registry/baseline/check/evidence tests, focused Admin Compare budget/idempotency Worker tests, focused Admin Music budget/idempotency Worker tests, admin text/embeddings cleanup/inspection Worker tests, durable idempotency Worker tests, internal caller-policy Worker tests, News Pulse/OpenClaw focused Worker tests, admin async video job focused Worker tests, charged Admin BFL image-test focused Worker tests, admin/platform budget policy helper tests, AI cost policy baseline/check tests, evidence tests, and the requested release checks. Live Cloudflare validation, live Stripe setup, real production webhook tests, remote migrations, real AI provider calls, and production deploy were not run.
- Production deploy status: blocked until live Cloudflare secret/binding/resource validation, auth migrations through `0055_add_platform_budget_evidence_archives.sql`, Stripe Testmode secret/URL/webhook endpoint verification, explicit `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true` canary enablement when needed, live Stripe credit-pack config with `ENABLE_LIVE_STRIPE_CREDIT_PACKS=true` only for a bounded operator canary, BITBI Pro subscription config verification if subscriptions are exposed, Worker verification, data lifecycle cleanup/executor verification, organization-flow verification, billing/entitlement/credit verification, org-scoped AI image/text usage retry/reservation verification, member personal image gateway idempotency/reservation/replay verification, member music gateway idempotency/reservation/replay/no-charge failure verification, member video gateway idempotency/reservation/replay/no-charge failure verification, admin async video job idempotency/budget metadata/queue-consumer verification, News Pulse visual budget metadata/status verification, admin text/embeddings durable idempotency verification, Admin Music durable metadata-only idempotency verification, AI usage attempt cleanup/admin inspection verification, Phase 4.15.1 Admin AI Budget Switch Control Plane list/update/effective-state verification, Phase 4.17 cap foundation verification, Phase 4.18 reconciliation verification, Phase 4.19 repair action verification, Phase 4.20 repair report/export evidence verification, Phase 4.21 archive create/download/expire/cleanup verification, replay object cleanup verification, synthetic billing webhook verification/admin inspection/no-side-effect verification, Stripe Testmode platform-admin-only checkout/webhook/exactly-once credit grant/non-admin-created no-credit verification, Phase 2-L live checkout/webhook/exactly-once credit grant/denial verification, Phase 2-M/4.3 Admin AI Lab BFL credit charging and budget-metadata verification, Phase 2-N Organization/Credits/Admin AI Lab context-sharing and debit-visibility verification, member credit bucket/subscription/storage-quota behavior, Phase 2.3 billing review UI/resolution no-mutation behavior, Phase 2.4 read-only billing reconciliation report verification, Phase 2.6 main-only release evidence if deploying directly from `main`, Admin Control Plane UI verification, and admin-only Pricing checkout/return/model-status verification are complete.
...
| Phase 2-K | Completed for current scope | Server-side platform-admin-only Stripe Testmode checkout creation, explicit `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true` kill switch, route-policy/release updates, and webhook credit grants restricted to persisted checkout sessions created by active platform admins. | Targeted `check:js` and Phase 2-J/2-K Worker tests passed during implementation; full validation required before merge. | Verify admin-only Testmode checkout canary, disabled flag failure, non-admin/org-owner/org-admin denial, admin-created session grants, non-admin-created session no-credit behavior, and no live billing side effects. | `PHASE2K_ADMIN_STRIPE_TESTMODE_LOCKDOWN_REPORT.md` |
| Phase 2-L | Completed for current scope | Adds narrow live Stripe one-time credit packs for platform admins and active organization owners, live webhook endpoint, exactly-once live credit grants after persisted checkout authorization revalidation, additive migration `0040`, and gated `/account/credits.html` dashboard/profile nav link. | Baseline preflight, `check:js`, route-policy check, Worker 352/352, and focused Credits static tests passed during implementation; full validation required before merge. | Apply migration `0040`, configure live Stripe values with `ENABLE_LIVE_STRIPE_CREDIT_PACKS=false` by default, then verify live checkout authorization, live webhook signature validation, exactly-once grant, no-credit failure paths, org-owner revocation behavior, and no public pricing exposure in staging or a bounded operator canary. | `PHASE2L_LIVE_STRIPE_CREDIT_PACKS_AND_CREDITS_DASHBOARD_REPORT.md` |
| Phase 2-M | Completed for current scope | Updates live credit-pack economics to 5,000 credits for 9.99 EUR and 12,000 credits for 19.99 EUR, retires new `live_credits_10000` checkout creation, and charges platform-admin-only Admin AI image tests for Flux 1 schnell and Flux 2 klein 9B against a selected organization's existing credits. | Baseline preflight, focused Phase 2-M/Phase 2-L Worker tests, focused Admin AI/Credits static tests, Worker 359/359, static 168/168, static build, and release preflight passed. | Verify Admin AI Lab org selector, BFL cost labels, sufficient-credit denial, provider-failure no-charge behavior, exactly-once debit/idempotency, sanitized ledger/Credits dashboard display, and no member/public charging change in staging or bounded canary. | `PHASE2M_ADMIN_BFL_IMAGE_TEST_CREDIT_PRICING_REPORT.md` |
| Phase 2-N | Completed for current scope | Adds `/account/organization.html`, `GET /api/orgs/:id/organization-dashboard`, shared active organization context, profile Organization link, owner/platform-admin role visibility, and Admin AI Lab debit diagnostics. | Targeted Worker/static tests passed during implementation; full validation is recorded in the Phase 2-N report/final response. | Verify solo-admin BITBI owner setup, active organization selector/defaulting, Organization/Credits/Admin AI Lab shared context, post-test balance refresh, and debit diagnostics against real staging/canary data. | `PHASE2N_ORGANIZATION_CONTEXT_AND_CREDIT_DEBIT_VISIBILITY_REPORT.md` |
| Phase 2-O | Completed for current scope | Redesigns `/pricing.html` hero, removes visible stale Testmode runtime copy, shows Free/5,000/12,000 live credit-pack tiers, routes paid CTAs to `/account/credits.html`, and compacts profile navigation with Wallet/Credits/Organization. | Validation is recorded in the Phase 2-O report/final response. | Verify Pricing copy/CTA behavior, profile navigation eligibility/layout, and Credits dashboard checkout behavior against real auth/org states in staging/canary. | `PHASE2O_PRICING_HERO_LIVE_PACKS_AND_PROFILE_NAV_REPORT.md` |
| BITBI Pro member subscriptions and buckets | Completed for current scope | Adds member subscription checkout/session scaffolding, member subscription rows, member credit buckets/events, asset storage quota tracking, and release compatibility through auth migration `0047_add_member_subscriptions_and_credit_buckets.sql`. Phase 2.1 adds review-only live Stripe failure/refund/dispute/expired-checkout classification; Phase 2.2 adds admin-only review queue/detail/resolution metadata; Phase 2.3 adds Admin Control Plane UI for the review queue; Phase 2.4 adds a read-only local D1 billing reconciliation report. | Validation evidence must be read from the implementation report/final response for that later phase; this Alpha Audit reconciliation only corrects current-state documentation. | Apply auth migrations through `0047`, configure optional live subscription price/status values only in an approved environment, verify Phase 2.3 review queue UI/resolution behavior and Phase 2.4 local-only reconciliation reporting, and implement/approve remediation behavior before any live billing readiness claim. | Current source docs plus `config/release-compat.json`; historical phase report needs verification if present. |
| Phase 3.1 AI Cost Gateway design and route inventory | Completed for current scope | Adds AI Cost Gateway documentation, provider-cost route inventory, target lifecycle design, phased roadmap, and report-only `check:ai-cost-policy` tooling. | Local validation covers documentation currentness and the report-only policy check/test. | Design phase itself did not change runtime behavior; Phase 3.4 later uses the foundation only for member personal image generation. Full route coverage remains incomplete. | `docs/ai-cost-gateway/README.md`, `docs/ai-cost-gateway/AI_COST_ROUTE_INVENTORY.md`, `docs/ai-cost-gateway/AI_COST_GATEWAY_DESIGN.md`, `docs/ai-cost-gateway/AI_COST_GATEWAY_ROADMAP.md` |
| Phase 3.2 AI Cost Gateway contract and tests | Completed for current scope | Adds gateway constants, config normalization, request fingerprinting, scoped idempotency key helpers, pure plan classification, and deterministic tests. | `npm run test:ai-cost-gateway` covers valid/invalid configs, fingerprint determinism, volatile-field exclusion, prompt hashing, scoped idempotency, plan states, and no external calls. | Used by Phase 3.4 only for member personal image generation. It does not prove full AI cost readiness or migrate music/video/admin/platform/internal routes. | `workers/auth/src/lib/ai-cost-gateway.js`, `scripts/test-ai-cost-gateway.mjs` |
| Phase 3.3 AI Cost Operation Registry | Completed for current scope | Adds a central operation registry for known provider-cost/current-policy AI operations, current enforcement metadata, route-policy/source-file baselines, registry summary helper, and initial report-only check/test coverage. | `npm run test:ai-cost-operations` validates unique operation ids, normalizer compatibility, high-risk operation coverage, deterministic summary counts, duplicate rejection, and no external calls. Phase 3.9 later turns default `npm run check:ai-cost-policy` into a baseline-enforced local guard. | Used by member image/music/video gateway operations. It does not migrate admin/platform/internal routes, call providers, mutate billing, deploy, or prove production readiness. | `workers/auth/src/lib/ai-cost-operations.js`, `scripts/test-ai-cost-operations.mjs`, `scripts/check-ai-cost-policy.mjs` |
| Phase 3.4 Member personal image gateway pilot | Completed for current scope | Adds additive migration `0048_add_member_ai_usage_attempts.sql`, member attempt helpers, and a gateway adapter for only member personal `/api/ai/generate-image`. The route now requires `Idempotency-Key`, reserves member credits before provider execution, suppresses same-key duplicate provider calls, replays safe temporary image metadata when available, debits once after success, and releases/no-charges on provider failure. | Focused Worker tests cover missing/malformed idempotency, insufficient credits before provider, provider failure no debit, success exactly-once debit, same-key replay/no provider call, conflict behavior, safe metadata, and org/admin compatibility. | Does not migrate music, video, admin AI, platform/background AI, internal AI Worker routes, org-scoped routes, pricing, Stripe, public billing, or production readiness. Migration `0048` must be applied by an operator before auth deploy. | `workers/auth/migrations/0048_add_member_ai_usage_attempts.sql`, `workers/auth/src/lib/member-ai-usage-attempts.js`, `workers/auth/src/lib/ai-usage-policy.js`, `workers/auth/src/routes/ai/images-write.js` |
| Phase 3.4.1 Member image gateway main-only release evidence | Completed for documentation/checklist scope | Adds a dedicated main-only release checklist for the Phase 3.4 member personal image pilot, updates the main-only runbook/checklist/evidence template, and clarifies that migration `0048` must be applied and verified before auth Worker deployment. | Local docs/check tooling validation only. | Does not deploy, apply remote migrations, call AI providers, mutate Cloudflare/Stripe/GitHub, change route behavior, change credit behavior, enable live billing, or prove production readiness. | `docs/production-readiness/PHASE3_MEMBER_IMAGE_GATEWAY_MAIN_CHECKLIST.md`, `docs/production-readiness/MAIN_ONLY_RELEASE_RUNBOOK.md`, `docs/production-readiness/MAIN_ONLY_RELEASE_CHECKLIST.md`, `docs/production-readiness/EVIDENCE_TEMPLATE.md` |
| Phase 3.5 Member music cost decomposition | Completed for design/check scope | Decomposes member music into parent request, optional lyrics, required audio, and background cover provider-cost operations; adds `MEMBER_MUSIC_COST_DECOMPOSITION.md`; updates the registry/check output/tests for music sub-operation gaps. | Deterministic AI cost operation/policy tests cover the music parent/lyrics/audio/cover entries, bundled metadata, and no external calls. | Historical prep phase; runtime migration happens in Phase 3.6. | `docs/ai-cost-gateway/MEMBER_MUSIC_COST_DECOMPOSITION.md`, `workers/auth/src/lib/ai-cost-operations.js`, `scripts/check-ai-cost-policy.mjs`, `scripts/test-ai-cost-policy.mjs`, `scripts/test-ai-cost-operations.mjs` |
| Phase 3.6 Member music gateway migration | Completed for current scope | Migrates only member `/api/ai/generate-music` to mandatory idempotency, one parent member attempt reservation, duplicate provider-call suppression, safe persisted-asset replay metadata, exactly-once debit after audio persistence, no-charge lyrics/audio provider failure handling, terminal no-paid-replay storage/billing failure handling, and bundled cover policy. | Focused Worker tests cover missing/malformed idempotency, insufficient credits, provider/storage/billing failures, duplicate in-progress suppression, completed replay, conflict behavior, safe metadata, and no external calls. | Does not add a migration, deploy, apply remote migrations, call real AI providers in tests, call Stripe, change public pricing, migrate video/admin/platform/internal/OpenClaw routes, or prove production readiness. | `workers/auth/src/routes/ai/music-generate.js`, `workers/auth/src/lib/ai-usage-policy.js`, `workers/auth/src/lib/member-ai-usage-attempts.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/app/route-policy.js`, `tests/workers.spec.js` |
| Phase 3.7 Member image/music gateway hardening | Completed for current scope | Hardens only migrated member image/music gateway flows for replay-unavailable no-provider/no-double-debit behavior, safe replay response metadata, music cover status writeback, explicit finalization failure handling, and scheduled member-attempt cleanup/expiry. | Focused Worker tests cover member image missing replay object, member music missing replay metadata, cover success/failure status writeback, scheduled cleanup release/temp-object deletion safety, and no external calls. | Does not add a migration, deploy, apply remote migrations, call real AI providers in tests, call Stripe, change pricing, migrate member video/admin/platform/internal/OpenClaw routes, or prove production readiness. | `workers/auth/src/lib/member-ai-usage-attempts.js`, `workers/auth/src/routes/ai/images-write.js`, `workers/auth/src/routes/ai/music-generate.js`, `workers/auth/src/lib/member-music-cover.js`, `workers/auth/src/index.js`, `tests/workers.spec.js` |
| Phase 3.8 Member video gateway migration | Completed for current scope | Migrates only member `/api/ai/generate-video` to mandatory idempotency, one parent member attempt reservation, duplicate provider-call suppression, safe durable saved-asset replay metadata, exactly-once debit after durable video persistence, and safe provider/storage/finalization failure handling. | Focused Worker tests cover missing/malformed idempotency, insufficient credits, provider/storage/finalization failure safety, duplicate in-progress suppression, same-key replay, replay-unavailable no-provider/no-double-debit behavior, and no external calls. | Does not add a migration, deploy, apply remote migrations, call real AI providers in tests, call Stripe, change pricing, migrate admin video jobs/admin/platform/internal/OpenClaw routes, or prove production readiness. | `workers/auth/src/routes/ai/video-generate.js`, `workers/auth/src/lib/ai-usage-policy.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/app/route-policy.js`, `tests/workers.spec.js` |
| Phase 3.9 AI Cost Gateway enforcement guard | Completed for validation/check scope | Adds `config/ai-cost-policy-baseline.json`, validates known admin/platform/internal/OpenClaw gaps, fails default `check:ai-cost-policy` on unregistered provider-cost source files, duplicate registry/baseline ids, unbaselined route-policy gaps, missing baseline references, and member image/music/video gateway regressions, and includes the default guard in release preflight. | Deterministic policy tests cover baseline load, duplicate baseline ids, duplicate registry ids, known gap pass behavior, unknown provider-cost route failure, migrated member route regression failure, strict-mode determinism, no secret output, and no external calls. | Does not add a migration, deploy, apply remote migrations, call real AI providers in tests, call Stripe, change runtime route behavior, mutate credits, change pricing, migrate admin/platform/internal/OpenClaw routes, or prove production readiness. | `config/ai-cost-policy-baseline.json`, `scripts/check-ai-cost-policy.mjs`, `scripts/test-ai-cost-policy.mjs`, `scripts/lib/release-plan.mjs` |
| Phase 4.1 Admin/platform AI budget policy design | Completed for design/check scope | Defines budget scopes for admin/org-charged admin tests, platform admin lab budget, platform background budget, OpenClaw/News Pulse budget, internal AI Worker caller-enforced routes, explicit unmetered admin exceptions, and external-provider-only cases. Updates registry metadata, known-gap baseline metadata, AI cost policy report grouping, and docs. | Deterministic policy/operation tests must cover budget scope taxonomy, baseline future phase/temporary allowance fields, admin/platform/internal target budget scopes, member image/music/video exclusion from baseline gaps, default baseline pass, strict-mode failure while gaps remain, and no external calls. | Does not add a migration, deploy, apply remote migrations, call real AI providers in tests, call Stripe, change runtime route behavior, mutate credits, change pricing, migrate admin/platform/internal/OpenClaw routes, add Admin UI, or prove production readiness. | `docs/ai-cost-gateway/ADMIN_PLATFORM_BUDGET_POLICY.md`, `workers/auth/src/lib/ai-cost-operations.js`, `config/ai-cost-policy-baseline.json`, `scripts/check-ai-cost-policy.mjs`, `scripts/test-ai-cost-policy.mjs`, `scripts/test-ai-cost-operations.mjs` |
| Phase 4.2 Admin/platform AI budget policy contract | Completed for contract/helper/test scope | Adds a pure admin/platform budget policy helper contract for scope validation, kill-switch metadata, safe audit fields, deterministic fingerprints, and budget plan classification. Tightens the known-gap baseline/check requirements so every admin/platform/internal gap carries a kill-switch target or explicit exemption and a future enforcement path. | Deterministic tests cover valid admin lab/background/OpenClaw/internal operations, explicit-unmetered justification, missing budget scope/idempotency/kill-switch failures, safe audit redaction, fingerprint determinism, plan statuses, baseline field requirements, no secret output, and no external calls. | Does not add a migration, deploy, apply remote migrations, call real AI providers in tests, call Stripe, import the helper into runtime routes, change credits/pricing, migrate admin/platform/internal/OpenClaw routes, add Admin UI, or prove production readiness. | `workers/auth/src/lib/admin-platform-budget-policy.js`, `scripts/test-admin-platform-budget-policy.mjs`, `config/ai-cost-policy-baseline.json`, `scripts/check-ai-cost-policy.mjs`, `scripts/test-ai-cost-policy.mjs`, `scripts/test-ai-cost-operations.mjs` |
| Phase 4.3 Charged Admin BFL image-test budget hardening | Completed for narrow runtime scope | Imports the admin/platform budget policy helper only into the existing charged Admin image-test branch for supported BFL models, adds safe `admin_org_credit_account` plan/audit metadata to admin responses, usage events, and attempts, and preserves selected-organization debit/no-charge semantics. | Focused Worker tests cover budget-policy metadata, malformed idempotency rejection before provider calls, safe response/usage/attempt metadata, same-key replay metadata, and no raw prompt/secret/header leakage. | Does not add a migration, deploy, apply remote migrations, call real AI providers in tests, call Stripe, enforce a new runtime env kill switch, change public pricing, migrate broad admin AI/admin video/admin music/admin text/compare/live-agent/platform/internal/OpenClaw routes, or prove production readiness. | `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/admin-platform-budget-policy.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/app/route-policy.js`, `tests/workers.spec.js` |
| Phase 4.4 Admin/platform AI budget evidence collector | Completed for read-only evidence/reporting scope | Adds a local evidence helper, admin-only read endpoint, local report script, and route-policy metadata for operators to see covered, baselined, unmetered, caller-enforced, external-only, and missing-runtime-enforcement flows. The report stays blocked while known gaps remain. | Deterministic tests cover budget-scope grouping, Admin BFL implemented/hardened evidence, member image/music/video gateway coverage, admin video/OpenClaw/internal baseline gaps, report bounds, sanitization, blocked verdict, no provider calls, endpoint admin denial, and route-policy coverage. | Does not add runtime budget enforcement, route migration, D1 migration, deploy, remote migration, provider calls, Stripe calls, Cloudflare/GitHub/DNS/WAF/secrets mutation, live billing, credit mutation, public billing, Admin UI, remediation automation, or production readiness. | `workers/auth/src/lib/admin-platform-budget-evidence.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/app/route-policy.js`, `scripts/report-ai-budget-evidence.mjs`, `scripts/test-admin-platform-budget-evidence.mjs`, `tests/workers.spec.js` |
| Phase 4.5 Admin async video job budget enforcement | Completed for narrow runtime scope | Adds additive migration `0049`, uses the admin/platform budget policy helper only for `POST /api/admin/ai/video-jobs`, stores sanitized `platform_admin_lab_budget` job metadata, queues a bounded budget summary, validates job budget state before internal video task create/poll, and suppresses duplicate provider task creation after a create attempt. | Focused Worker tests cover admin-only access, idempotent same-key reuse, same-key/different-body conflict, safe job/queue metadata, invalid budget policy denial before queue/provider calls, missing metadata denial before provider calls, duplicate queue retry suppression, output-persistence failure safety, route-policy metadata, and no raw prompt/secret/header leakage. | Adds an additive local migration but does not apply it remotely, deploy, call real AI providers in tests, call Stripe, change public pricing, mutate credits, enforce a new runtime env kill switch, migrate broad Admin AI/Admin music/text/compare/live-agent/OpenClaw/platform/background routes, globally enforce internal AI Worker routes, or prove production readiness. | `workers/auth/src/lib/ai-video-jobs.js`, `workers/auth/migrations/0049_add_admin_video_job_budget_metadata.sql`, `workers/auth/src/lib/ai-cost-operations.js`, `config/ai-cost-policy-baseline.json`, `workers/auth/src/app/route-policy.js`, `tests/workers.spec.js` |
| Phase 4.6 OpenClaw/News Pulse visual budget controls | Completed for narrow runtime scope | Adds additive migration `0050`, uses the admin/platform budget policy helper only for OpenClaw/News Pulse visual generation/backfill, stores sanitized `openclaw_news_pulse_budget` visual metadata, blocks invalid budget policy before provider execution, and preserves ready/pending/status/attempt duplicate suppression. | Focused Worker tests cover OpenClaw signed ingest compatibility, scheduled visual metadata, invalid budget policy provider blocking, ready/pending duplicate suppression, provider/storage failure safety, public read fallback, route-policy/check updates, and no raw prompt/secret/header/R2-key leakage. | Adds an additive local migration but does not apply it remotely, deploy, call real AI providers in tests, call Stripe, change public pricing, mutate credits, enforce a new runtime env kill switch, migrate broad Admin AI/Admin music/text/compare/live-agent/Admin video beyond Phase 4.5/platform-background routes outside News Pulse visuals, globally enforce internal AI Worker routes, or prove production readiness. | `workers/auth/src/lib/news-pulse-visuals.js`, `workers/auth/src/lib/news-pulse.js`, `workers/auth/src/routes/openclaw-news-pulse.js`, `workers/auth/migrations/0050_add_news_pulse_visual_budget_metadata.sql`, `workers/auth/src/lib/ai-cost-operations.js`, `config/ai-cost-policy-baseline.json`, `workers/auth/src/app/route-policy.js`, `tests/workers.spec.js` |
| Phase 4.7 Internal AI Worker caller-policy guard | Completed for narrow validation/metadata scope | Adds a signed reserved JSON body metadata contract, validates caller policy in the AI Worker after service-auth, strips policy metadata before provider handlers, and propagates safe metadata from covered Auth Worker callers where feasible. | Focused Worker tests cover service-auth-before-policy order, required policy on video-task create, malformed policy rejection, valid policy acceptance, Auth Worker propagation, and reserved-key stripping before mocked provider payloads. | Adds no migration, deploy, remote migration, Stripe call, real provider call in tests, credit mutation, public billing change, broad Admin AI migration, Admin music/text/compare/live-agent migration, OpenClaw/News Pulse change beyond Phase 4.6 compatibility, global hard-fail internal route migration, member image/music/video billing change, or production readiness proof. | `workers/shared/ai-caller-policy.mjs`, `workers/ai/src/lib/caller-policy.js`, `workers/ai/src/index.js`, `workers/ai/src/lib/validate.js`, `workers/auth/src/lib/admin-ai-proxy.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/ai-video-jobs.js`, `workers/auth/src/routes/ai/music-generate.js`, `tests/workers.spec.js` |
| Phase 4.8 Admin text/embeddings budget enforcement | Completed for narrow runtime scope | Uses the admin/platform budget policy helper only for `POST /api/admin/ai/test-text` and `POST /api/admin/ai/test-embeddings`, requires `Idempotency-Key`, returns sanitized `platform_admin_lab_budget` metadata, and propagates signed `budget_metadata_only` caller-policy metadata to the AI Worker. | Focused Worker tests cover non-admin denial, missing/malformed idempotency before internal calls, safe budget/caller metadata, metadata stripping before mocked provider payloads, AI Worker malformed policy rejection, and no raw prompt/input/secret/header leakage. | Adds no migration, deploy, remote migration, Stripe call, real provider call in tests, credit mutation, credit clawback, public billing change, Admin music/video/compare/live-agent migration, OpenClaw/News Pulse change, platform/background migration, unrelated internal route migration, member image/music/video or org-scoped member route behavior change, or production readiness proof. | `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `workers/auth/src/app/route-policy.js`, `config/ai-cost-policy-baseline.json`, `scripts/check-ai-cost-policy.mjs`, `tests/workers.spec.js` |
| Phase 4.8.1 Admin text/embeddings durable idempotency foundation | Completed for narrow runtime scope | Adds additive migration `0051` and `admin_ai_usage_attempts` metadata-only attempts for `POST /api/admin/ai/test-text` and `POST /api/admin/ai/test-embeddings`; same-key/same-request pending, completed, or failed attempts do not re-run provider-cost work, and same-key/different-request retries conflict before provider execution. | Focused Worker tests cover missing/malformed idempotency before provider calls, completed duplicate suppression, in-progress duplicate suppression, same-key/different-request conflict, provider-failure terminal attempts, table-unavailable fail-closed behavior, safe metadata, and no raw prompt/input/vector/secret/header leakage. | Adds no deploy, remote migration, Stripe call, real provider call in tests, credit mutation, credit clawback, public billing change, Admin music/video/compare/live-agent migration, OpenClaw/News Pulse change, platform/background migration, unrelated internal route migration, member image/music/video or org-scoped member route behavior change, full result replay, runtime env kill-switch enforcement, or production readiness proof. | `workers/auth/migrations/0051_add_admin_ai_usage_attempts.sql`, `workers/auth/src/lib/admin-ai-idempotency.js`, `workers/auth/src/routes/admin-ai.js`, `tests/helpers/auth-worker-harness.js`, `tests/workers.spec.js`, AI cost registry/check/evidence docs and scripts |
| Phase 4.8.2 Admin text/embeddings usage-attempt cleanup and inspection | Completed for narrow operational scope | Adds admin-only sanitized list/detail APIs, a dry-run-default bounded cleanup endpoint, scheduled bounded cleanup, route-policy entries, and budget-evidence summary fields for `admin_ai_usage_attempts`; cleanup marks only expired pending/running rows and retains completed/failed rows. | Focused Worker tests cover non-admin denial, sanitized list/detail, invalid filters, dry-run no mutation, cleanup execution for expired active rows only, completed-row retention, rate-limit/same-origin/table-unavailable safety, scheduled cleanup isolation, and no provider/credit/billing side effects. | Adds no migration, deploy, remote migration, Stripe call, real provider call in tests, credit mutation, credit clawback, public billing change, Admin music/video/compare/live-agent migration, OpenClaw/News Pulse change, platform/background migration, unrelated internal route migration, member image/music/video or org-scoped member route behavior change, destructive deletion, Admin UI, or production readiness proof. | `workers/auth/src/lib/admin-ai-idempotency.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/index.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `workers/auth/src/app/route-policy.js`, `tests/helpers/auth-worker-harness.js`, `tests/workers.spec.js` |
| Phase 4.9 Admin Music budget enforcement | Completed for narrow metadata-only runtime scope | Uses the admin/platform budget helper only for `POST /api/admin/ai/test-music`, requires valid `Idempotency-Key`, stores sanitized `platform_admin_lab_budget` metadata-only attempts, suppresses duplicate same-key provider calls, conflicts same-key/different-request retries, and propagates signed caller policy to `/internal/ai/test-music`. | Focused Worker tests cover missing/malformed idempotency, completed and in-progress duplicate suppression, same-key/different-request conflict, provider failure terminal attempts, table-unavailable fail-closed behavior, signed caller-policy propagation, metadata stripping, and no raw prompt/lyrics/audio/provider body leakage. | Adds no migration, deploy, remote migration, Stripe call, real provider call in tests, credit mutation, credit clawback, public billing change, Admin Compare/Live-Agent/sync video/unmetered image migration, OpenClaw/News Pulse change, platform/background migration, unrelated internal route migration, member image/music/video or org-scoped member route behavior change, full result/audio replay, runtime env kill-switch enforcement, or production readiness proof. | `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/admin-ai-idempotency.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `workers/auth/src/app/route-policy.js`, `tests/workers.spec.js` |
| Phase 4.10 Admin Compare budget enforcement | Completed for narrow metadata-only runtime scope | Uses the admin/platform budget helper only for `POST /api/admin/ai/compare`, requires valid `Idempotency-Key`, stores sanitized `platform_admin_lab_budget` metadata-only attempts, suppresses duplicate same-key provider fanout, conflicts same-key/different-request retries, and propagates signed caller policy to `/internal/ai/compare`. | Focused Worker tests cover missing/malformed idempotency, completed and in-progress duplicate suppression, same-key/different-request conflict, all-provider failure terminal attempts, table-unavailable fail-closed behavior, signed caller-policy propagation, metadata stripping, partial-success metadata, and no raw prompt/compare-output/provider body leakage. | Adds no migration, deploy, remote migration, Stripe call, real provider call in tests, credit mutation, credit clawback, public billing change, Admin Live-Agent/sync video/unmetered image migration, Admin Music changes beyond Phase 4.9, OpenClaw/News Pulse change, platform/background migration, unrelated internal route migration, member image/music/video or org-scoped member route behavior change, full compare-result replay, runtime env kill-switch enforcement, or production readiness proof. | `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/admin-ai-idempotency.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `workers/auth/src/app/route-policy.js`, `tests/workers.spec.js` |
| Phase 4.11 Admin Live-Agent budget flow audit | Completed for audit/design/prep scope only | Audits `POST /api/admin/ai/live-agent`, `/internal/ai/live-agent`, current admin authorization/MFA policy, service-auth/caller-policy behavior, one-call streaming Workers AI model use, request/response shape, lack of persistence, lack of idempotency, and Phase 4.12 target enforcement model. | Deterministic tests/checks keep Admin Live-Agent baselined and not implemented while asserting target budget scope `platform_admin_lab_budget`, future phase `Phase 4.12 admin live-agent budget enforcement`, kill-switch target `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`, and design-prep evidence. | Adds no migration, deploy, remote migration, Stripe call, real provider call, credit mutation, credit clawback, public billing change, Admin Live-Agent runtime behavior change, new Live-Agent `Idempotency-Key` requirement, durable Live-Agent attempts, caller-policy enforcement change, Admin Music/Compare/Text/Video/OpenClaw/member route change, runtime env kill-switch enforcement, or production readiness proof. | `docs/ai-cost-gateway/ADMIN_LIVE_AGENT_BUDGET_FLOW_AUDIT.md`, `workers/auth/src/lib/ai-cost-operations.js`, `config/ai-cost-policy-baseline.json`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `scripts/check-ai-cost-policy.mjs`, AI cost policy tests/docs |
| Phase 4.12 Admin Live-Agent budget enforcement | Completed for narrow metadata-only runtime scope | Uses the admin/platform budget helper only for `POST /api/admin/ai/live-agent`, requires valid `Idempotency-Key`, stores sanitized `platform_admin_lab_budget` metadata-only stream-session attempts, suppresses duplicate same-key provider streams, conflicts same-key/different-request retries, propagates signed caller policy to `/internal/ai/live-agent`, and records observable stream completion/failure summaries. | Focused Worker tests cover missing/malformed idempotency, completed and in-progress duplicate suppression, same-key/different-request conflict, provider setup failure terminal attempts, table-unavailable fail-closed behavior, signed caller-policy propagation, metadata stripping, stream success finalization, and no raw message/output/provider body leakage. | Adds no migration, deploy, remote migration, Stripe call, real provider call in tests, credit mutation, credit clawback, public billing change, sync video/unmetered image migration, Admin Text/Embeddings/Music/Compare changes beyond prior phases, OpenClaw/News Pulse change, platform/background migration, unrelated internal route migration, member image/music/video or org-scoped member route behavior change, full stream replay, runtime env kill-switch enforcement, or production readiness proof. | `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/admin-ai-idempotency.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `workers/auth/src/app/route-policy.js`, `workers/ai/src/lib/caller-policy.js`, `tests/workers.spec.js` |
| Phase 4.13 Sync Video Debug retirement decision | Completed for narrow retirement/classification scope | Audits `POST /api/admin/ai/test-video`, chooses Path A, and classifies the sync debug route as retired from normal provider-cost operations. The route remains disabled by default and is retained only behind `ALLOW_SYNC_VIDEO_DEBUG=true` for explicitly approved emergency compatibility; async admin video jobs remain the supported budgeted path. | Deterministic checks/tests cover route-policy retirement metadata, default disabled behavior with no AI Worker/provider call, evidence-report retired debug status, check output classification, and existing async admin video job compatibility. | Adds no migration, deploy, remote migration, Stripe call, real provider call, credit mutation, credit clawback, public billing change, Admin Text/Embeddings/Music/Compare/Live-Agent behavior change, Admin async video job behavior change, unmetered image migration, OpenClaw/News Pulse change, member route change, runtime env kill-switch enforcement, live platform caps, or production readiness proof. | `docs/ai-cost-gateway/ADMIN_SYNC_VIDEO_DEBUG_RETIREMENT_AUDIT.md`, `workers/auth/src/app/route-policy.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `config/ai-cost-policy-baseline.json`, `scripts/check-ai-cost-policy.mjs`, AI cost policy tests, `tests/workers.spec.js` |
| Phase 4.14 Admin Image branch handling | Completed for narrow branch-classification scope | Classifies `POST /api/admin/ai/test-image` branches so charged priced models remain on the selected-organization `admin_org_credit_account` path, FLUX.2 Dev is an intentional `explicit_unmetered_admin` exception with sanitized metadata and future kill-switch target `ENABLE_ADMIN_AI_UNMETERED_IMAGE_TESTS`, and unknown/unclassified Admin Image models block before AI Worker/provider execution. | Focused Worker/check tests cover charged branch compatibility, explicit-unmetered budget/caller metadata, metadata stripping before provider payloads, unsupported model no-AI-Worker/provider/credit behavior, evidence-report image branch classification, and check output classification. | Adds no migration, deploy, remote migration, Stripe call, real provider call in tests, credit mutation beyond existing charged branch behavior, credit clawback, public billing change, Admin Text/Embeddings/Music/Compare/Live-Agent behavior change, Admin async video job behavior change, sync video debug behavior change, OpenClaw/News Pulse change, member route change, runtime env kill-switch enforcement, live platform caps, or production readiness proof. | `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/admin-ai-image-credit-pricing.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `workers/auth/src/app/route-policy.js`, `config/ai-cost-policy-baseline.json`, `scripts/check-ai-cost-policy.mjs`, AI cost policy tests, `tests/workers.spec.js` |
| Phase 4.15 Runtime budget kill-switch enforcement | Completed for already budget-classified admin/platform paths only | Adds a shared default-disabled runtime switch helper and enforces route/domain flags before provider/queue/credit/durable-attempt work for charged Admin Image models, FLUX.2 Dev explicit-unmetered image tests, admin async video jobs, News Pulse visuals, admin text, admin embeddings, Admin Music, Admin Compare, and Admin Live-Agent. | Focused Worker and script tests cover safe boolean parsing, sanitized disabled responses/log fields, disabled admin routes before internal AI/provider/queue/credit/attempt work, News Pulse skipped visuals with public reads unaffected, evidence switch state, check-policy runtime switch assertions, and enabled compatibility through existing tests. | Adds no migration, deploy, remote migration, Stripe call, real provider call in tests, credit mutation, credit clawback, public billing change, member image/music/video behavior change, org-scoped member route change, new provider route migration, live platform budget caps, or production readiness proof. | `workers/auth/src/lib/admin-platform-budget-switches.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/ai-video-jobs.js`, `workers/auth/src/lib/news-pulse-visuals.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/app/route-policy.js`, AI cost policy tests, `tests/workers.spec.js` |
| Phase 4.15.1 Admin AI Budget Switch Control Plane | Completed for app-level switch control scope only | Adds additive migration `0052`, D1 app-level switch state/history, admin-only list/update APIs, and an Admin Control Plane AI Budget Switches panel. Effective provider-cost execution requires Cloudflare master flag enabled and D1 app switch enabled. Missing app rows, disabled app rows, unavailable D1, or disabled/missing Cloudflare master flags fail closed before provider/internal AI/queue/credit/durable-attempt work. | Focused Worker/static tests cover switch allowlist, master/app/effective state, D1 unavailable fail-closed behavior, admin-only APIs, required `Idempotency-Key` and bounded reason on updates, route-policy coverage, Admin UI copy/update behavior, and Phase 4.16 cap-status preservation. | Does not deploy, apply remote migration, call Cloudflare APIs, store Cloudflare API tokens, mutate Cloudflare variables, call Stripe, call real providers in tests, enable live billing, migrate new provider-cost routes, change member/org routes, add live platform budget caps, or prove production readiness. | `workers/auth/migrations/0052_add_admin_runtime_budget_switches.sql`, `workers/auth/src/lib/admin-platform-budget-switches.js`, `workers/auth/src/routes/admin-ai.js`, `admin/index.html`, `js/pages/admin/control-plane.js`, `js/shared/auth-api.js`, `config/release-compat.json`, Worker/static tests |
| Phase 4.16 Live platform budget caps design/evidence | Completed for design/evidence/check scope only | Defines live platform/admin budget cap semantics, scope taxonomy, candidate usage-ledger/window schema, cap-exceeded behavior, operator override behavior, and recommends `platform_admin_lab_budget` as the Phase 4.17 first cap target. Adds registry cap-readiness metadata and read-only evidence fields showing caps are `not_implemented`. | Deterministic script tests cover cap metadata, check output, evidence report `liveBudgetCapsStatus: not_implemented`, switch-enforced versus cap-enforced distinction, member route separation, and no provider calls. | Adds no migration, deploy, remote migration, Stripe call, real provider call, runtime cap enforcement, route behavior change, credit mutation, credit clawback, public billing change, member/org route behavior change, new provider route migration, or production readiness proof. | `docs/ai-cost-gateway/LIVE_PLATFORM_BUDGET_CAPS_DESIGN.md`, `workers/auth/src/lib/ai-cost-operations.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `scripts/check-ai-cost-policy.mjs`, AI cost policy/evidence tests |
| Phase 4.17 First platform_admin_lab_budget cap foundation | Completed for narrow runtime scope | Adds additive migration `0053`, D1-backed daily/monthly cap limits, bounded usage events, admin-only cap APIs, and an Admin Control Plane cap panel for selected `platform_admin_lab_budget` routes. Covered routes require Cloudflare master switch, D1 app switch, then cap allowance before provider/internal AI/queue/durable-attempt work. Successful supported completions record bounded usage evidence after provider-cost success. | Focused Worker/static/script tests cover cap helper validation, cap-exceeded blocking before provider/queue work, D1 unavailable fail-closed behavior, successful Admin Text usage recording, idempotent replay no-double-counting, async video cap checks, admin cap APIs, cap UI behavior, evidence report coverage, and no provider calls. | Adds no deploy, remote migration, Stripe call, real provider call in tests, customer billing, member/org billing change, new route migration, OpenClaw/News Pulse cap enforcement, org-credit image cap enforcement, credit clawback, or production readiness proof. Other budget scopes remain future work; later Phase 4.18/4.19 add reconciliation and explicit repair actions for this scope. | `workers/auth/migrations/0053_add_platform_budget_caps.sql`, `workers/auth/src/lib/platform-budget-caps.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/ai-video-jobs.js`, `admin/index.html`, `js/pages/admin/control-plane.js`, `js/shared/auth-api.js`, cap/evidence/policy tests |
| Phase 4.18 Platform budget usage reconciliation | Completed for read-only evidence scope | Adds bounded read-only reconciliation for `platform_admin_lab_budget` usage events against admin AI attempts and admin async video jobs. It reports missing usage, duplicates, orphan events, failed-source usage, window mismatches, invalid units, not-checkable limitations, and dry-run repair candidates. | Focused Worker/static/script tests cover sanitized reports, deterministic candidates, read-only endpoint behavior, Budget Reconciliation UI, evidence report integration, and no provider/Stripe/credit/source mutation. | Adds no migration, deploy, remote migration, Stripe call, real provider call, automatic repair, usage/source mutation, member/org billing change, credit mutation, runtime provider route behavior change, or production readiness proof. | `workers/auth/src/lib/platform-budget-reconciliation.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `admin/index.html`, `js/pages/admin/control-plane.js`, `js/shared/auth-api.js`, reconciliation/evidence/static tests |
| Phase 4.19 Platform budget usage repair executor | Completed for explicit admin-approved safe repairs | Adds additive migration `0054`, `platform_budget_repair_actions`, a bounded repair helper, admin-only repair/list/detail APIs, and Admin Control Plane repair controls. `create_missing_usage_event` can insert exactly one missing usage event only after recomputing and verifying a still-successful local D1 source. Duplicate/orphan/failed-source/window/cost candidates are review-only action rows. | Focused Worker/static/script tests cover dry-run no-write behavior, idempotent execution, same-key conflict, stale-source rejection, review-only actions, non-admin/missing idempotency/missing reason/confirm failures, sanitized action listing/detail, UI dry-run/apply controls, and no provider/Stripe/credit/source mutation. | Adds no deploy, remote migration, provider call, Stripe call, Cloudflare mutation, automatic repair, scheduled repair, deletion/rewrite of usage events, source attempt/job mutation, member/org/customer billing change, credit clawback, or production readiness proof. | `workers/auth/migrations/0054_add_platform_budget_repair_actions.sql`, `workers/auth/src/lib/platform-budget-repair.js`, `workers/auth/src/lib/platform-budget-reconciliation.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `admin/index.html`, `js/pages/admin/control-plane.js`, `js/shared/auth-api.js`, repair/evidence/static tests |
| Phase 4.20 Repair evidence export/operator report | Completed for read-only operator evidence | Adds bounded admin-only repair report/export helper, report/export APIs, JSON/Markdown export support, evidence-report integration, and a compact Admin Control Plane report panel for `platform_admin_lab_budget` repair actions. | Focused Worker/static/script tests cover report summary, rollups, export content types, invalid scope rejection, non-admin denial, sanitized related usage evidence, no raw prompt/provider/idempotency/Stripe/Cloudflare exposure, no writes during report/export, and UI export behavior. | Adds no migration, deploy, remote migration, repair execution, automatic repair, provider call, Stripe call, Cloudflare mutation, usage/source/repair row mutation, member/org/customer billing change, credit mutation, credit clawback, or production readiness proof. | `workers/auth/src/lib/platform-budget-repair-report.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/app/route-policy.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `admin/index.html`, `js/pages/admin/control-plane.js`, `js/shared/auth-api.js`, repair-report/static tests |
| Phase 4.21 Evidence archive/retention workflow | Completed for sanitized operator evidence archives | Adds additive migration `0055`, `platform_budget_evidence_archives`, bounded admin-only archive create/list/detail/download/expire/cleanup APIs, JSON/Markdown archive support, evidence-report integration, and a compact Admin Control Plane Evidence Archives panel. Archives are generated from Phase 4.20 sanitized report snapshots and stored in private `AUDIT_ARCHIVE` under `platform-budget-evidence/`. | Focused Worker/static/script tests cover archive creation with reason/idempotency, approved-prefix R2 writes, sanitized JSON/Markdown content, list/detail/download, expire/cleanup behavior, unsafe-key refusal, no raw prompt/provider/idempotency/Stripe/Cloudflare/private-key exposure, and no repair/provider/Stripe/credit/source mutation. | Adds no deploy, remote migration, repair execution, automatic repair, provider call, Stripe call, Cloudflare mutation, usage/source/repair row mutation, member/org/customer billing change, credit mutation, credit clawback, unrelated R2 deletion, or production readiness proof. | `workers/auth/migrations/0055_add_platform_budget_evidence_archives.sql`, `workers/auth/src/lib/platform-budget-evidence-archive.js`, `workers/auth/src/routes/admin-ai.js`, `workers/auth/src/app/route-policy.js`, `workers/auth/src/lib/admin-platform-budget-evidence.js`, `admin/index.html`, `js/pages/admin/control-plane.js`, `js/shared/auth-api.js`, archive/static/evidence tests |
...
| Pricing / Credit Purchase | Completed for current scope | Admin-only `/pricing.html`, admin-only desktop/mobile Pricing header link, Phase 2-O runtime UI alignment with Free/5,000/12,000 live credit-pack tiers, and later BITBI Pro copy/scaffolding where documented in current source files. Paid Pricing CTAs route to gated account billing surfaces; direct checkout creation remains backend-gated. | Original validation is recorded in `PHASE_PRICING_PAGE_CREDIT_PACKS_REPORT.md`; Phase 2-O validation is recorded in `PHASE2O_PRICING_HERO_LIVE_PACKS_AND_PROFILE_NAV_REPORT.md`. Later subscription scaffolding does not by itself prove live billing readiness. | Staging/canary pricing link/access-gate verification, Credits/BITBI Pro checkout behavior, model status truthfulness, lifecycle failure/refund/dispute/reconciliation handling, and no unintended public billing side effects. | `PHASE_PRICING_PAGE_CREDIT_PACKS_REPORT.md`, `PHASE2O_PRICING_HERO_LIVE_PACKS_AND_PROFILE_NAV_REPORT.md` |
| Stripe Testmode config diagnostics | Completed for current scope | `workers/auth/src/lib/stripe-billing.js` now fails closed with safe `config_names` / `missing_config_names` response metadata for missing Stripe Testmode checkout/webhook configuration without printing values. Tests cover missing `STRIPE_MODE`, `STRIPE_SECRET_KEY`, `STRIPE_CHECKOUT_SUCCESS_URL`, live-mode rejection, and missing `STRIPE_WEBHOOK_SECRET`. | `npm run test:workers`, `npm run test:release-compat`, `npm run test:cloudflare-prereqs`, `npm run validate:cloudflare-prereqs`, `npm run release:preflight`, and `git diff --check` passed in the corrective pass. | Operators must still configure `STRIPE_MODE=test`, `STRIPE_SECRET_KEY`, `STRIPE_CHECKOUT_SUCCESS_URL`, `STRIPE_CHECKOUT_CANCEL_URL`, and the webhook-only `STRIPE_WEBHOOK_SECRET` in staging before checkout/webhook verification. | `workers/auth/CLAUDE.md` |

Current status estimate:

- The repository has completed the Phase 0/1 hardening roadmap through Phase 1-J, Phase 2-A organization/RBAC foundation, Phase 2-B billing/entitlement foundation, targeted Phase 2-C org-scoped AI image usage enforcement, Phase 2-D org-scoped image usage retry/reservation hardening, Phase 2-E usage-attempt cleanup/admin inspection, Phase 2-F temporary replay object cleanup, Phase 2-G inspection/no-op, Phase 2-H backend-only org-scoped member text generation, Phase 2-I provider-neutral billing event ingestion, Phase 2-J Stripe Testmode credit-pack checkout foundation, Phase 2-K admin-only Stripe Testmode checkout lockdown, Phase 2-L narrow live credit-pack checkout/Credits dashboard, Phase 2-M admin-only BFL image-test credit charging/live pack economics, Phase 2-O Pricing UI cleanup/live-pack alignment, Phase 2.1 Stripe live billing lifecycle operator-review foundations, Phase 2.2 billing review queue/resolution foundations, Phase 2.3 Admin Control Plane billing review UI, Phase 2.4 read-only local D1 billing reconciliation reporting, Phase 2.5 staging evidence planning, Phase 2.6 main-only release gate/checklist evidence planning, Phase 3.1 AI Cost Gateway design/inventory/report-only checking, Phase 3.2 AI Cost Gateway contract/helpers/tests, Phase 3.3 AI Cost Operation Registry/report-only baseline, Phase 3.4 member personal image gateway pilot, Phase 3.4.1 main-only release/evidence guidance, Phase 3.5 member music cost decomposition/gateway prep, Phase 3.6 member music gateway migration, Phase 3.7 member image/music replay-result/finalization/cleanup hardening, Phase 3.8 member video gateway migration, Phase 3.9 AI Cost Gateway enforcement guard/known-gap baseline, Phase 4.1 admin/platform AI budget policy design, Phase 4.2 admin/platform budget policy helper contract/tests, Phase 4.3 charged Admin BFL image-test budget hardening, Phase 4.4 read-only Admin/Platform AI budget evidence reporting, Phase 4.5 Admin async video job budget enforcement, Phase 4.6 OpenClaw/News Pulse visual budget controls, Phase 4.7 internal AI Worker caller-policy guard, Phase 4.8 Admin text/embeddings budget enforcement, Phase 4.8.1 Admin text/embeddings durable idempotency foundation, Phase 4.8.2 Admin text/embeddings usage-attempt cleanup/inspection, Phase 4.9 Admin Music budget enforcement, Phase 4.10 Admin Compare budget enforcement, Phase 4.11 Admin Live-Agent budget flow audit/design prep, Phase 4.12 Admin Live-Agent budget enforcement, Phase 4.13 Sync Video Debug retirement decision, Phase 4.14 Admin Image branch handling, Phase 4.15 runtime budget kill-switch enforcement, Phase 4.15.1 D1/Admin UI budget switch control, Phase 4.16 live platform budget caps design/evidence and Phase 4.17 platform_admin_lab_budget cap foundation, and Stripe Testmode config-diagnostics hardening for the documented scope.
- It is substantially safer and more operable than the original audit baseline, but it is not full enterprise SaaS maturity. Phase 2-N specifically reduces organization-context ambiguity for solo platform admins by making the selected credit-owning organization visible and shared across Organization, Credits, and Admin AI Lab.
- Existing assets remain user-owned; full tenant isolation, production-ready subscription lifecycle, invoices, customer portal, Stripe Tax, production-trusted broad webhooks, refund/chargeback automation, approved billing remediation workflow, runtime AI Cost Gateway coverage for every provider-cost route, permanent provider-result cache, full IaC/dashboard drift enforcement, full type/lint migration, full user self-service privacy flows, irreversible deletion execution, formal load budgets, and live SLO/alert evidence remain open. Phase 2.3 review queue UI/resolution metadata and Phase 2.4 reconciliation reporting are not automated remediation and do not call Stripe or claw back credits. Phase 2.6 and Phase 3.4.1 are evidence/runbook tooling only; Phase 3.4 changes only member personal image generation, Phase 3.6 changes only member music generation, Phase 3.7 hardens those two migrated member flows, Phase 3.8 changes only member video generation, Phase 3.9 adds guardrails/baseline only, Phase 4.1 adds design/check metadata only, Phase 4.2 adds helper/contracts/tests, Phase 4.3 hardens only the charged Admin BFL image-test branch, Phase 4.4 only reports read-only evidence, Phase 4.5 changes only admin async video jobs, Phase 4.6 changes only OpenClaw/News Pulse visuals, Phase 4.7 changes only internal caller-policy validation/metadata handling, Phase 4.8 changes only admin text/embeddings metadata-only budget enforcement, Phase 4.8.1 changes only admin text/embeddings durable metadata-only idempotency, Phase 4.8.2 changes only admin text/embeddings attempt cleanup/inspection, Phase 4.9 changes only Admin Music metadata-only budget/idempotency enforcement, Phase 4.10 changes only Admin Compare metadata-only budget/idempotency enforcement, Phase 4.11 changes no runtime route behavior because it is Admin Live-Agent audit/design prep only, Phase 4.12 changes only Admin Live-Agent metadata-only budget/idempotency enforcement, Phase 4.13 retires only sync video debug as disabled-by-default/emergency-only, Phase 4.14 changes only Admin Image branch classification/blocked unsupported behavior, Phase 4.15 changes only runtime switch enforcement for already-classified admin/platform paths, Phase 4.15.1 changes only app-level D1/Admin UI switch control on top of Cloudflare master flags, and Phase 4.16 is cap design/evidence only and Phase 4.17 is the first narrow platform_admin_lab_budget cap foundation. None deploys, enables live billing, calls real providers in tests, calls Stripe, changes member image/music/video billing behavior, migrates platform-background outside News Pulse visuals, mutates Cloudflare variables, or proves production readiness.
- The next implementation phase should be chosen between operator-run main-only evidence collection, billing lifecycle/reconciliation hardening, next-scope platform budget cap work, remaining internal caller-policy work, and domain-by-domain tenant ownership migration. Do not redo Phase 0/1/2-A/2-B/2-C/2-D/2-E/2-F/2-G/2-H/2-I/2-J or Admin Control Plane foundations.
...
- `SAAS_PROGRESS_AND_CURRENT_STATE_REPORT.md` contains the current end-to-end SaaS progress/current-state reconciliation, phase timeline, maturity assessment, production blockers, staging checklist, and next recommended phases.
- `CURRENT_IMPLEMENTATION_HANDOFF.md` contains the concise restart handoff for future Codex sessions after the latest committed work.
...
- `PHASE2K_ADMIN_STRIPE_TESTMODE_LOCKDOWN_REPORT.md` contains the Phase 2-K server-side platform-admin-only checkout lockdown, explicit Testmode checkout kill switch, admin-created checkout webhook grant requirement, validation evidence, and operator-only canary checklist.
- `PHASE2L_LIVE_STRIPE_CREDIT_PACKS_AND_CREDITS_DASHBOARD_REPORT.md` contains the Phase 2-L live one-time credit-pack checkout, gated Credits dashboard, migration `0040`, live webhook verification, exactly-once live credit grant behavior, validation evidence, and operator-only production canary checklist.
- `PHASE2M_ADMIN_BFL_IMAGE_TEST_CREDIT_PRICING_REPORT.md` contains the Phase 2-M live pack economics update, Black Forest Labs admin image-test pricing formulas, platform-admin-only charging boundary, organization credit deduction behavior, validation evidence, and remaining non-goals.
- `PHASE2N_ORGANIZATION_CONTEXT_AND_CREDIT_DEBIT_VISIBILITY_REPORT.md` contains the Phase 2-N Organization page, shared active organization selection/defaulting, platform-admin/owner access boundary, Admin AI Lab selected-organization integration, safe debit diagnostics, validation evidence, and remaining non-goals.
...
- `PHASE_PRICING_PAGE_CREDIT_PACKS_REPORT.md` contains the original admin-only Pricing page implementation evidence; `PHASE2O_PRICING_HERO_LIVE_PACKS_AND_PROFILE_NAV_REPORT.md` contains the current Pricing hero/live-pack/profile-nav cleanup.
...
| Synchronous AI video provider polling | Retired from normal path | The default admin UI and async queue path no longer call the old long synchronous provider route. Phase 1-C default-disables `/api/admin/ai/test-video` unless `ALLOW_SYNC_VIDEO_DEBUG=true`, and Phase 4.13 classifies the route as retired/disabled-by-default with the flag retained only for explicitly approved emergency compatibility. |
...
| Billing/plans/entitlements foundation | Reduced | `0035_add_billing_entitlements.sql`, `0036_add_ai_usage_attempts.sql`, `0037_add_billing_event_ingestion.sql`, `0038_add_stripe_credit_pack_checkout.sql`, `0039_raise_credit_balance_cap_for_pricing_packs.sql`, `0040_add_live_stripe_credit_pack_scope.sql`, `0041_add_member_credit_ledger.sql`, `0042_add_member_live_stripe_checkout.sql`, `0047_add_member_subscriptions_and_credit_buckets.sql`, `0048_add_member_ai_usage_attempts.sql`, `0049_add_admin_video_job_budget_metadata.sql`, `0050_add_news_pulse_visual_budget_metadata.sql`, `0051_add_admin_ai_usage_attempts.sql`, `0052_add_admin_runtime_budget_switches.sql`, `0053_add_platform_budget_caps.sql`, `0054_add_platform_budget_repair_actions.sql`, `0055_add_platform_budget_evidence_archives.sql`, `workers/auth/src/lib/billing.js`, `workers/auth/src/lib/member-credit-buckets.js`, `workers/auth/src/lib/ai-usage-policy.js`, `workers/auth/src/lib/ai-usage-attempts.js`, `workers/auth/src/lib/member-ai-usage-attempts.js`, `workers/auth/src/lib/admin-ai-idempotency.js`, `workers/auth/src/lib/admin-platform-budget-switches.js`, `workers/auth/src/lib/platform-budget-caps.js`, `workers/auth/src/lib/platform-budget-reconciliation.js`, `workers/auth/src/lib/platform-budget-repair.js`, `workers/auth/src/lib/platform-budget-repair-report.js`, `workers/auth/src/lib/platform-budget-evidence-archive.js`, `workers/auth/src/lib/billing-events.js`, `workers/auth/src/lib/stripe-billing.js`, `workers/auth/src/routes/orgs.js`, `workers/auth/src/routes/admin-billing.js`, `workers/auth/src/routes/billing-webhooks.js`, `workers/auth/src/routes/ai/text-generate.js`, route-policy/release updates, and Worker tests add default free plan resolution, org billing/entitlement reads, admin plan/org billing inspection, member credit ledger/buckets, idempotent credit grants, idempotent credit consumption helpers, org-scoped image and text generation credit enforcement, member personal image/member music/member video gateway reservation/replay/debit enforcement, member image/music replay-unavailable/finalization/cleanup hardening, admin async video job budget metadata/enforcement, News Pulse visual budget metadata/status controls, admin text/embeddings metadata-only idempotency attempts, app-level Admin AI budget switch state, first `platform_admin_lab_budget` cap limits/usage events, read-only platform budget reconciliation, explicit admin-approved platform budget repair action audit, read-only repair report/export evidence, sanitized platform budget evidence archives, credit reservations, duplicate provider-call suppression, temporary/durable result replay metadata, bounded cleanup, provider-neutral billing event ingestion, Stripe Testmode credit-pack checkout, server-side platform-admin-only Testmode checkout lockdown, verified Testmode checkout webhook credit grants only for admin-created persisted sessions, gated live one-time Stripe credit-pack checkout for platform admins/org owners, verified live webhook credit grants only for authorized persisted sessions, member checkout/session/subscription scaffolding, Phase 2.1 review-only live failure/refund/dispute/expired-checkout billing action metadata, Phase 2.2 admin-only billing review queue/detail/resolution metadata, Phase 2.3 Admin Control Plane review queue UI, Phase 2.4 read-only local D1 billing reconciliation report, admin-only Pricing page checkout initiation, gated Credits dashboard, and sanitized admin billing/usage inspection. Invoices, customer portal, taxes, public production billing evidence, refund/dispute/chargeback automation, approved billing remediation workflow beyond Phase 4.19's narrow usage-evidence repair, full platform budget cap coverage, and broad admin/platform/internal route-level AI Cost Gateway enforcement remain open. |

Findings still open:

- Synchronous AI video provider polling is retired from normal operations by Phase 4.13; keep `ALLOW_SYNC_VIDEO_DEBUG` absent/false except during an explicitly approved emergency compatibility window.
...
- Historical audit/activity backfill, non-video queue schemas/DLQ, full tenant isolation, live payment-provider activation, permanent provider-result caching beyond current member replay metadata, admin/platform/internal AI cost enforcement, user self-service privacy flows, irreversible deletion execution, full observability/SLOs, and load/performance budgets remain open or deferred.

Current merge/deploy status:

| Area | Status | Notes |
| --- | --- | --- |
| Merge readiness | Ready for review after Phase 4.21 validation | The current Phase 3.4 runtime/migration foundation through Phase 4.21 evidence archive checks, Phase 4.20 repair evidence report/export checks, Phase 4.19 repair executor checks, Phase 4.18 reconciliation checks, Phase 4.17 cap foundation checks, Phase 4.16 cap design/evidence checks, and Phase 4.15.1 D1/Admin UI app-switch control tests must pass the requested local validation suite; no commit was made. |
| Production deploy readiness | Blocked | Do not deploy until all required Worker secrets/bindings are live-verified, auth migrations `0028`-`0055` are applied, `SERVICE_AUTH_REPLAY`, `bitbi-ai-video-jobs`, `USER_IMAGES`, `AUDIT_ARCHIVE`, and `AI_LAB` are verified, Stripe Testmode config/secrets and webhook endpoint are configured, all prior migration-dependent behavior is verified, Phase 4.15.1 Admin AI budget switch list/update/effective-state behavior is verified after migration `0052`, Phase 4.17 platform budget cap list/update/usage behavior is verified after migration `0053`, Phase 4.19 platform budget repair action behavior is verified after migration `0054`, Phase 4.20 repair report/export behavior is verified as read-only/sanitized, Phase 4.21 archive create/download/expire/cleanup behavior is verified after migration `0055`, Phase 2-M/4.3 Admin AI Lab BFL credit charging/no-charge/idempotency/budget-metadata behavior is verified, and the Admin Control Plane UI is verified against real admin APIs. |
| Current recommended next phase | Verification evidence, next platform budget scope, billing operations, or tenant track | Because the owner deploys directly from `main`, collect main-only evidence for migration `0055`, member personal image/music/video gateway behavior, Phase 4.4/4.8.2/4.10/4.12/4.13/4.14/4.15/4.15.1/4.16/4.17/4.18/4.19/4.20/4.21 budget evidence output, Phase 4.15.1 Admin AI budget switch intended-state/effective-state evidence, Phase 4.17 cap-foundation evidence, Phase 4.19 repair-action evidence, Phase 4.20 repair report/export evidence, and Phase 4.21 archive/retention evidence before making live claims. Any bounded operator canary still needs immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: next-scope platform budget cap work, archive retention operations, one remaining internal caller-policy hardening gap, turn Phase 2.3 review queue UI/resolution metadata plus Phase 2.4 read-only reconciliation signals into an approved remediation workflow, or migrate one tenant ownership domain. Do not combine all tracks into one broad rewrite. |
