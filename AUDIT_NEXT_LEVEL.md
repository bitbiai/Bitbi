...
| Area | Status | Notes |
| --- | --- | --- |
| Merge readiness | Ready for review after Phase 4.3 validation | The current Phase 3.4 runtime/migration foundation, Phase 3.4.1 evidence guidance, Phase 3.5 decomposition, Phase 3.6 member music gateway migration, Phase 3.7 member image/music hardening, Phase 3.8 member video gateway tests, Phase 3.9 AI cost baseline/guard tests, Phase 4.1 budget policy design/check tests, Phase 4.2 pure budget helper/check tests, and Phase 4.3 charged Admin BFL image-test budget hardening tests must pass the requested local validation suite; no commit was made. |
| Production deploy readiness | Blocked | Do not deploy until all required Worker secrets/bindings are live-verified, auth migrations `0028`-`0048` are applied, `SERVICE_AUTH_REPLAY`, `bitbi-ai-video-jobs`, `USER_IMAGES`, `AUDIT_ARCHIVE`, and `AI_LAB` are verified, Stripe Testmode config/secrets and webhook endpoint are configured, `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true` is used only for a bounded admin-only canary window, live Stripe config is configured with `ENABLE_LIVE_STRIPE_CREDIT_PACKS=false` by default, live webhook endpoint is configured, optional BITBI Pro subscription config is verified only in an approved environment, `VIDU_API_KEY` is provisioned if Vidu async jobs are enabled, `ALLOW_SYNC_VIDEO_DEBUG` is absent/false unless explicitly approved, live health/header checks run with `--require-live`, dashboard-managed WAF/header/RUM/alert controls are verified, Phase 1-G activity projection writes/search are verified, Phase 1-H/1-I/1-J data lifecycle planning/archive generation/archive cleanup/safe executor behavior is verified, Phase 2-A organization flows are verified after migration `0034`, Phase 2-B billing/entitlement/credit flows are verified after migration `0035`, Phase 2-D/E/F/H AI usage/replay/cleanup behavior is verified after migration `0036`, Phase 2-I billing event ingestion is verified after migration `0037`, Phase 2-J/2-K Stripe Testmode platform-admin-only checkout/webhook/exactly-once-credit-grant and non-admin-created no-credit behavior is verified after migration `0038`, Pricing 5000/10000-credit grant compatibility is verified after migration `0039`, Phase 2-L live checkout/webhook/Credits dashboard behavior is verified after migration `0040`, member credit ledger/live checkout behavior is verified after migrations `0041` and `0042`, News Pulse/OpenClaw/storage quota behavior is verified after migrations `0043`-`0046`, member subscription and credit bucket behavior is verified after migration `0047`, member personal image/member music/member video gateway behavior is verified after migration `0048`, Phase 2-M/4.3 Admin AI Lab BFL credit charging/no-charge/idempotency/budget-metadata behavior is verified, and the Admin Control Plane UI is verified against real admin APIs. |
| Current recommended next phase | Verification evidence, Phase 4.4 admin async video budget enforcement, billing operations, or tenant track | Because the owner deploys directly from `main`, collect main-only evidence for migration `0048`, member personal image gateway behavior, member music gateway behavior, Phase 3.7 replay/finalization/cleanup hardening, member video gateway behavior, Phase 3.9 `check:ai-cost-policy` baseline output, Phase 4.1/4.2 admin/platform budget policy design/helper output, and Phase 4.3 charged Admin BFL image-test budget metadata before making live claims. Any bounded operator canary still needs immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: Phase 4.4 admin async video job budget enforcement, add a report-only budget evidence collector, turn Phase 2.3 review queue UI/resolution metadata plus Phase 2.4 read-only reconciliation signals into an approved remediation workflow, or migrate one tenant ownership domain. Do not combine all tracks into one broad rewrite. |
