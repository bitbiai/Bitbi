...
- Latest implemented product/security phase: Phase 3.3 AI Cost Operation Registry/report-only baseline on top of Phase 3.2 AI Cost Gateway contract/helpers/tests, Phase 3.1 design/inventory/report-only policy-gap checking, Phase 2.6 main-only direct-release runbook/checklist and local readiness gate, Phase 2.4 read-only admin billing reconciliation report, Phase 2.3 billing review UI, Phase 2.2 review APIs, Phase 2-O Pricing hero/live-pack cleanup, member credit buckets, BITBI Pro subscription scaffolding, and asset storage quota tracking.
...
| Phase 3.1 AI Cost Gateway design and route inventory | Completed for current scope | Adds AI Cost Gateway documentation, provider-cost route inventory, target lifecycle design, phased roadmap, and report-only `check:ai-cost-policy` tooling. | Local validation covers documentation currentness and the report-only policy check/test. | Does not change runtime AI charging behavior, call providers, mutate billing, require idempotency, reserve credits, suppress provider calls, add replay/cache, deploy, or prove production readiness. Phase 3.2 adds only the unused contract/test foundation and Phase 3.3 adds only registry/report-only metadata; route migration remains a separate Phase 3.4 decision. | `docs/ai-cost-gateway/README.md`, `docs/ai-cost-gateway/AI_COST_ROUTE_INVENTORY.md`, `docs/ai-cost-gateway/AI_COST_GATEWAY_DESIGN.md`, `docs/ai-cost-gateway/AI_COST_GATEWAY_ROADMAP.md` |
| Phase 3.2 AI Cost Gateway contract and tests | Completed for current scope | Adds unused gateway constants, config normalization, request fingerprinting, scoped idempotency key helpers, pure plan classification, and deterministic tests. | `npm run test:ai-cost-gateway` covers valid/invalid configs, fingerprint determinism, volatile-field exclusion, prompt hashing, scoped idempotency, plan states, and no external calls. | Does not import the gateway into live routes, change Idempotency-Key requirements, reserve/debit credits, add provider replay/cache, call providers, mutate billing, deploy, or prove production readiness. | `workers/auth/src/lib/ai-cost-gateway.js`, `scripts/test-ai-cost-gateway.mjs` |
| Phase 3.3 AI Cost Operation Registry | Completed for current scope | Adds a central unused operation registry for 30 known provider-cost/current-policy AI operations, current enforcement metadata, route-policy/source-file baselines, registry summary helper, and report-only check/test coverage. | `npm run test:ai-cost-operations` validates unique operation ids, normalizer compatibility, high-risk operation coverage, deterministic summary counts, duplicate rejection, and no external calls. `npm run check:ai-cost-policy` now loads the registry and remains report-only by default. | Does not import the registry into live routes, migrate routes, change Idempotency-Key requirements, reserve/debit credits, add provider replay/cache, call providers, mutate billing, deploy, or prove production readiness. | `workers/auth/src/lib/ai-cost-operations.js`, `scripts/test-ai-cost-operations.mjs`, `scripts/check-ai-cost-policy.mjs` |
...
| Billing/plans/entitlements foundation | Reduced | `0035_add_billing_entitlements.sql`, `0036_add_ai_usage_attempts.sql`, `0037_add_billing_event_ingestion.sql`, `0038_add_stripe_credit_pack_checkout.sql`, `0039_raise_credit_balance_cap_for_pricing_packs.sql`, `0040_add_live_stripe_credit_pack_scope.sql`, `0041_add_member_credit_ledger.sql`, `0042_add_member_live_stripe_checkout.sql`, `0047_add_member_subscriptions_and_credit_buckets.sql`, `workers/auth/src/lib/billing.js`, `workers/auth/src/lib/member-credit-buckets.js`, `workers/auth/src/lib/ai-usage-policy.js`, `workers/auth/src/lib/ai-usage-attempts.js`, `workers/auth/src/lib/billing-events.js`, `workers/auth/src/lib/stripe-billing.js`, `workers/auth/src/routes/orgs.js`, `workers/auth/src/routes/admin-billing.js`, `workers/auth/src/routes/billing-webhooks.js`, `workers/auth/src/routes/ai/text-generate.js`, route-policy/release updates, and Worker tests add default free plan resolution, org billing/entitlement reads, admin plan/org billing inspection, member credit ledger/buckets, idempotent credit grants, idempotent credit consumption helpers, org-scoped image and text generation credit enforcement, credit reservations, duplicate provider-call suppression, temporary result replay, bounded cleanup, provider-neutral billing event ingestion, Stripe Testmode credit-pack checkout, server-side platform-admin-only Testmode checkout lockdown, verified Testmode checkout webhook credit grants only for admin-created persisted sessions, gated live one-time Stripe credit-pack checkout for platform admins/org owners, verified live webhook credit grants only for authorized persisted sessions, member checkout/session/subscription scaffolding, Phase 2.1 review-only live failure/refund/dispute/expired-checkout billing action metadata, Phase 2.2 admin-only billing review queue/detail/resolution metadata, Phase 2.3 Admin Control Plane review queue UI, Phase 2.4 read-only local D1 billing reconciliation report, admin-only Pricing page checkout initiation, gated Credits dashboard, and sanitized admin billing/usage inspection. Invoices, customer portal, taxes, public production billing evidence, refund/dispute/chargeback automation, approved billing remediation workflow, and broad/video route-level credit enforcement remain open. |
