...
- Latest implemented product/security phase: Phase 2-O Pricing hero/live-pack cleanup, member credit buckets, BITBI Pro subscription scaffolding, and asset storage quota tracking on top of Phase 2-N Organization page/context selection and Phase 2-M admin-only BFL image-test credit charging.
...
- Existing assets remain user-owned; full tenant isolation, production-ready subscription lifecycle, invoices, customer portal, Stripe Tax, production-trusted broad webhooks, refunds/chargebacks, billing reconciliation/admin needs-review workflow, broad AI route credit enforcement beyond explicitly wired paths, permanent provider-result cache, full IaC/dashboard drift enforcement, full type/lint migration, full user self-service privacy flows, irreversible deletion execution, formal load budgets, and live SLO/alert evidence remain open.
...
| Current recommended next phase | Verification evidence, billing operations, video AI route, or tenant track | Prefer staging verification and deployment evidence first. If the operator intentionally skips staging, perform only bounded operator canaries with immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: refund/chargeback/manual-review billing policy, video route entitlement wiring, or domain-by-domain tenant ownership migration. Do not combine all tracks into one broad rewrite. |
