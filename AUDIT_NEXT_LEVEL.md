...
- Latest auth D1 migration: `0052_add_admin_runtime_budget_switches.sql`
...
| Area | Status | Notes |
| --- | --- | --- |
| Merge readiness | Ready for review after Phase 4.15.1 validation | The current Phase 3.4 runtime/migration foundation through Phase 4.16 live cap design/evidence checks plus Phase 4.15.1 D1/Admin UI app-switch control tests must pass the requested local validation suite; no commit was made. |
| Production deploy readiness | Blocked | Do not deploy until all required Worker secrets/bindings are live-verified, auth migrations `0028`-`0052` are applied, `SERVICE_AUTH_REPLAY`, `bitbi-ai-video-jobs`, `USER_IMAGES`, `AUDIT_ARCHIVE`, and `AI_LAB` are verified, Stripe Testmode config/secrets and webhook endpoint are configured, all prior migration-dependent behavior is verified, Phase 4.15.1 Admin AI budget switch list/update/effective-state behavior is verified after migration `0052`, Phase 2-M/4.3 Admin AI Lab BFL credit charging/no-charge/idempotency/budget-metadata behavior is verified, and the Admin Control Plane UI is verified against real admin APIs. |
| Current recommended next phase | Verification evidence, Phase 4.17 live cap foundation, billing operations, or tenant track | Because the owner deploys directly from `main`, collect main-only evidence for migration `0052`, member personal image/music/video gateway behavior, Phase 4.4/4.8.2/4.10/4.12/4.13/4.14/4.15/4.15.1/4.16 budget evidence output, Phase 4.15.1 Admin AI budget switch intended-state/effective-state evidence, and Phase 4.16 cap-not-implemented evidence before making live claims. Any bounded operator canary still needs immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: Phase 4.17 `platform_admin_lab_budget` live cap foundation, one remaining internal caller-policy hardening gap, turn Phase 2.3 review queue UI/resolution metadata plus Phase 2.4 read-only reconciliation signals into an approved remediation workflow, or migrate one tenant ownership domain. Do not combine all tracks into one broad rewrite. |
