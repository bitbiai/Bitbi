...
- Latest implemented product/security phase: Phase 3.5 member music cost decomposition/gateway prep, on top of Phase 3.4.1 main-only deploy/evidence checklist guidance for the Phase 3.4 member personal image AI Cost Gateway pilot, Phase 3.3 AI Cost Operation Registry/report-only baseline, Phase 3.2 AI Cost Gateway contract/helpers/tests, Phase 3.1 design/inventory/report-only policy-gap checking, Phase 2.6 main-only direct-release runbook/checklist and local readiness gate, Phase 2.4 read-only admin billing reconciliation report, Phase 2.3 billing review UI, Phase 2.2 review APIs, Phase 2-O Pricing hero/live-pack cleanup, member credit buckets, BITBI Pro subscription scaffolding, and asset storage quota tracking.
...
| Phase 3.5 Member music cost decomposition | Completed for design/check scope | Decomposes member music into parent request, optional lyrics, required audio, and background cover provider-cost operations; adds `MEMBER_MUSIC_COST_DECOMPOSITION.md`; updates the registry/check output/tests for music sub-operation gaps. | Deterministic AI cost operation/policy tests cover the music parent/lyrics/audio/cover entries, bundled metadata, report-only unmigrated status, and no external calls. | Does not migrate `/api/ai/generate-music`, require music `Idempotency-Key`, reserve music credits, change debits, add replay/cache, call providers, mutate billing, deploy, or prove production readiness. | `docs/ai-cost-gateway/MEMBER_MUSIC_COST_DECOMPOSITION.md`, `workers/auth/src/lib/ai-cost-operations.js`, `scripts/check-ai-cost-policy.mjs`, `scripts/test-ai-cost-policy.mjs`, `scripts/test-ai-cost-operations.mjs` |
