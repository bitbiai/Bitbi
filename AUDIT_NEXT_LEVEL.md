...
## Current Remediation Status After Phase 4.8 Admin Text/Embeddings Budget Enforcement, Phase 4.7 Internal AI Worker Caller-Policy Guard, Phase 4.6 OpenClaw/News Pulse Visual Budget Controls, Phase 4.5 Admin Async Video Job Budget Enforcement, Phase 4.4 Admin/Platform Budget Evidence, Phase 4.3 Charged Admin BFL Image-Test Budget Hardening, Phase 4.2 Admin/Platform AI Budget Policy Contract, Phase 4.1 Admin/Platform AI Budget Policy Design, Phase 3.9 AI Cost Gateway Guard, Phase 3.8 Member Video Gateway Migration, Phase 3.7 Member Image/Music Gateway Hardening, Phase 3.6 Member Music Gateway Migration, Phase 3.4 Member Image Gateway Pilot, Phase 2.6 Main-Only Release Gate, Phase 2.4 Billing Reconciliation, BITBI Pro, Admin Control Plane, Pricing Rollout, And Stripe Testmode Diagnostics

This section is the current implementation checkpoint as of 2026-05-16. The Admin Control Plane received a corrective polish/data-accuracy pass that fixed new card/grid/nav/mobile rendering defects, tightened status wording and sanitization, and added layout/data-accuracy regression tests. The Pricing / Credit Purchase rollout adds a controlled admin-only pricing surface. Phase 2-K makes Testmode checkout creation server-side platform-admin-only, requires `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true`, and blocks verified Testmode webhook credit grants unless the persisted checkout session was created by an active platform admin. Phase 2-L adds a separate gated live one-time credit-pack flow and Credits dashboard for platform admins and active organization owners only. Phase 2-M updates the live pack economics and charges selected Black Forest Labs Admin AI image tests against existing organization credits for platform admins only. Phase 2-N adds the Organization page, shared active organization selection/defaulting, owner/platform-admin role visibility, and safe Admin AI Lab credit-debit diagnostics so operators can confirm which organization owns credits and receives admin image-test debits. Phase 2-O updates the admin-only Pricing runtime UI to use the current live 5,000/12,000 credit-pack presentation, removes visible stale Testmode wording, and routes paid CTAs to the gated Credits dashboard. Later BITBI Pro work adds member subscription scaffolding, member credit buckets, member storage-quota support, and related release-config coverage through auth migration `0047_add_member_subscriptions_and_credit_buckets.sql`. Phase 2.1/2.2 added operator-review-only live Stripe lifecycle classification and admin review APIs; Phase 2.3 exposes that queue in the Admin Control Plane UI with safe details and note/confirmation-gated resolve/dismiss actions; Phase 2.4 adds a read-only local D1 billing reconciliation report for admins/operators; Phase 2.6 adds main-only release runbook/checklist evidence and a local readiness gate for the owner’s direct-`main` workflow. Phase 3.1 adds AI Cost Gateway design, provider-cost route inventory, a phased roadmap, and local policy-gap checking. Phase 3.2 adds the AI Cost Gateway contract/helpers/tests. Phase 3.3 adds an AI Cost Operation Registry and baseline tests for known provider-cost/current-policy operations. Phase 3.4 migrates only member personal image generation to that gateway pilot with mandatory idempotency, pre-provider member-credit reservation, duplicate provider-call suppression, safe replay metadata, exactly-once success debit, and no debit on provider failure. Phase 3.4.1 adds only main-only release/evidence guidance for applying/verifying auth migration `0048` before auth Worker deployment and recording member image smoke evidence. Phase 3.5 decomposes member music into parent, lyrics, audio, and cover provider-cost operations. Phase 3.6 migrates only member music generation to the gateway with mandatory idempotency, one parent member-credit reservation, duplicate provider-call suppression, safe persisted-asset replay metadata, exactly-once debit after audio persistence, no debit on lyrics/audio provider failure, terminal no-paid-replay storage/billing failure behavior, and bundled cover policy. Phase 3.7 hardens only the already migrated member image/music flows: replay-unavailable same-key attempts do not re-run providers or double debit, replay responses omit raw prompt/lyrics, music cover status is written back safely, finalization failures are explicit terminal states, and scheduled cleanup releases expired member attempts and deletes only approved linked temp replay objects. Phase 3.8 migrates only member video generation to the gateway with mandatory idempotency, one parent member-credit reservation before PixVerse/HappyHorse provider work and output ingest, duplicate provider-call suppression, safe durable saved-asset replay metadata, exactly-once debit after durable video persistence, and safe no-charge/replay-unavailable/finalization behavior. Phase 3.9 adds `config/ai-cost-policy-baseline.json`, default local enforcement against unbaselined provider-cost drift, member image/music/video regression detection, and release-preflight integration for `npm run check:ai-cost-policy`. Phase 4.1 adds admin/platform AI budget policy design only: budget scopes, registry budget metadata, known-gap baseline budget scopes, and report/test output grouping remaining admin/platform/internal/OpenClaw gaps. Phase 4.2 adds the pure admin/platform budget policy helper contract, deterministic tests, kill-switch/audit/fingerprint/plan validation, and stricter baseline checks for kill-switch targets or exemptions plus future enforcement paths. Phase 4.3 imports that helper only into the existing charged Admin BFL image-test branch, records safe `admin_org_credit_account` plan/audit metadata in admin responses, usage events, and usage attempts, and preserves selected-organization credit debits plus no-charge-on-provider-failure behavior. Phase 4.4 adds read-only Admin/Platform AI budget evidence reporting. Phase 4.5 imports the Phase 4.2 helper only into admin async video jobs, adds additive migration `0049_add_admin_video_job_budget_metadata.sql`, stores sanitized job-level `platform_admin_lab_budget` metadata, queues a bounded budget summary, verifies job budget state before internal video task create/poll, and suppresses duplicate provider task creation after a create attempt. Phase 4.6 imports the Phase 4.2 helper only into OpenClaw/News Pulse visual generation, adds additive migration `0050_add_news_pulse_visual_budget_metadata.sql`, stores sanitized `openclaw_news_pulse_budget` visual metadata, blocks invalid policy before provider execution, and preserves ready/pending/status/attempt duplicate suppression. Phase 4.7 adds a signed reserved JSON body caller-policy contract, validates caller-policy metadata in the AI Worker after service-auth, strips it before provider handlers, and propagates safe metadata for covered Auth Worker callers where feasible. Phase 4.8 covers only admin text/embeddings with required `Idempotency-Key`, safe `platform_admin_lab_budget` metadata, future kill-switch metadata, and signed `budget_metadata_only` caller-policy propagation. Phase 2.6 through Phase 4.8 do not deploy, run remote migrations, enable live billing, call real AI providers in tests, call Stripe, migrate Admin music/compare/live-agent, migrate Admin video beyond Phase 4.5, migrate OpenClaw/News Pulse beyond Phase 4.6, migrate platform/background outside News Pulse visuals, hard-fail all internal AI Worker routes, enforce a new runtime env kill switch, change member image/music/video billing behavior, change org-scoped member route behavior, or prove production readiness. This does not prove live billing readiness, full subscription lifecycle maturity, automated refund/dispute remediation, automatic credit clawback, approved reconciliation/remediation, full runtime AI cost readiness, or full SaaS maturity. The original audit findings, risk ratings, score estimates, and command output below remain a historical pre-remediation baseline from 2026-04-24. They are intentionally preserved and must not be read as the current state after Phase 0-A through the current Phase 4.8 work.
...
| Area | Status | Notes |
| --- | --- | --- |
| Merge readiness | Ready for review after Phase 4.8 validation | The current Phase 3.4 runtime/migration foundation, Phase 3.4.1 evidence guidance, Phase 3.5 decomposition, Phase 3.6 member music gateway migration, Phase 3.7 member image/music hardening, Phase 3.8 member video gateway tests, Phase 3.9 AI cost baseline/guard tests, Phase 4.1 budget policy design/check tests, Phase 4.2 pure budget helper/check tests, Phase 4.3 charged Admin BFL image-test budget hardening tests, Phase 4.4 read-only budget evidence tests, Phase 4.5 admin async video job budget tests, Phase 4.6 News Pulse visual budget tests, Phase 4.7 internal caller-policy guard tests, and Phase 4.8 admin text/embeddings budget tests must pass the requested local validation suite; no commit was made. |
| Production deploy readiness | Blocked | Do not deploy until all required Worker secrets/bindings are live-verified, auth migrations `0028`-`0050` are applied, `SERVICE_AUTH_REPLAY`, `bitbi-ai-video-jobs`, `USER_IMAGES`, `AUDIT_ARCHIVE`, and `AI_LAB` are verified, Stripe Testmode config/secrets and webhook endpoint are configured, `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true` is used only for a bounded admin-only canary window, live Stripe config is configured with `ENABLE_LIVE_STRIPE_CREDIT_PACKS=false` by default, live webhook endpoint is configured, optional BITBI Pro subscription config is verified only in an approved environment, `VIDU_API_KEY` is provisioned if Vidu async jobs are enabled, `ALLOW_SYNC_VIDEO_DEBUG` is absent/false unless explicitly approved, live health/header checks run with `--require-live`, dashboard-managed WAF/header/RUM/alert controls are verified, Phase 1-G activity projection writes/search are verified, Phase 1-H/1-I/1-J data lifecycle planning/archive generation/archive cleanup/safe executor behavior is verified, Phase 2-A organization flows are verified after migration `0034`, Phase 2-B billing/entitlement/credit flows are verified after migration `0035`, Phase 2-D/E/F/H AI usage/replay/cleanup behavior is verified after migration `0036`, Phase 2-I billing event ingestion is verified after migration `0037`, Phase 2-J/2-K Stripe Testmode platform-admin-only checkout/webhook/exactly-once-credit-grant and non-admin-created no-credit behavior is verified after migration `0038`, Pricing 5000/10000-credit grant compatibility is verified after migration `0039`, Phase 2-L live checkout/webhook/Credits dashboard behavior is verified after migration `0040`, member credit ledger/live checkout behavior is verified after migrations `0041` and `0042`, News Pulse/OpenClaw/storage quota behavior is verified after migrations `0043`-`0046`, member subscription and credit bucket behavior is verified after migration `0047`, member personal image/member music/member video gateway behavior is verified after migration `0048`, Phase 4.5 admin async video job budget metadata/queue behavior is verified after migration `0049`, Phase 4.6 News Pulse visual budget metadata/status behavior is verified after migration `0050`, Phase 2-M/4.3 Admin AI Lab BFL credit charging/no-charge/idempotency/budget-metadata behavior is verified, and the Admin Control Plane UI is verified against real admin APIs. |
| Current recommended next phase | Verification evidence, Phase 4.9 targeted remaining Admin AI/internal caller migration, billing operations, or tenant track | Because the owner deploys directly from `main`, collect main-only evidence for migration `0050`, member personal image gateway behavior, member music gateway behavior, Phase 3.7 replay/finalization/cleanup hardening, member video gateway behavior, Phase 3.9 `check:ai-cost-policy` baseline output, Phase 4.1/4.2 admin/platform budget policy design/helper output, Phase 4.3 charged Admin BFL image-test budget metadata, Phase 4.4/4.8 budget evidence output, Phase 4.5 admin video budget metadata/queue behavior, Phase 4.6 News Pulse visual budget metadata/status behavior, Phase 4.7 internal caller-policy guard behavior, and Phase 4.8 admin text/embeddings metadata behavior before making live claims. Any bounded operator canary still needs immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: Phase 4.9 targeted remaining Admin AI/internal caller migration, turn Phase 2.3 review queue UI/resolution metadata plus Phase 2.4 read-only reconciliation signals into an approved remediation workflow, or migrate one tenant ownership domain. Do not combine all tracks into one broad rewrite. |
