...
## Current Remediation Status After Phase 4.17 Platform Budget Cap Foundation

This section is the current implementation checkpoint as of 2026-05-16. Phase 4.8.1 adds additive migration `0051_add_admin_ai_usage_attempts.sql` and narrow `admin_ai_usage_attempts` metadata-only idempotency rows for Admin Text/Embeddings. Phase 4.8.2 adds cleanup/inspection for those rows. Phase 4.9, Phase 4.10, and Phase 4.12 extend the same metadata-only attempt foundation to Admin Music, Admin Compare, and Admin Live-Agent respectively. Phase 4.13 retires sync video debug from normal provider-cost operations as disabled-by-default/emergency-only. Phase 4.14 classifies Admin Image branches. Phase 4.15 enforces runtime Cloudflare master budget switches for already budget-classified admin/platform provider-cost paths. Phase 4.15.1 adds additive migration `0052_add_admin_runtime_budget_switches.sql`, app-level D1 switch state/history, admin-only switch list/update APIs, and an Admin Control Plane panel; effective provider-cost execution now requires both the Cloudflare master flag and the D1 app switch. Missing D1 rows, disabled app switches, unavailable D1, or disabled/missing Cloudflare flags fail closed before provider/internal AI/queue/credit/durable-attempt work. The Admin UI does not mutate Cloudflare variables, does not call Cloudflare APIs, and does not store Cloudflare API tokens. Phase 4.16 remains completed as design/evidence/checking for live platform budget caps. Phase 4.17 adds additive migration `0053_add_platform_budget_caps.sql`, daily/monthly caps, bounded usage events, admin-only cap APIs, and an Admin Control Plane cap panel only for selected `platform_admin_lab_budget` routes. Other budget scopes remain future work. These rows and responses do not store or return raw prompts, raw lyrics, raw embedding input, raw Live-Agent messages/output, generated text, embedding vectors, audio, provider request bodies, provider response bodies, cookies, auth headers, Stripe data, Cloudflare tokens, secrets, private keys, private R2 keys, raw idempotency keys, idempotency hashes, or request fingerprints. Phase 2.6 through Phase 4.17 do not deploy, run remote migrations, enable live billing, call real AI providers in tests, call Stripe, migrate new provider-cost routes, change member image/music/video billing behavior, change org-scoped member route behavior, or prove production readiness. This does not prove live billing readiness, full subscription lifecycle maturity, automated refund/dispute remediation, automatic credit clawback, approved reconciliation/remediation, full platform budget cap coverage, full runtime AI cost readiness, or full SaaS maturity. The original audit findings, risk ratings, score estimates, and command output below remain a historical pre-remediation baseline from 2026-04-24.
...
| Billing/plans/entitlements foundation | Reduced | `0035_add_billing_entitlements.sql`, `0036_add_ai_usage_attempts.sql`, `0037_add_billing_event_ingestion.sql`, `0038_add_stripe_credit_pack_checkout.sql`, `0039_raise_credit_balance_cap_for_pricing_packs.sql`, `0040_add_live_stripe_credit_pack_scope.sql`, `0041_add_member_credit_ledger.sql`, `0042_add_member_live_stripe_checkout.sql`, `0047_add_member_subscriptions_and_credit_buckets.sql`, `0048_add_member_ai_usage_attempts.sql`, `0049_add_admin_video_job_budget_metadata.sql`, `0050_add_news_pulse_visual_budget_metadata.sql`, `0051_add_admin_ai_usage_attempts.sql`, `0052_add_admin_runtime_budget_switches.sql`, `0053_add_platform_budget_caps.sql`, `workers/auth/src/lib/billing.js`, `workers/auth/src/lib/member-credit-buckets.js`, `workers/auth/src/lib/ai-usage-policy.js`, `workers/auth/src/lib/ai-usage-attempts.js`, `workers/auth/src/lib/member-ai-usage-attempts.js`, `workers/auth/src/lib/admin-ai-idempotency.js`, `workers/auth/src/lib/admin-platform-budget-switches.js`, `workers/auth/src/lib/platform-budget-caps.js`, `workers/auth/src/lib/billing-events.js`, `workers/auth/src/lib/stripe-billing.js`, `workers/auth/src/routes/orgs.js`, `workers/auth/src/routes/admin-billing.js`, `workers/auth/src/routes/billing-webhooks.js`, `workers/auth/src/routes/ai/text-generate.js`, route-policy/release updates, and Worker tests add default free plan resolution, org billing/entitlement reads, admin plan/org billing inspection, member credit ledger/buckets, idempotent credit grants, idempotent credit consumption helpers, org-scoped image and text generation credit enforcement, member personal image/member music/member video gateway reservation/replay/debit enforcement, member image/music replay-unavailable/finalization/cleanup hardening, admin async video job budget metadata/enforcement, News Pulse visual budget metadata/status controls, admin text/embeddings metadata-only idempotency attempts, app-level Admin AI budget switch state, first `platform_admin_lab_budget` cap limits/usage events, credit reservations, duplicate provider-call suppression, temporary/durable result replay metadata, bounded cleanup, provider-neutral billing event ingestion, Stripe Testmode credit-pack checkout, server-side platform-admin-only Testmode checkout lockdown, verified Testmode checkout webhook credit grants only for admin-created persisted sessions, gated live one-time Stripe credit-pack checkout for platform admins/org owners, verified live webhook credit grants only for authorized persisted sessions, member checkout/session/subscription scaffolding, Phase 2.1 review-only live failure/refund/dispute/expired-checkout billing action metadata, Phase 2.2 admin-only billing review queue/detail/resolution metadata, Phase 2.3 Admin Control Plane review queue UI, Phase 2.4 read-only local D1 billing reconciliation report, admin-only Pricing page checkout initiation, gated Credits dashboard, and sanitized admin billing/usage inspection. Invoices, customer portal, taxes, public production billing evidence, refund/dispute/chargeback automation, approved billing remediation workflow, full platform budget cap coverage, and broad admin/platform/internal route-level AI Cost Gateway enforcement remain open. |
...
| Area | Status | Notes |
| --- | --- | --- |
| Merge readiness | Ready for review after Phase 4.17 validation | The current Phase 3.4 runtime/migration foundation through Phase 4.17 cap foundation checks, Phase 4.16 cap design/evidence checks, and Phase 4.15.1 D1/Admin UI app-switch control tests must pass the requested local validation suite; no commit was made. |
| Production deploy readiness | Blocked | Do not deploy until all required Worker secrets/bindings are live-verified, auth migrations `0028`-`0053` are applied, `SERVICE_AUTH_REPLAY`, `bitbi-ai-video-jobs`, `USER_IMAGES`, `AUDIT_ARCHIVE`, and `AI_LAB` are verified, Stripe Testmode config/secrets and webhook endpoint are configured, all prior migration-dependent behavior is verified, Phase 4.15.1 Admin AI budget switch list/update/effective-state behavior is verified after migration `0052`, Phase 4.17 platform budget cap list/update/usage behavior is verified after migration `0053`, Phase 2-M/4.3 Admin AI Lab BFL credit charging/no-charge/idempotency/budget-metadata behavior is verified, and the Admin Control Plane UI is verified against real admin APIs. |
| Current recommended next phase | Verification evidence, Phase 4.17 platform_admin_lab_budget cap foundation, billing operations, or tenant track | Because the owner deploys directly from `main`, collect main-only evidence for migration `0053`, member personal image/music/video gateway behavior, Phase 4.4/4.8.2/4.10/4.12/4.13/4.14/4.15/4.15.1/4.16 budget evidence output, Phase 4.15.1 Admin AI budget switch intended-state/effective-state evidence, and Phase 4.17 cap-foundation evidence before making live claims. Any bounded operator canary still needs immediate rollback by disabling `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS`. After that, choose one narrow track: Phase 4.17 follow-up cap hardening/next-scope work, one remaining internal caller-policy hardening gap, turn Phase 2.3 review queue UI/resolution metadata plus Phase 2.4 read-only reconciliation signals into an approved remediation workflow, or migrate one tenant ownership domain. Do not combine all tracks into one broad rewrite. |
