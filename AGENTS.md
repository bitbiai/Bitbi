...
- For Q4 Pages, run full cumulative candidate validation once, then use the existing workflow’s explicit source run/attempt publication path. Preserve exact tested artifact identity, required job evidence, final dependency guard and shared write lock; never dispatch a duplicate full run merely to publish.
