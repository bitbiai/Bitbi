...
| Merge readiness | Ready only after current Phase 4.6 validation passes; no commit made by Codex |
...
- A single runtime AI Cost Gateway for all expensive member/admin/platform AI routes with required idempotency, pre-provider reservations, provider-result replay/cache, and release-on-failure semantics. Phase 3.4 covers member personal image generation, Phase 3.6 covers member music generation, Phase 3.7 hardens those two migrated flows, and Phase 3.8 covers member video generation. Phase 4.3 hardens only the charged Admin BFL image-test branch with budget-policy metadata; Phase 4.4 reports evidence only; Phase 4.5 covers only admin async video jobs; Phase 4.6 covers only OpenClaw/News Pulse visuals. Broad admin AI, Admin music/text/compare/live-agent, platform/background AI outside News Pulse visuals, and global internal AI Worker provider routes remain unmigrated.
...
Recommended next phase: collect main-only verification evidence for migrations through `0050`, the Phase 3.4 member image gateway behavior, the Phase 3.6 member music gateway behavior, the Phase 3.7 replay/finalization/cleanup hardening, the Phase 3.8 member video gateway behavior, the Phase 3.9 AI cost policy baseline/preflight guard, the Phase 4.1/4.2 budget policy design/helper/check output, the Phase 4.3 charged Admin BFL image-test budget metadata, the Phase 4.4 budget evidence report, the Phase 4.5 admin video job budget metadata/queue behavior, and the Phase 4.6 News Pulse visual budget metadata/status behavior before any live claim. If staging is intentionally skipped, use a bounded operator-run production canary with `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true` for Testmode and `ENABLE_LIVE_STRIPE_CREDIT_PACKS=true` only for the shortest possible live credit-pack canary window. Include Phase 2-N organization selection, Phase 2-M/4.3 admin-only BFL image-test charging and budget metadata, member image gateway behavior, member music gateway behavior, member video gateway behavior, admin async video budget metadata/queue behavior, News Pulse visual budget metadata/status behavior, `report:ai-budget-evidence`, and `check:ai-cost-policy` output in that evidence set. This is not production-readiness approval.
