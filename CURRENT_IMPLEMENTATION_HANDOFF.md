...
| Latest implemented work | Phase 3.5 member music cost decomposition and gateway prep, Phase 3.4.1 main-only deploy/evidence checklist guidance for the Phase 3.4 member personal image AI Cost Gateway pilot, Phase 3.3 AI Cost Operation Registry/report-only baseline, Phase 3.2 AI Cost Gateway contract/helpers/tests, Phase 3.1 route inventory/roadmap/report-only local policy-gap check, Phase 2.6 main-only direct-release runbook/checklist and local readiness gate, Phase 2.4 read-only admin billing reconciliation report, Phase 2.3 Admin Control Plane UI for the Stripe live billing lifecycle review queue, prior Phase 2.2 review APIs, BITBI Pro member subscription/credit bucket foundation, and Alpha Audit documentation reconciliation |
...
| Merge readiness | Ready only after current Phase 3.5 validation passes; no commit made by Codex |
...
- Phase 3.5 decomposes member music generation for future gateway migration only. It adds `docs/ai-cost-gateway/MEMBER_MUSIC_COST_DECOMPOSITION.md`, refines registry entries for `member.music.generate`, `member.music.lyrics.generate`, `member.music.audio.generate`, and `member.music.cover.generate`, and strengthens report-only AI cost policy tests/check output. It does not change `/api/ai/generate-music`, require `Idempotency-Key`, reserve credits, change debits, add replay/cache, call providers, mutate billing, or prove production readiness.
...
- A single runtime AI Cost Gateway for all expensive member/admin/platform AI routes with required idempotency, pre-provider reservations, provider-result replay/cache, and release-on-failure semantics. Phase 3.4 covers only member personal image generation; Phase 3.5 only decomposes/prepares music. Member music/video, admin AI, platform/background AI, and internal AI Worker provider routes remain unmigrated.
...
Recommended next phase: collect Phase 3.4.1 main-only verification evidence for the Phase 3.4 member image pilot, then Phase 3.6 AI Cost Gateway migration for member music generation if the owner approves proceeding. If staging is intentionally skipped, use a bounded operator-run production canary with `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true` for Testmode and `ENABLE_LIVE_STRIPE_CREDIT_PACKS=true` only for the shortest possible live credit-pack canary window. Include Phase 2-N organization selection, Phase 2-M admin-only BFL image-test charging, and Phase 3.4 member image gateway behavior in that evidence set. This is not production-readiness approval.
...
9. Run `npm run check:ai-cost-policy` when changing AI route policy metadata or provider-calling routes; it is report-only and now validates the Phase 3.3 operation registry plus current Phase 3.4 pilot coverage/gaps.
10. Choose one next implementation track: Phase 3.6 member music AI Cost Gateway migration, refund/chargeback/manual-remediation policy, video AI entitlement wiring, or tenant-owned asset migration.
