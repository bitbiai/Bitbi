...
| Merge readiness | Ready only after current Phase 3.8 validation passes; no commit made by Codex |
...
- Phase 3.3 adds an AI Cost Operation Registry in `workers/auth/src/lib/ai-cost-operations.js` plus `test:ai-cost-operations`. It records normalized target operation configs, current enforcement metadata, route-policy baselines, and source-file inventory for known AI cost operations. `check:ai-cost-policy` validates that registry and remains report-only by default.
...
Recommended next phase: collect main-only verification evidence for migrations through `0048`, the Phase 3.4 member image gateway behavior, the Phase 3.6 member music gateway behavior, the Phase 3.7 replay/finalization/cleanup hardening, and the Phase 3.8 member video gateway behavior before any live claim. If staging is intentionally skipped, use a bounded operator-run production canary with `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true` for Testmode and `ENABLE_LIVE_STRIPE_CREDIT_PACKS=true` only for the shortest possible live credit-pack canary window. Include Phase 2-N organization selection, Phase 2-M admin-only BFL image-test charging, member image gateway behavior, member music gateway behavior, and member video gateway behavior in that evidence set. This is not production-readiness approval.
...
9. Run `npm run check:ai-cost-policy` when changing AI route policy metadata or provider-calling routes; it is report-only and now validates the Phase 3.3 operation registry plus current member image/music/video gateway coverage and remaining gaps.
10. Choose one next implementation track: admin/platform AI cost telemetry and route-metadata guard, refund/chargeback/manual-remediation policy, video AI entitlement wiring for org/admin boundaries, or tenant-owned asset migration.
