...
| A-005 | P1 | AI cost control | Phase 3.1 documents provider-cost routes in `docs/ai-cost-gateway/AI_COST_ROUTE_INVENTORY.md`; Phase 3.2 adds contract helpers in `workers/auth/src/lib/ai-cost-gateway.js`; Phase 3.3 adds `workers/auth/src/lib/ai-cost-operations.js` as a registry for 30 known AI cost operations; Phase 3.4 imports those helpers only for member personal image generation and adds additive migration `0048_add_member_ai_usage_attempts.sql`. Music, video, admin, platform/background, internal AI Worker, and org-scoped route migrations remain out of scope. | Provider calls and credit debits can still diverge across non-pilot flows, increasing cost and abuse risk. | Continue migrating one route at a time through the shared AI Cost Gateway after the image pilot evidence is reviewed. | L | High | Route-level provider-call suppression, reservation release, replay, and cost telemetry tests. |
| A-006 | P1 | AI idempotency/replay | Phase 3.4 requires `Idempotency-Key` for member personal `/api/ai/generate-image`, records member-credit reservations in `member_ai_usage_attempts`, suppresses same-key duplicate provider execution, replays safe stored temporary image metadata where available, and releases/no-charges on provider failure. Member music/video, generated music covers, News Pulse visuals, most unmetered admin routes, and internal AI Worker provider routes are still not uniformly covered by required idempotency/reservation/replay. | Expensive retries can duplicate provider spend or charge users inconsistently outside the pilot route. | Extend idempotency/reservation/finalization through the gateway one route at a time before broad paid rollout. | L | High | Provider failure/retry/replay tests per route. |
