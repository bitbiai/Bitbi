...
Scope: Alpha Audit hardening status. Phase 0 reconciled audit/documentation currentness. Phase 2.1 added Stripe live billing lifecycle operator-review ingestion for failed-payment, refund, dispute, and expired-checkout events; Phase 2.2 added an admin-only billing review queue/detail/resolution API for those events; Phase 2.3 exposes that API in the Admin Control Plane UI; Phase 2.4 adds a read-only local D1 billing reconciliation report for admins/operators; Phase 2.6 adds main-only direct-release runbook/checklist evidence guidance and a local non-mutating main-release readiness gate; Phase 3.1 adds AI Cost Gateway design, provider-cost route inventory, roadmap, and a report-only local policy-gap check; Phase 3.2 adds an unused AI Cost Gateway contract/helper module and deterministic tests; Phase 3.3 adds an unused AI Cost Operation Registry and report-only baseline. This report is based on repository files and local release metadata. It does not approve production deploy, remote migrations, Cloudflare changes, Stripe changes, AI provider calls, live billing readiness, full SaaS maturity, full tenant isolation, or legal compliance.
...
| A-005 | P1 | AI cost control | Phase 3.1 documents provider-cost routes in `docs/ai-cost-gateway/AI_COST_ROUTE_INVENTORY.md`; Phase 3.2 adds the unused contract helpers in `workers/auth/src/lib/ai-cost-gateway.js`; Phase 3.3 adds `workers/auth/src/lib/ai-cost-operations.js` as a report-only registry for 30 known AI cost operations. No live route is migrated yet, so there is still no runtime AI Cost Gateway across image/text/video/music/admin/platform flows. | Provider calls and credit debits can diverge across flows, increasing cost and abuse risk. | Migrate one route at a time through the shared AI Cost Gateway after the registry baseline. | L | High | Route-level provider-call suppression, reservation release, replay, and cost telemetry tests. |
| A-006 | P1 | AI idempotency/replay | Phase 3.3 registry confirms org-scoped image/text and charged admin image paths have stronger reservation/replay behavior, while member image personal mode, member music/video, generated music covers, News Pulse visuals, most unmetered admin routes, and internal AI Worker provider routes are not uniformly covered by required idempotency/reservation/replay. Phase 3.2/3.3 tests helper and registry logic but does not change any existing route requirement. | Expensive retries can duplicate provider spend or charge users inconsistently. | Extend idempotency/reservation/finalization through the gateway one route at a time before broad paid rollout. | L | High | Provider failure/retry/replay tests per route. |
...
| Phase 4: full AI cost gateway and LLM efficiency | Phase 3.1 completed design/inventory/report-only drift checking. Phase 3.2 completed the unused gateway contract/helpers/tests. Phase 3.3 completed the unused operation registry/report-only baseline. Next implementation should migrate exactly one low-risk route, preferably member personal image generation, without broad rewrites. | Cover image/text/video/music/admin/platform paths without broad rewrites. |
