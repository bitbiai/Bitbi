...
The weakest current area is evidence integrity. Several current-state docs previously lagged the release contract and implied older auth migrations (`0040`, `0046`, or `0048`) were latest/current. The current release contract now expects `0049`; stale migration docs create a direct production-readiness risk because operators could apply the wrong migration boundary before deploying code that expects current tables.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A-001 | P0 | Documentation integrity / release safety | Current release config declares `0049_add_admin_video_job_budget_metadata.sql`; before the Phase 0 reconciliation, current-state docs still referenced `0040`/`0046` as current/latest, and current docs must now track `0049` after Phase 4.5. Files: `README.md`, `CURRENT_IMPLEMENTATION_HANDOFF.md`, `SAAS_PROGRESS_AND_CURRENT_STATE_REPORT.md`, `AUDIT_ACTION_PLAN.md`, `AUDIT_NEXT_LEVEL.md`, `DATA_INVENTORY.md`, `docs/DATA_RETENTION_POLICY.md`, `docs/privacy-data-flow-audit.md`, `workers/auth/CLAUDE.md`. | Operators can deploy code that depends on missing tables or falsely claim readiness from stale docs. | Keep current docs reconciled to `0049`; preserve historical phase reports; keep `check:doc-currentness`. | S | Low | `npm run check:doc-currentness`, `npm run test:doc-currentness`, `npm run release:preflight`. |
...
2. Reconcile current-state docs to latest auth D1 migration `0049_add_admin_video_job_budget_metadata.sql`.
3. Preserve historical phase reports exactly as historical evidence.
4. Run the doc-currentness guard in CI/preflight.
5. Before any live claim, verify migrations through `0049`, live Cloudflare bindings/secrets/resources, Stripe Testmode/live webhooks, BITBI Pro subscription behavior, member personal image/music/video gateway behavior, admin async video job budget metadata/queue behavior, failure/refund/dispute/chargeback operator-review UI and resolution evidence, Phase 2.4 read-only reconciliation report evidence, main-only release evidence if deploying directly from `main`, approved remediation workflow, and rollback.
...
| Phase 4: full AI cost gateway and LLM efficiency | Phase 3.1 completed design/inventory/report-only drift checking. Phase 3.2 completed the gateway contract/helpers/tests. Phase 3.3 completed the operation registry/report-only baseline. Phase 3.4 completed the member personal image pilot. Phase 3.4.1 adds only main-only deploy/evidence guidance for migration `0048` before auth Worker deployment. Phase 3.5 decomposes member music. Phase 3.6 migrates only member music. Phase 3.7 hardens replay/result metadata, cover status, finalization, and cleanup for member image/music. Phase 3.8 migrates only member video. Phase 3.9 adds the known-gap baseline and default local enforcement guard against unregistered provider-cost drift and member gateway regressions. Phase 4.1 adds admin/platform budget policy design and target budget-scope metadata only. Phase 4.2 adds pure admin/platform budget helper contracts and tests. Phase 4.3 hardens only the charged Admin BFL image-test branch with safe budget-policy metadata while preserving existing org-credit charging. Phase 4.4 adds read-only budget evidence reporting. Phase 4.5 covers only admin async video job budget metadata/enforcement with additive migration `0049` and keeps broad Admin AI, Admin music/text/compare/live-agent, OpenClaw/News Pulse, platform/background, and global internal AI Worker enforcement as baselined gaps. Next implementation should migrate one remaining admin/platform domain, with OpenClaw/News Pulse visuals recommended, while preserving member image/music/video, charged Admin BFL, and admin async video invariants. | Cover admin/platform/internal/OpenClaw/provider-cost paths without broad rewrites. |
