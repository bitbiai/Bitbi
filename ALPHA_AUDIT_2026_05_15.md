...
| A-005 | P1 | AI cost control | Phase 3.1 documents provider-cost routes in `docs/ai-cost-gateway/AI_COST_ROUTE_INVENTORY.md`; Phase 3.2 adds contract helpers in `workers/auth/src/lib/ai-cost-gateway.js`; Phase 3.3 adds `workers/auth/src/lib/ai-cost-operations.js` as a registry now covering 31 known AI cost operations; Phase 3.4 imports those helpers for member personal image generation and adds additive migration `0048_add_member_ai_usage_attempts.sql`; Phase 3.4.1 adds only main-only deploy/evidence guidance and no runtime change; Phase 3.5 decomposes member music; Phase 3.6 migrates member music only. Member video, admin, platform/background, internal AI Worker, OpenClaw/News Pulse, and org-scoped route migrations remain out of scope. | Provider calls and credit debits can still diverge across non-migrated flows, increasing cost and abuse risk. | Continue migrating one route at a time through the shared AI Cost Gateway after evidence is reviewed. | L | High | Route-level provider-call suppression, reservation release, replay, and cost telemetry tests. |
| A-006 | P1 | AI idempotency/replay | Phase 3.4 requires `Idempotency-Key` for member personal `/api/ai/generate-image`, records member-credit reservations in `member_ai_usage_attempts`, suppresses same-key duplicate provider execution, replays safe stored temporary image metadata where available, and releases/no-charges on provider failure. Phase 3.6 now applies the same parent-reservation pattern to member `/api/ai/generate-music`: mandatory `Idempotency-Key`, one parent reservation covering lyrics/audio/cover work, duplicate in-progress/completed suppression, safe asset replay metadata, no debit on lyrics/audio provider failure, terminal no-charge behavior on storage/billing failure, and exactly-once debit after audio persistence. Member video, News Pulse visuals, most unmetered admin routes, platform/background routes, and internal AI Worker provider routes are still not uniformly covered by required idempotency/reservation/replay. | Expensive retries can duplicate provider spend or charge users inconsistently outside the migrated routes. | Extend idempotency/reservation/finalization through the gateway one route at a time before broad paid rollout. | L | High | Provider failure/retry/replay tests per route. |
...
| Phase 4: full AI cost gateway and LLM efficiency | Phase 3.1 completed design/inventory/report-only drift checking. Phase 3.2 completed the gateway contract/helpers/tests. Phase 3.3 completed the operation registry/report-only baseline. Phase 3.4 completed the member personal image pilot. Phase 3.4.1 adds only main-only deploy/evidence guidance for migration `0048` before auth Worker deployment. Phase 3.5 decomposes member music. Phase 3.6 migrates only member music. Next implementation should harden replay/result-cache behavior and then migrate exactly one remaining high-risk route, with member video still the primary member gap. | Cover image/text/video/admin/platform paths without broad rewrites. |
