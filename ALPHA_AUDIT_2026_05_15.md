...
| Phase 4: full AI cost gateway and LLM efficiency | Phase 3.1 completed design/inventory/report-only drift checking. Phase 3.2 completed the gateway contract/helpers/tests. Phase 3.3 completed the operation registry/report-only baseline. Phase 3.4 completed the member personal image pilot. Phase 3.4.1 adds only main-only deploy/evidence guidance for migration `0048` before auth Worker deployment. Phase 3.5 decomposes member music. Phase 3.6 migrates only member music. Phase 3.7 hardens replay/result metadata, cover status, finalization, and cleanup for member image/music. Phase 3.8 migrates only member video. Phase 3.9 adds the known-gap baseline and default local enforcement guard against unregistered provider-cost drift and member gateway regressions. Phase 4.1 adds admin/platform budget policy design and target budget-scope metadata only. Phase 4.2 adds pure admin/platform budget helper contracts and tests. Phase 4.3 hardens only the charged Admin BFL image-test branch with safe budget-policy metadata while preserving existing org-credit charging. Phase 4.4 adds read-only budget evidence reporting. Phase 4.5 covers only admin async video job budget metadata/enforcement with additive migration `0049`. Phase 4.6 covers only OpenClaw/News Pulse visual budget metadata/status controls with additive migration `0050` and keeps broad Admin AI, Admin music/text/compare/live-agent, platform/background outside News Pulse visuals, and global internal AI Worker enforcement as baselined gaps. Next implementation should migrate one remaining admin/platform/internal domain, with internal AI Worker caller-policy guards recommended, while preserving member image/music/video, charged Admin BFL, admin async video, and News Pulse visual invariants. | Cover admin/platform/internal/OpenClaw/provider-cost paths without broad rewrites. |
