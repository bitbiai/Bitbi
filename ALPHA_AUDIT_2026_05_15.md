...
| A-005 | P1 | AI cost control | Phase 3.1 documents provider-cost routes in `docs/ai-cost-gateway/AI_COST_ROUTE_INVENTORY.md`; Phase 3.2 adds contract helpers in `workers/auth/src/lib/ai-cost-gateway.js`; Phase 3.3 adds `workers/auth/src/lib/ai-cost-operations.js` as a registry now covering 31 known AI cost operations; Phase 3.4 imports those helpers only for member personal image generation and adds additive migration `0048_add_member_ai_usage_attempts.sql`; Phase 3.4.1 adds only main-only deploy/evidence guidance and no runtime change; Phase 3.5 decomposes member music into parent, lyrics, audio, and cover operation metadata and report-only checks. Music route behavior, video, admin, platform/background, internal AI Worker, and org-scoped route migrations remain out of scope. | Provider calls and credit debits can still diverge across non-pilot flows, increasing cost and abuse risk. | Continue migrating one route at a time through the shared AI Cost Gateway after the image pilot evidence is reviewed. | L | High | Route-level provider-call suppression, reservation release, replay, and cost telemetry tests. |
| A-006 | P1 | AI idempotency/replay | Phase 3.4 requires `Idempotency-Key` for member personal `/api/ai/generate-image`, records member-credit reservations in `member_ai_usage_attempts`, suppresses same-key duplicate provider execution, replays safe stored temporary image metadata where available, and releases/no-charges on provider failure. Phase 3.5 documents that member music still has optional/recommended idempotency only and decomposes its lyrics/audio/cover replay and partial-success risks; it does not change the route. Member music/video, generated music covers, News Pulse visuals, most unmetered admin routes, and internal AI Worker provider routes are still not uniformly covered by required idempotency/reservation/replay. | Expensive retries can duplicate provider spend or charge users inconsistently outside the pilot route. | Extend idempotency/reservation/finalization through the gateway one route at a time before broad paid rollout. | L | High | Provider failure/retry/replay tests per route. |
...
5. Before any live claim, verify migrations through `0048`, live Cloudflare bindings/secrets/resources, Stripe Testmode/live webhooks, BITBI Pro subscription behavior, member personal image gateway pilot behavior using the Phase 3.4.1 checklist, failure/refund/dispute/chargeback operator-review UI and resolution evidence, Phase 2.4 read-only reconciliation report evidence, main-only release evidence if deploying directly from `main`, approved remediation workflow, and rollback.
...
| Phase 4: full AI cost gateway and LLM efficiency | Phase 3.1 completed design/inventory/report-only drift checking. Phase 3.2 completed the gateway contract/helpers/tests. Phase 3.3 completed the operation registry/report-only baseline. Phase 3.4 completed the member personal image pilot only. Phase 3.4.1 adds only main-only deploy/evidence guidance for migration `0048` before auth Worker deployment. Phase 3.5 decomposes member music and strengthens report-only checks without changing runtime behavior. Next implementation should migrate member music in Phase 3.6 as a narrow PR after evidence is recorded or explicitly waived by the owner. | Cover image/text/video/music/admin/platform paths without broad rewrites. |
