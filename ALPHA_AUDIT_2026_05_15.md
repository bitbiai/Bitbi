...
The weakest current area is evidence integrity. Several current-state docs previously lagged the release contract and implied older auth migrations (`0040` or `0046`) were latest/current. The current release contract now expects `0048`; stale migration docs create a direct production-readiness risk because operators could apply the wrong migration boundary before deploying code that expects current tables.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A-001 | P0 | Documentation integrity / release safety | Current release config declares `0048_add_member_ai_usage_attempts.sql`; before the Phase 0 reconciliation, current-state docs still referenced `0040`/`0046` as current/latest, and current docs must now track `0048` after Phase 3.4. Files: `README.md`, `CURRENT_IMPLEMENTATION_HANDOFF.md`, `SAAS_PROGRESS_AND_CURRENT_STATE_REPORT.md`, `AUDIT_ACTION_PLAN.md`, `AUDIT_NEXT_LEVEL.md`, `DATA_INVENTORY.md`, `docs/DATA_RETENTION_POLICY.md`, `docs/privacy-data-flow-audit.md`, `workers/auth/CLAUDE.md`. | Operators can deploy code that depends on missing tables or falsely claim readiness from stale docs. | Keep current docs reconciled to `0048`; preserve historical phase reports; keep `check:doc-currentness`. | S | Low | `npm run check:doc-currentness`, `npm run test:doc-currentness`, `npm run release:preflight`. |
...
2. Reconcile current-state docs to latest auth D1 migration `0048_add_member_ai_usage_attempts.sql`.
3. Preserve historical phase reports exactly as historical evidence.
4. Run the doc-currentness guard in CI/preflight.
5. Before any live claim, verify migrations through `0048`, live Cloudflare bindings/secrets/resources, Stripe Testmode/live webhooks, BITBI Pro subscription behavior, member personal image and member music gateway behavior, failure/refund/dispute/chargeback operator-review UI and resolution evidence, Phase 2.4 read-only reconciliation report evidence, main-only release evidence if deploying directly from `main`, approved remediation workflow, and rollback.
