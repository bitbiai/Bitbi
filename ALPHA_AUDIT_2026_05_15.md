...
Scope: Alpha Audit hardening status. Phase 0 reconciled audit/documentation currentness. Phase 2.1 added Stripe live billing lifecycle operator-review ingestion for failed-payment, refund, dispute, and expired-checkout events; Phase 2.2 added an admin-only billing review queue/detail/resolution API for those events; Phase 2.3 exposes that API in the Admin Control Plane UI; Phase 2.4 adds a read-only local D1 billing reconciliation report for admins/operators; Phase 2.6 adds main-only direct-release runbook/checklist evidence guidance and a local non-mutating main-release readiness gate; Phase 3.1 adds AI Cost Gateway design, provider-cost route inventory, roadmap, and a report-only local policy-gap check. This report is based on repository files and local release metadata. It does not approve production deploy, remote migrations, Cloudflare changes, Stripe changes, AI provider calls, live billing readiness, full SaaS maturity, full tenant isolation, or legal compliance.
...
| A-005 | P1 | AI cost control | Phase 3.1 documents provider-cost routes in `docs/ai-cost-gateway/AI_COST_ROUTE_INVENTORY.md` and confirms there is still no single AI Cost Gateway across image/text/video/music/admin/platform flows. | Provider calls and credit debits can diverge across flows, increasing cost and abuse risk. | Implement the shared AI Cost Gateway contract in a later phase and keep route-specific adapters thin. | L | High | Route-level provider-call suppression, reservation release, replay, and cost telemetry tests. |
| A-006 | P1 | AI idempotency/replay | Phase 3.1 inventory confirms org-scoped image/text and charged admin image paths have stronger reservation/replay behavior, while member image personal mode, member music/video, generated music covers, News Pulse visuals, most unmetered admin routes, and internal AI Worker provider routes are not uniformly covered by required idempotency/reservation/replay. | Expensive retries can duplicate provider spend or charge users inconsistently. | Extend idempotency/reservation/finalization through the gateway one route at a time before broad paid rollout. | L | High | Provider failure/retry/replay tests per route. |
...
| Phase 4: full AI cost gateway and LLM efficiency | Phase 3.1 completed design/inventory/report-only drift checking. Next implementation should add the gateway contract/tests first, then migrate one low-risk route at a time. | Cover image/text/video/music/admin/platform paths without broad rewrites. |
