...
| Private admin Fable chat | JSON and streaming `POST /api/admin/fable-chat/conversations/:id/messages*`; owner-scoped conversation settings | `platform_admin_lab_budget` | Admin/MFA gate, fixed Fable model, required hashed idempotency, serialized conversations, exact-once durable result replay, native server-loaded roles/private provider-block continuity, server presets/effort limits, existing Admin Text switches, and atomic effort/context-weighted cap admission before provider execution. | Provider availability and live cap/usage evidence remain required; failed or unknown provider attempts retain admitted weighted units, and ambiguous outcomes never auto-retry. |
| Private Fable rolling-memory compaction | Internal `/internal/ai/fable-chat/memory`, scheduled only after durable Fable finalization | `platform_admin_lab_budget` | Fixed Cloudflare-hosted Qwen model and exact token pricing, immutable Standard/Lite input fingerprints, one active compaction per profile, atomic pre-provider cap admission, private versioned checkpoints, and prior-checkpoint fallback. | Qwen failure or an ambiguous outcome never advances memory; the existing raw Fable context remains available. |
...
Current route-policy IDs covered here include `admin.ai.test-text`, `admin.fable-chat.messages.send`, `admin.fable-chat.messages.stream`, `admin.fable-chat.conversations.settings.update`, `admin.ai.test-embeddings`, `admin.ai.test-music`, `admin.ai.compare`, `admin.ai.live-agent`, `admin.ai.video-jobs`, `admin.ai.video-job-read`, `admin.ai.video-job-cancel`, `admin.ai.test-video`, `openclaw.news_pulse.ingest`, and the member AI generation routes.
