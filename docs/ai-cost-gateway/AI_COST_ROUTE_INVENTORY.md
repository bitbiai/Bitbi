...
| Private admin Fable chat | `POST /api/admin/fable-chat/conversations/:id/messages` | `platform_admin_lab_budget` | Admin/MFA gate, fixed Fable model, required hashed idempotency, serialized conversations, durable result replay, native server-loaded roles, existing Admin Text switches, atomic conservative cap admission before provider execution, and content-free audit metadata. | Provider availability and live cap/usage evidence remain required; failed or unknown provider attempts retain the admitted cap unit, and unknown outcomes never auto-retry. |
...
Current route-policy IDs covered here include `admin.ai.test-text`, `admin.fable-chat.messages.send`, `admin.ai.test-embeddings`, `admin.ai.test-music`, `admin.ai.compare`, `admin.ai.live-agent`, `admin.ai.video-jobs`, `admin.ai.video-job-read`, `admin.ai.video-job-cancel`, `admin.ai.test-video`, `openclaw.news_pulse.ingest`, and the member AI generation routes.
