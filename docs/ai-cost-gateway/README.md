...
Status: Phase 5.1 Admin Control Plane UX/navigation consolidation on top of Phase 4.21 sanitized `platform_admin_lab_budget` evidence archive/retention workflow, Phase 4.20 read-only repair evidence report/export, Phase 4.19 explicit admin-approved usage repair execution, Phase 4.18 read-only reconciliation evidence, Phase 4.17's first narrow cap foundation, completed Phase 4.16 design/evidence, and Phase 4.15/4.15.1 switch enforcement. Phase 4.21 adds local migration `0055_add_platform_budget_evidence_archives.sql` and private `AUDIT_ARCHIVE` archive snapshots under `platform-budget-evidence/`. Phase 5.1 adds no migration and only improves operator wayfinding, help text, badges, responsive panel links, and direct hash aliases for existing Admin Control Plane budget panels. Reports/exports/archives are bounded and sanitized and do not apply repairs, mutate usage/source/repair rows, run automatic repairs, call Stripe/providers, deploy, run remote migrations, enable live billing, change member/org billing behavior, change public pricing, mutate credits, or apply credit clawbacks.
...
- Phase 4.16 adds `docs/ai-cost-gateway/LIVE_PLATFORM_BUDGET_CAPS_DESIGN.md`, registry cap-readiness metadata, policy-check output, and read-only evidence fields for cap planning. It remains preserved. Phase 4.17 implements the first `platform_admin_lab_budget` cap foundation with `liveBudgetCapsStatus: platform_admin_lab_budget_foundation`, daily/monthly cap limits, bounded usage events, and admin-only cap APIs/UI for Admin Text, Embeddings, Music, Compare, Live-Agent, and Admin async video jobs. Phase 4.18 adds read-only reconciliation evidence for those same sources, Phase 4.19 adds an explicit admin-approved repair executor for missing usage evidence plus review-only repair action audit rows, Phase 4.20 adds read-only operator repair evidence report/export in JSON and Markdown, and Phase 4.21 adds sanitized operator-approved report archives under `AUDIT_ARCHIVE` with retention metadata and approved-prefix-only cleanup. Other scopes remain future work.
- Phase 5.1 consolidates Admin Control Plane discoverability for these existing controls. The existing grouped navigation and section ids remain, while the AI budget area now has an operator control map, keyboard-accessible help, status badges, responsive links, and direct hash aliases for caps, reconciliation, repair, repair evidence, and evidence archives. It changes no backend route behavior or AI cost enforcement semantics.
...
- `workers/auth/migrations/0055_add_platform_budget_evidence_archives.sql`
...
`npm run test:admin-platform-budget-evidence` validates the read-only evidence helper, local script, report bounds, sanitization, blocked verdict, member gateway coverage, Admin BFL hardening evidence, admin video job coverage, News Pulse coverage, internal caller-policy guard evidence, admin text/embeddings metadata-only coverage, sync video debug retired/disabled-by-default evidence, Admin Image branch classification evidence, Phase 4.15 runtime switch evidence, Phase 4.16 cap-design evidence, Phase 4.17 `platform_admin_lab_budget` cap-foundation evidence, Phase 4.18 reconciliation evidence, Phase 4.19 repair executor evidence, Phase 4.20 repair report/export evidence, Phase 4.21 evidence archive/retention evidence, remaining baseline gaps, and no provider calls.
...
Next implementation phase: collect operator evidence for migration `0055` and the archive create/download/expire/cleanup flow, extend caps only one scope at a time, likely `openclaw_news_pulse_budget`, add admin-approved non-destructive review workflows for remaining repair candidate types, or choose one targeted remaining internal caller-policy gap. Production/live billing remains blocked until operator evidence is complete and reviewed.
