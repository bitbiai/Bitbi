...
Cleanup marks only expired active admin text/embeddings attempts as `expired`; it does not delete completed/failed rows, call providers, mutate credits, mutate billing, delete R2 objects, or alter public/member behavior. Inspection omits raw prompts, raw embedding inputs, generated text, embedding vectors, provider request bodies, raw idempotency keys, cookies, auth headers, Stripe data, Cloudflare tokens, secrets, private keys, and private R2 keys.

## Current Non-Goals

Phase 4.8/4.8.1/4.8.2 add:
