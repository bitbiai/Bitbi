...
- Phase 4.16 adds `docs/ai-cost-gateway/LIVE_PLATFORM_BUDGET_CAPS_DESIGN.md`, registry cap-readiness metadata, policy-check output, and read-only evidence fields showing `liveBudgetCapsStatus: not_implemented`. It distinguishes switch-enforced paths from cap-enforced paths, recommends `platform_admin_lab_budget` as the Phase 4.17 first cap target, and does not enforce runtime daily/monthly caps.
...
Next implementation phase: Phase 4.17 should implement a narrow `platform_admin_lab_budget` live cap foundation for already classified admin lab paths, or defer that explicitly and choose one targeted remaining internal caller-policy gap. Production/live billing remains blocked until operator evidence is complete and reviewed.
