...
Status: Phase 4.8.1 admin text/embeddings durable idempotency foundation on top of the Phase 4.2 contract/helper foundation, Phase 4.3 charged Admin BFL image-test hardening, Phase 4.4 read-only budget evidence reporting, Phase 4.5 admin async video job budget enforcement, Phase 4.6 OpenClaw/News Pulse visual budget controls, Phase 4.7 internal AI Worker caller-policy guard, and Phase 4.8 admin text/embeddings budget metadata. Phase 4.8.1 covers only `POST /api/admin/ai/test-text` and `POST /api/admin/ai/test-embeddings`: both now require `Idempotency-Key`, build safe `platform_admin_lab_budget` metadata, propagate signed `budget_metadata_only` caller-policy metadata to the AI Worker, and use `admin_ai_usage_attempts` for metadata-only duplicate suppression/conflict detection. Admin music/compare/live-agent, sync video debug, unmetered admin image, Admin video beyond Phase 4.5, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI, and broad internal AI Worker routes remain unmigrated or baseline-allowed. No real provider calls, Stripe calls, live billing enablement, public billing changes, credit mutations, deploys, remote migrations, or runtime route migrations outside this scope were performed.
...
- admin text/embeddings as Phase 4.8.1 `platform_admin_lab_budget` metadata-only coverage with required `Idempotency-Key`, durable duplicate suppression/conflict detection, and signed caller-policy propagation
...
Phase 4.8.1 does not change Admin music, Admin video beyond Phase 4.5, Admin compare, Admin live-agent, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI, unrelated internal AI Worker routes, member image/music/video behavior, org-scoped member image/text behavior, public pricing, Stripe behavior, credit debit behavior, credit clawback behavior, or live billing readiness.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Admin text test | `platform_admin_lab_budget` | Phase 4.8.1 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only idempotency attempts; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate suppression through `admin_ai_usage_attempts`. | Runtime env kill switch and live platform budget caps remain future; current enforcement is metadata plan/fingerprint/caller-policy validation plus durable attempt state. | No member charge; provider failure does not mutate billing; completed duplicate replay is metadata-only with no generated text. | Future target `ENABLE_ADMIN_AI_TEXT_BUDGET`; metadata-only in Phase 4.8.1. | Missing/malformed key, conflict, in-progress duplicate, provider failure terminal retry, sanitized metadata, caller-policy stripping. | Phase 4.8.1 completed for admin text only. |
...
| Admin embeddings | `platform_admin_lab_budget` | Phase 4.8.1 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only idempotency attempts; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate suppression through `admin_ai_usage_attempts`. | Runtime env kill switch and live platform budget caps remain future; current enforcement is metadata plan/fingerprint/caller-policy validation plus durable attempt state. | No member charge; provider failure does not mutate billing; completed duplicate replay is metadata-only with no raw input or embedding vectors. | Future target `ENABLE_ADMIN_AI_EMBEDDINGS_BUDGET`; metadata-only in Phase 4.8.1. | Missing/malformed key, conflict, in-progress duplicate, provider failure terminal retry, sanitized metadata, caller-policy stripping. | Phase 4.8.1 completed for admin embeddings only. |
...
| `/api/admin/ai/test-embeddings` / `admin.embeddings.test` | `platform_admin_lab_budget` | Phase 4.8.1 partial coverage: required `Idempotency-Key`, safe budget metadata, signed caller-policy metadata, durable metadata-only duplicate suppression/conflict detection; no vectors/full result replay/live caps | Completed for admin embeddings durable idempotency foundation |
...
9. Phase 4.9: Migrate one remaining Admin AI/internal caller path, such as admin music, compare, live-agent, sync video debug, unmetered image, or a remaining internal caller, without broad rewrites.
