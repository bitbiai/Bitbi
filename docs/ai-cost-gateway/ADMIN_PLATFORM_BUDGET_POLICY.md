...
Status: Phase 4.8 admin text/embeddings budget enforcement on top of the Phase 4.2 contract/helper foundation, Phase 4.3 charged Admin BFL image-test hardening, Phase 4.4 read-only budget evidence reporting, Phase 4.5 admin async video job budget enforcement, Phase 4.6 OpenClaw/News Pulse visual budget controls, and Phase 4.7 internal AI Worker caller-policy guard. Phase 4.8 covers only `POST /api/admin/ai/test-text` and `POST /api/admin/ai/test-embeddings`: both now require `Idempotency-Key`, build safe `platform_admin_lab_budget` metadata, and propagate signed `budget_metadata_only` caller-policy metadata to the AI Worker. Admin music/compare/live-agent, sync video debug, unmetered admin image, Admin video beyond Phase 4.5, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI, and broad internal AI Worker routes remain unmigrated or baseline-allowed. No real provider calls, Stripe calls, live billing enablement, public billing changes, credit mutations, deploys, remote migrations, or runtime route migrations outside this scope were performed.
...
- No broad runtime budget enforcement beyond the charged Admin BFL image-test branch, admin async video job caller path, News Pulse visual caller-side budget metadata/status checks, Phase 4.7 internal caller-policy guard for covered routes, and Phase 4.8 admin text/embeddings metadata-only coverage.
- No broad admin route migration beyond the charged Admin BFL image-test branch, admin async video job path, and admin text/embeddings test routes.
- No D1 schema except additive migrations `0049_add_admin_video_job_budget_metadata.sql` for sanitized job budget metadata and `0050_add_news_pulse_visual_budget_metadata.sql` for sanitized News Pulse visual budget metadata. Phase 4.7 and Phase 4.8 add no migration.
- No Admin UI.
...
- admin text/embeddings as Phase 4.8 `platform_admin_lab_budget` metadata-only coverage with required `Idempotency-Key` and signed caller-policy propagation
- Admin music/compare/live-agent, sync video debug, unmetered admin image, platform/background AI outside News Pulse visuals, and baseline-allowed internal AI Worker routes beyond covered caller paths as baselined gaps
...
- Known broad internal routes (`test-image`, `test-music`, sync video debug, compare, and live-agent) still allow missing policy only as explicit `baseline_allowed` gaps; admin text/embeddings now supply Phase 4.8 policy metadata, while missing policy remains allowed for other compatibility callers on those shared internal routes. Malformed supplied metadata is rejected.

Limits:

- Phase 4.7 does not migrate broad Admin AI, Admin music/text/compare/live-agent, sync video debug, OpenClaw/News Pulse beyond Phase 4.6 compatibility, or platform/background AI globally. Phase 4.8 narrows that by covering only admin text/embeddings; Admin music/compare/live-agent, sync video debug, unmetered admin image, OpenClaw/News Pulse beyond Phase 4.6, and platform/background AI remain unmigrated.
...
- There is no durable replay/conflict persistence yet, so the required idempotency key is metadata-only for duplicate suppression evidence and does not claim exactly-once provider execution.

Phase 4.8 does not change Admin music, Admin video beyond Phase 4.5, Admin compare, Admin live-agent, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI, unrelated internal AI Worker routes, member image/music/video behavior, org-scoped member image/text behavior, public pricing, Stripe behavior, credit debit behavior, credit clawback behavior, or live billing readiness.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Admin text test | `platform_admin_lab_budget` | Phase 4.8 requires `Idempotency-Key`, builds safe budget metadata, and propagates signed caller-policy metadata; no credits are debited. | Required, metadata-only; no durable replay/conflict persistence yet. | Runtime env kill switch and live platform budget caps remain future; current enforcement is metadata plan/fingerprint/caller-policy validation. | No member charge; provider failure does not mutate billing; replay remains disabled. | Future target `ENABLE_ADMIN_AI_TEXT_BUDGET`; metadata-only in Phase 4.8. | Missing/malformed key, sanitized metadata, caller-policy stripping, no provider on invalid key. | Phase 4.8 completed for admin text only. |
...
| Admin embeddings | `platform_admin_lab_budget` | Phase 4.8 requires `Idempotency-Key`, builds safe budget metadata, and propagates signed caller-policy metadata; no credits are debited. | Required, metadata-only; no durable replay/conflict persistence yet. | Runtime env kill switch and live platform budget caps remain future; current enforcement is metadata plan/fingerprint/caller-policy validation. | No member charge; provider failure does not mutate billing; replay remains disabled. | Future target `ENABLE_ADMIN_AI_EMBEDDINGS_BUDGET`; metadata-only in Phase 4.8. | Missing/malformed key, sanitized metadata, caller-policy stripping, no raw embedding input in metadata. | Phase 4.8 completed for admin embeddings only. |
...
| `/api/admin/ai/test-embeddings` / `admin.embeddings.test` | `platform_admin_lab_budget` | Phase 4.8 partial metadata-only coverage: required `Idempotency-Key`, safe budget metadata, signed caller-policy metadata; no durable replay/live caps | Completed for admin embeddings metadata-only budget enforcement |
...
7. Phase 4.8: Add admin text/embeddings budget metadata, required idempotency, and caller-policy propagation. Completed only for those two routes; durable replay/live platform caps remain future work.
8. Phase 4.9: Migrate one remaining Admin AI/internal caller path, such as admin music, compare, live-agent, sync video debug, unmetered image, or a remaining internal caller, without broad rewrites.
