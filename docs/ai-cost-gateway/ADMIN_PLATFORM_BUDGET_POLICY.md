...
Status: Phase 4.14 Admin Image branch handling on top of the Phase 4.2 contract/helper foundation, Phase 4.3 charged Admin BFL image-test hardening, Phase 4.4 read-only budget evidence reporting, Phase 4.5 admin async video job budget enforcement, Phase 4.6 OpenClaw/News Pulse visual budget controls, Phase 4.7 internal AI Worker caller-policy guard, Phase 4.8 admin text/embeddings budget metadata, Phase 4.8.1 durable idempotency, Phase 4.8.2 admin text/embeddings usage-attempt cleanup/inspection, Phase 4.9 Admin Music budget enforcement, Phase 4.10 Admin Compare budget enforcement, Phase 4.11 Admin Live-Agent audit/design prep, Phase 4.12 Admin Live-Agent budget enforcement, and Phase 4.13 Sync Video Debug retirement. Phase 4.13 adds no migration and classifies `POST /api/admin/ai/test-video` as retired/disabled-by-default rather than a normal provider-cost path. Phase 4.14 adds no migration and classifies `POST /api/admin/ai/test-image` branches: charged priced models remain the selected-organization `admin_org_credit_account` path, FLUX.2 Dev is the only explicit `explicit_unmetered_admin` exception with safe metadata, and unknown/unclassified Admin Image models are blocked before AI Worker/provider execution. Admin video beyond Phase 4.5, Admin text/embeddings beyond Phase 4.8/4.8.1/4.8.2, Admin Music beyond Phase 4.9, Admin Compare beyond Phase 4.10, Admin Live-Agent beyond Phase 4.12, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI, and broad internal AI Worker routes remain unmigrated or baseline-allowed. No real provider calls, Stripe calls, live billing enablement, public billing changes, credit mutations, deploys, remote migrations, or runtime route migrations outside this scope were performed.
...
- `@cf/black-forest-labs/flux-2-dev` remains available only as an intentional `explicit_unmetered_admin` admin lab exception. It emits sanitized `budget_policy` and `caller_policy` metadata, uses future kill-switch target `ENABLE_ADMIN_AI_UNMETERED_IMAGE_TESTS`, does not debit credits, does not add durable idempotency/replay, and does not store raw prompts or provider bodies in metadata.
...
- The route builds a Phase 4.2 budget plan with budget scope `platform_admin_lab_budget`, operation id `admin.live_agent`, fixed model `@cf/google/gemma-4-26b-a4b-it`, and future kill-switch target `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`.
...
Phase 4.12 adds no migration and does not add full stream replay, runtime env kill-switch enforcement, live platform budget caps, credit debit behavior, credit clawback behavior, public billing/pricing changes, Admin UI, sync video debug migration, unmetered admin image branch migration, Admin Video changes beyond Phase 4.5, Admin Text/Embeddings changes beyond Phase 4.8/4.8.1/4.8.2, Admin Music changes beyond Phase 4.9, Admin Compare changes beyond Phase 4.10, OpenClaw/News Pulse changes beyond Phase 4.6, platform/background AI migration, unrelated internal AI Worker migration, member image/music/video changes, org-scoped route changes, Stripe calls, real provider calls in tests, deploys, remote migrations, or live billing readiness.
...
| Charged admin BFL/priced image tests | `admin_org_credit_account` | Existing selected-org credit debit for priced Admin Image tests. | Required. | Phase 4.3 preserves org-credit debit and adds explicit admin budget policy plan/audit metadata, deterministic policy fingerprint, kill-switch target metadata, and sanitized usage/attempt metadata. Phase 4.14 preserves this branch unchanged. | No charge on provider failure; metadata-only replay remains acceptable until full output replay is designed. | Existing org balance plus metadata target `ENABLE_ADMIN_AI_BFL_IMAGE_BUDGET`; no new env flag is enforced in this phase. | Existing charge tests plus policy metadata and no unmetered fallback. | Phase 4.3 completed for charged branch hardening; Phase 4.14 preserved compatibility. |
| Explicit unmetered admin image branch | `explicit_unmetered_admin` | FLUX.2 Dev admin-only provider call, no debit. | Optional by documented exception. | Phase 4.14 classifies FLUX.2 Dev as intentional explicit-unmetered admin behavior, emits safe budget/caller metadata, and keeps it admin-only. | No credit debit; replay remains disabled and metadata-only. | Future metadata target `ENABLE_ADMIN_AI_UNMETERED_IMAGE_TESTS`; no runtime env flag is enforced in this phase. | Explicit-unmetered metadata, caller-policy stripping, and no raw prompt/provider body in metadata. | Phase 4.14 completed for explicit exception classification. |
...
| Admin live-agent | `platform_admin_lab_budget` | Phase 4.12 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only stream-session attempts before the streaming provider call; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate stream suppression through `admin_ai_usage_attempts`. | Runtime env kill switch, live platform caps, and explicit output-token/duration caps remain future; current enforcement is metadata plan/fingerprint/caller-policy validation plus durable stream-session attempt state. | Completed duplicate replay is metadata-only with no raw messages or streamed output; observable stream completion marks success, observable setup/stream errors mark failure, and unobserved stale active rows rely on bounded cleanup. | Future target `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`; metadata-only in Phase 4.12. | Missing/malformed key, conflict, in-progress duplicate, provider setup failure, stream finalization, caller-policy stripping, no message/output/provider body storage. | Phase 4.12 completed for Admin Live-Agent only. |
...
16. Phase 4.15: Address runtime budget kill-switch enforcement, live platform budget caps, or broader internal caller-policy gaps as the next narrow provider-cost gap.
