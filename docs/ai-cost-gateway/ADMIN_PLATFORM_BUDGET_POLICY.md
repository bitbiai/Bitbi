...
Status: Phase 4.15.1 Admin AI Budget Switch Control Plane on top of completed Phase 4.16 live platform budget cap design/evidence. Phase 4.15.1 adds additive migration `0052_add_admin_runtime_budget_switches.sql`, D1-backed app-level switch state/history, admin-only list/update APIs, and an Admin Control Plane panel. Provider-cost admin/platform execution now requires the Phase 4.15 Cloudflare master flag and the D1 app switch to be enabled; a missing/disabled master flag, missing/disabled D1 row, or unavailable D1 switch store fails closed before provider/internal AI/queue/credit/durable-attempt work. The app does not call Cloudflare APIs, edit Worker variables, store Cloudflare API tokens, or override disabled/missing Cloudflare master flags. Phase 4.16 remains completed and unchanged: it adds no migration and no runtime route behavior change, defines future daily/monthly cap semantics, adds read-only cap countability evidence, and records `liveBudgetCapsStatus: not_implemented` for already classified admin/platform paths. This is not live budget cap enforcement and not production readiness. Admin video beyond Phase 4.5, Admin text/embeddings beyond Phase 4.8/4.8.1/4.8.2, Admin Music beyond Phase 4.9, Admin Compare beyond Phase 4.10, Admin Live-Agent beyond Phase 4.12, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI outside News Pulse visuals, and broad internal AI Worker routes remain unmigrated or baseline-allowed. No real provider calls, Stripe calls, live billing enablement, public billing changes, credit mutations, deploys, remote migrations, Cloudflare changes, GitHub settings changes, runtime cap enforcement, or runtime route migrations outside this scope were performed.
...
- No live platform budget cap enforcement in Phase 4.16 and no broad runtime migration beyond the already budget-classified paths protected by Phase 4.15 runtime switches plus the Phase 4.15.1 D1 app-level switch: charged Admin Image, explicit-unmetered FLUX.2 Dev, admin async video jobs, News Pulse visuals, and admin text/embeddings/music/compare/live-agent.
...
- No D1 schema except additive migrations `0049_add_admin_video_job_budget_metadata.sql` for sanitized job budget metadata, `0050_add_news_pulse_visual_budget_metadata.sql` for sanitized News Pulse visual budget metadata, `0051_add_admin_ai_usage_attempts.sql` for admin text/embeddings/music/compare/live-agent idempotency attempts, and `0052_add_admin_runtime_budget_switches.sql` for app-level Admin AI budget switch state/history. Phase 4.7, Phase 4.8, Phase 4.8.2, Phase 4.9, Phase 4.10, Phase 4.11, Phase 4.12, Phase 4.13, Phase 4.14, Phase 4.15, and Phase 4.16 add no migration; Phase 4.15.1 adds `0052`.
- No Admin UI beyond the Phase 4.15.1 switch list/update panel. The panel cannot edit Cloudflare variables and does not expose Cloudflare values or secrets.
...
- Live platform budget cap evidence as Phase 4.16 coverage: `liveBudgetCapsStatus: not_implemented`, recommended first scope `platform_admin_lab_budget`, switch-enforced-but-not-cap-enforced operation ids, countability by budget scope, estimated-cost-unit availability, and durable completion timestamp availability
...
18. Phase 4.17: Recommended next target is the first narrow live cap foundation for `platform_admin_lab_budget`, covering Admin Text, Embeddings, Music, Compare, Live-Agent, and Admin async video jobs after an additive usage-ledger design is approved.
