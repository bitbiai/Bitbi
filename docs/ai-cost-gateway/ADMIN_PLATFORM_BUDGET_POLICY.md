...
Status: Phase 4.11 Admin Live-Agent budget flow audit and enforcement prep on top of the Phase 4.2 contract/helper foundation, Phase 4.3 charged Admin BFL image-test hardening, Phase 4.4 read-only budget evidence reporting, Phase 4.5 admin async video job budget enforcement, Phase 4.6 OpenClaw/News Pulse visual budget controls, Phase 4.7 internal AI Worker caller-policy guard, Phase 4.8 admin text/embeddings budget metadata, Phase 4.8.1 durable idempotency, Phase 4.8.2 admin text/embeddings usage-attempt cleanup/inspection, Phase 4.9 Admin Music budget enforcement, and Phase 4.10 Admin Compare budget enforcement. Phase 4.11 adds no migration and changes no Admin Live-Agent runtime behavior: no new `Idempotency-Key` requirement, no durable Live-Agent attempts, no provider calls, no budget enforcement, and no caller-policy behavior change. Admin Live-Agent remains a baseline gap with a Phase 4.12 target design; sync video debug, unmetered admin image, Admin video beyond Phase 4.5, Admin text/embeddings beyond Phase 4.8/4.8.1/4.8.2, Admin Music beyond Phase 4.9, Admin Compare beyond Phase 4.10, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI, and broad internal AI Worker routes remain unmigrated or baseline-allowed. No real provider calls, Stripe calls, live billing enablement, public billing changes, credit mutations, deploys, remote migrations, or runtime route migrations outside this scope were performed.
...
- No broad runtime budget enforcement beyond the charged Admin BFL image-test branch, admin async video job caller path, News Pulse visual caller-side budget metadata/status checks, Phase 4.7 internal caller-policy guard for covered routes, and Phase 4.8.1/4.9/4.10 admin text/embeddings/music/compare metadata-only idempotency coverage. Phase 4.8.2 only adds operational inspection/cleanup for those attempts, and Phase 4.11 is audit/design/prep only for Admin Live-Agent.
- No broad admin route migration beyond the charged Admin BFL image-test branch, admin async video job path, and admin text/embeddings/music/compare routes.
- No D1 schema except additive migrations `0049_add_admin_video_job_budget_metadata.sql` for sanitized job budget metadata, `0050_add_news_pulse_visual_budget_metadata.sql` for sanitized News Pulse visual budget metadata, and `0051_add_admin_ai_usage_attempts.sql` for admin text/embeddings/music/compare idempotency attempts. Phase 4.7, Phase 4.8, Phase 4.8.2, Phase 4.9, Phase 4.10, and Phase 4.11 add no migration; Phase 4.8.1 adds `0051`.
- No Admin UI.
...
- Admin live-agent as a Phase 4.11 audited/not-enforced baseline gap for Phase 4.12, plus sync video debug, unmetered admin image, platform/background AI outside News Pulse visuals, and baseline-allowed internal AI Worker routes beyond covered caller paths as baselined gaps
...
- The route records sanitized `budget_policy` and `caller_policy` summaries with operation id `admin.compare`, model summary `admin.compare.multi_model`, future kill-switch target `ENABLE_ADMIN_AI_COMPARE_BUDGET`, safe fingerprint, and only a hash of the idempotency key.
...
- The Auth Worker propagates `budget_metadata_only` caller-policy metadata under `__bitbi_ai_caller_policy`; the AI Worker validates supplied metadata after service-auth and strips the reserved key before provider payload construction.
- Phase 4.10 records the future kill-switch target only. Runtime env kill-switch enforcement and live platform budget caps remain future work.
...
Phase 4.11 is audit/design/prep only for Admin Live-Agent. It does not migrate `POST /api/admin/ai/live-agent`, does not require `Idempotency-Key`, does not add durable `admin_ai_usage_attempts` rows for Live-Agent, does not change caller-policy behavior, and does not enforce a platform budget.
...
- Caller-policy state: Auth currently sends baseline-style metadata when available, but `/internal/ai/live-agent` remains `baseline_allowed` if policy is missing. The AI Worker strips `__bitbi_ai_caller_policy` before provider payload construction.
...
- Budget scope `platform_admin_lab_budget`, operation id `admin.live_agent`, and future kill-switch target `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`.
...
| Admin compare | `platform_admin_lab_budget` | Phase 4.10 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only idempotency attempts before multi-model fanout; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate suppression through `admin_ai_usage_attempts`. | Runtime env kill switch and live platform budget caps remain future; current enforcement is metadata plan/fingerprint/caller-policy validation plus durable attempt state. | Partial model failure can still return successful compare output when at least one model succeeds; all-model provider failure marks the attempt failed. Completed duplicate replay is metadata-only with no compare results. | Future target `ENABLE_ADMIN_AI_COMPARE_BUDGET`; metadata-only in Phase 4.10. | Missing/malformed key, conflict, in-progress duplicate, provider failure terminal retry, partial success metadata, caller-policy stripping, no prompt/result/provider body storage. | Phase 4.10 completed for Admin Compare only. |
| Admin live-agent | `platform_admin_lab_budget` | Phase 4.11 audited only: admin-only streaming provider spend through `/internal/ai/live-agent`; no runtime behavior changed, no required idempotency, no durable attempt, no budget enforcement. | Not request-idempotent today; Phase 4.12 target is required request key plus durable parent stream-session attempt. | Stream-session budget metadata/lease with max duration/token estimate, signed caller-policy, and stop reason in Phase 4.12. | Metadata-only replay target; do not store raw messages or streamed output. | Future target `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`; stream duration/token caps and live platform caps remain future. | Phase 4.12 tests for idempotency, duplicate stream suppression, caller-policy propagation/stripping, stream failure/finalization safety, and no raw message/output leakage. | Phase 4.11 audit/design complete; Phase 4.12 runtime migration target. |
...
| `/api/admin/ai/live-agent` / `admin.live_agent` | `platform_admin_lab_budget` | Phase 4.11 audit/design complete only; still implicit streaming admin spend with no required idempotency, durable attempt, or runtime budget enforcement | Phase 4.12 admin live-agent budget enforcement |
...
| `/internal/ai/live-agent` | `internal.live_agent` | `internal_ai_worker_caller_enforced` | Admin Live-Agent caller remains baseline-compatible in Phase 4.11; Phase 4.12 should require signed policy for the covered admin caller |
...
12. Phase 4.11: Audit and design the Admin Live-Agent budget flow without runtime changes. Completed for Admin Live-Agent flow audit/prep only; Admin Live-Agent remains a baseline gap.
13. Phase 4.12: Implement the Admin Live-Agent budget migration using the Phase 4.11 target design: required idempotency, metadata-only durable stream-session attempts, signed caller-policy propagation, safe stream/failure finalization, and no broad Admin AI rewrite.
