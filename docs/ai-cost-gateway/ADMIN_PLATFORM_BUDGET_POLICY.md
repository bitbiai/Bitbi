...
Status: Phase 4.8.2 admin text/embeddings usage-attempt cleanup and inspection on top of the Phase 4.2 contract/helper foundation, Phase 4.3 charged Admin BFL image-test hardening, Phase 4.4 read-only budget evidence reporting, Phase 4.5 admin async video job budget enforcement, Phase 4.6 OpenClaw/News Pulse visual budget controls, Phase 4.7 internal AI Worker caller-policy guard, Phase 4.8 admin text/embeddings budget metadata, and Phase 4.8.1 durable idempotency. Phase 4.8.2 adds no migration and changes only operator safety for `admin_ai_usage_attempts`: admin-only sanitized list/detail APIs, a dry-run-default bounded cleanup endpoint, and scheduled bounded cleanup that marks expired pending/running attempts without deleting rows. Admin music/compare/live-agent, sync video debug, unmetered admin image, Admin video beyond Phase 4.5, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI, and broad internal AI Worker routes remain unmigrated or baseline-allowed. No real provider calls, Stripe calls, live billing enablement, public billing changes, credit mutations, deploys, remote migrations, or runtime route migrations outside this scope were performed.
...
- Phase 4.8.2 admin text/embeddings usage-attempt operations as bounded, non-destructive cleanup plus admin-only sanitized list/detail inspection
...
10. Phase 4.9: Migrate one remaining Admin AI/internal caller path, preferably admin music after the text/embeddings operational layer is validated, without broad rewrites.
