...
- No live platform budget cap enforcement beyond Phase 4.17's first `platform_admin_lab_budget` foundation for admin text, embeddings, music, compare, live-agent, and async video jobs. Other scopes remain future work.
...
- No D1 schema except additive migrations `0049_add_admin_video_job_budget_metadata.sql` for sanitized job budget metadata, `0050_add_news_pulse_visual_budget_metadata.sql` for sanitized News Pulse visual budget metadata, `0051_add_admin_ai_usage_attempts.sql` for admin text/embeddings/music/compare/live-agent idempotency attempts, `0052_add_admin_runtime_budget_switches.sql` for app-level Admin AI budget switch state/history, `0053_add_platform_budget_caps.sql` for the Phase 4.17 platform-admin-lab cap foundation, `0054_add_platform_budget_repair_actions.sql` for Phase 4.19 admin-approved repair audit rows, and `0055_add_platform_budget_evidence_archives.sql` for Phase 4.21 sanitized archive metadata. Phase 4.7, Phase 4.8, Phase 4.8.2, Phase 4.9, Phase 4.10, Phase 4.11, Phase 4.12, Phase 4.13, Phase 4.14, Phase 4.15, Phase 4.16, and Phase 4.20 add no migration.
- No Admin UI beyond the Phase 4.15.1 switch list/update panel, Phase 4.17 compact Platform Budget Caps panel, Phase 4.18 read-only Budget Reconciliation panel, Phase 4.19 explicit repair controls, Phase 4.20 read-only Repair Evidence Report/export panel, and Phase 4.21 Evidence Archives panel. The panels cannot edit Cloudflare variables, call Stripe, run automatic repair, expose Cloudflare values/secrets, mutate credits, mutate source attempts/jobs, or delete objects outside approved archive prefixes.
...
- Runtime budget switch evidence as Phase 4.15.1 coverage: Cloudflare master status plus D1 app switch status, effective master-and-app semantics, no secret values, default-disabled/fail-closed semantics, and no live cap claim
- Live platform budget cap evidence as Phase 4.17 coverage: `liveBudgetCapsStatus: platform_admin_lab_budget_foundation`, cap-enforced operation ids for the selected admin lab routes, safe daily/monthly limit and usage summaries when D1 is available, remaining switch-enforced-but-not-cap-enforced operation ids for other scopes, countability by budget scope, estimated-cost-unit availability, and durable completion timestamp availability
- Platform budget reconciliation and repair evidence as Phase 4.18/4.19/4.20 coverage: read-only `platform_admin_lab_budget` mismatch counts, repair candidate counts, not-checkable counts, explicit admin-approved repair executor availability, recent repair action summary, bounded repair evidence report/export availability, and confirmation that no automatic repair exists
...
The kill-switch target is `ENABLE_ADMIN_AI_VIDEO_JOB_BUDGET`; Phase 4.15 enforces it before job row creation and queueing, Phase 4.15.1 requires the D1 app switch, and Phase 4.17 checks `platform_admin_lab_budget` caps before queueing and records bounded usage when a job succeeds. Phase 4.7 keeps service-auth as the first AI Worker gate and adds caller-policy validation for the internal video task create/poll routes. No member image/music/video billing behavior, org-scoped member image/text behavior, public pricing, Stripe behavior, or organization credit debit behavior changed.
...
- Future kill-switch targets are recorded as `ENABLE_ADMIN_AI_TEXT_BUDGET` and `ENABLE_ADMIN_AI_EMBEDDINGS_BUDGET`; Phase 4.15/4.15.1 enforce master plus app switches, and Phase 4.17 enforces `platform_admin_lab_budget` daily/monthly caps before provider work.
...
- Phase 4.15 enforces `ENABLE_ADMIN_AI_MUSIC_BUDGET` before durable attempt creation or provider work, Phase 4.15.1 requires the D1 app switch, and Phase 4.17 enforces `platform_admin_lab_budget` caps before provider work and records bounded usage after success.
...
- The Auth Worker propagates `budget_metadata_only` caller-policy metadata under `__bitbi_ai_caller_policy`; the AI Worker validates supplied metadata after service-auth and strips the reserved key before provider payload construction.
- Phase 4.15 enforces `ENABLE_ADMIN_AI_COMPARE_BUDGET` before durable attempt creation or provider fanout, Phase 4.15.1 requires the D1 app switch, and Phase 4.17 enforces `platform_admin_lab_budget` caps before provider work and records bounded usage after successful compare output.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Admin text test | `platform_admin_lab_budget` | Phase 4.8.1 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only idempotency attempts; Phase 4.8.2 adds bounded cleanup and admin-only sanitized inspection; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate suppression through `admin_ai_usage_attempts`. | Phase 4.15 enforces `ENABLE_ADMIN_AI_TEXT_BUDGET`, Phase 4.15.1 requires the app switch, and Phase 4.17 enforces daily/monthly `platform_admin_lab_budget` caps before durable attempts or provider execution. | No member charge; provider failure does not mutate billing; successful completion records one bounded platform usage event; completed duplicate replay is metadata-only and does not double-count usage. | Runtime switch `ENABLE_ADMIN_AI_TEXT_BUDGET`; Phase 4.17 platform cap. | Missing/malformed key, disabled switch, missing/exceeded cap, conflict, in-progress duplicate, provider failure terminal retry, usage recording, sanitized metadata, caller-policy stripping, sanitized list/detail, dry-run cleanup, scheduled cleanup. | Phase 4.17 cap foundation. |
...
| Admin embeddings | `platform_admin_lab_budget` | Phase 4.8.1 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only idempotency attempts; Phase 4.8.2 adds bounded cleanup and admin-only sanitized inspection; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate suppression through `admin_ai_usage_attempts`. | Phase 4.15 enforces `ENABLE_ADMIN_AI_EMBEDDINGS_BUDGET`, Phase 4.15.1 requires the app switch, and Phase 4.17 enforces `platform_admin_lab_budget` caps before durable attempts or provider execution. | No member charge; provider failure does not mutate billing; successful completion records bounded usage; completed duplicate replay is metadata-only and does not double-count. | Runtime switch `ENABLE_ADMIN_AI_EMBEDDINGS_BUDGET`; Phase 4.17 platform cap. | Missing/malformed key, disabled switch, missing/exceeded cap, conflict, in-progress duplicate, provider failure terminal retry, usage recording, sanitized metadata, caller-policy stripping, sanitized list/detail, dry-run cleanup, scheduled cleanup. | Phase 4.17 cap foundation. |
| Admin music test | `platform_admin_lab_budget` | Phase 4.9 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only idempotency attempts; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate suppression through `admin_ai_usage_attempts`. | Phase 4.15 enforces `ENABLE_ADMIN_AI_MUSIC_BUDGET`, Phase 4.15.1 requires the app switch, and Phase 4.17 enforces `platform_admin_lab_budget` caps before durable attempts or provider execution. | No member charge; provider failure does not mutate billing; successful completion records bounded usage; completed duplicate replay is metadata-only with no audio, lyrics, prompts, or provider body and does not double-count. | Runtime switch `ENABLE_ADMIN_AI_MUSIC_BUDGET`; Phase 4.17 platform cap. | Missing/malformed key, disabled switch, missing/exceeded cap, conflict, in-progress duplicate, provider failure terminal retry, usage recording, sanitized metadata, caller-policy stripping, no audio/lyrics/provider body storage. | Phase 4.17 cap foundation. |
| Admin compare | `platform_admin_lab_budget` | Phase 4.10 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only idempotency attempts before multi-model fanout; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate suppression through `admin_ai_usage_attempts`. | Phase 4.15 enforces `ENABLE_ADMIN_AI_COMPARE_BUDGET`, Phase 4.15.1 requires the app switch, and Phase 4.17 enforces `platform_admin_lab_budget` caps before durable attempts or provider fanout. | Partial model failure can still return successful compare output when at least one model succeeds; successful completion records bounded usage; all-model provider failure marks the attempt failed and records no usage. Completed duplicate replay is metadata-only with no compare results and no double-count. | Runtime switch `ENABLE_ADMIN_AI_COMPARE_BUDGET`; Phase 4.17 platform cap. | Missing/malformed key, disabled switch, missing/exceeded cap, conflict, in-progress duplicate, provider failure terminal retry, partial success metadata, usage recording, caller-policy stripping, no prompt/result/provider body storage. | Phase 4.17 cap foundation. |
| Admin live-agent | `platform_admin_lab_budget` | Phase 4.12 requires `Idempotency-Key`, builds safe budget metadata, propagates signed caller-policy metadata, and stores metadata-only stream-session attempts before the streaming provider call; no credits are debited. | Required; durable key-hash/request-fingerprint conflict detection and duplicate stream suppression through `admin_ai_usage_attempts`. | Phase 4.15 enforces `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`, Phase 4.15.1 requires the app switch, and Phase 4.17 enforces `platform_admin_lab_budget` caps before durable stream-session attempts or provider stream setup. | Completed duplicate replay is metadata-only with no raw messages or streamed output; observable stream completion marks success and records bounded usage; observable setup/stream errors mark failure and record no usage; unobserved stale active rows rely on bounded cleanup. | Runtime switch `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`; Phase 4.17 platform cap; explicit output-token/duration caps remain future. | Missing/malformed key, disabled switch, missing/exceeded cap, conflict, in-progress duplicate, provider setup failure, stream finalization, usage recording, caller-policy stripping, no message/output/provider body storage. | Phase 4.17 cap foundation. |
...
| Admin async video jobs | `platform_admin_lab_budget` with internal caller-enforced subroutes | Phase 4.5 stores sanitized job/queue budget metadata before provider-cost processing; no credits are debited. | Job create requires `Idempotency-Key`; internal tasks inherit the job budget state. | Phase 4.15 enforces `ENABLE_ADMIN_AI_VIDEO_JOB_BUDGET`, Phase 4.15.1 requires the app switch, and Phase 4.17 enforces `platform_admin_lab_budget` caps before job row creation or queueing. | Polling is tied to persisted task id; output/poster success is preserved; output persistence failure does not mark budget state successful; successful jobs record bounded platform usage. | Runtime switch `ENABLE_ADMIN_AI_VIDEO_JOB_BUDGET`; Phase 4.17 platform cap. | Missing/invalid metadata, disabled switch, missing/exceeded cap, duplicate delivery, response-loss retry suppression, provider/output failures, usage recording, sanitized telemetry. | Phase 4.17 cap foundation. |
...
16. Phase 4.15: Enforce runtime budget switches for already budget-classified admin/platform provider-cost paths. Completed; live platform budget caps and broader internal caller-policy gaps remained future work at that phase.
17. Phase 4.16: Design live platform budget caps and add read-only cap evidence/check metadata only. Completed with no runtime cap enforcement, no route behavior change, no migration, and no provider/Stripe calls.
18. Phase 4.17: Implement the first narrow `platform_admin_lab_budget` cap foundation for Admin Text, Embeddings, Music, Compare, Live-Agent, and Admin async video jobs. Completed with additive local migration `0053_add_platform_budget_caps.sql`, admin-only cap APIs/UI, fail-closed daily/monthly cap checks, and bounded usage events; other scopes remain future work.
19. Phase 4.19: Add an explicit admin-approved `platform_admin_lab_budget` repair executor. Completed with additive local migration `0054_add_platform_budget_repair_actions.sql`, admin-only repair APIs/UI, idempotent repair audit rows, executable `create_missing_usage_event` only from still-successful local D1 source evidence, and review-only records for duplicate/orphan/failed-source/window/cost candidates. No automatic repair, provider call, Stripe call, credit mutation, source attempt/job mutation, or customer billing mutation is performed.
20. Phase 4.20: Add read-only operator repair evidence report/export for `platform_admin_lab_budget`. Completed with admin-only report/export APIs, JSON and Markdown exports, a compact Admin Control Plane report panel, bounded filters, and sanitized summaries over `platform_budget_repair_actions` plus related local D1 evidence. No migration, repair execution, automatic repair, provider call, Stripe call, usage/source mutation, credit mutation, member/org billing change, or live billing enablement is performed.
21. Phase 4.21: Add sanitized platform budget evidence archive/retention workflow for `platform_admin_lab_budget`. Completed with additive local migration `0055_add_platform_budget_evidence_archives.sql`, admin-only archive create/list/detail/download/expire/cleanup APIs, JSON and Markdown archives in `AUDIT_ARCHIVE` under `platform-budget-evidence/`, a compact Admin Control Plane archive panel, retention metadata, and bounded approved-prefix-only cleanup. Archive creation applies no repair, mutates no usage/source/repair rows, calls no providers/Stripe/Cloudflare, mutates no credits/member/org/customer billing, and does not prove production/live billing readiness.
