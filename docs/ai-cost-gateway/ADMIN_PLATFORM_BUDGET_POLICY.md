...
Status: Phase 4.17 first narrow `platform_admin_lab_budget` cap foundation on top of Phase 4.15 runtime Cloudflare master switches, Phase 4.15.1 D1 app-level switches, and completed Phase 4.16 cap design/evidence. Phase 4.17 adds additive migration `0053_add_platform_budget_caps.sql`, D1-backed daily/monthly cap limits, bounded usage events, admin-only cap APIs, and an Admin Control Plane cap panel for selected `platform_admin_lab_budget` routes only. Provider-cost execution for covered admin lab routes now requires the Cloudflare master flag, D1 app switch, and active cap allowance before provider/internal AI/queue/durable-attempt work. This is not customer billing, not Stripe/live billing, not production readiness, and not cap enforcement for other scopes. Admin org-credit image tests, explicit-unmetered image tests, OpenClaw/News Pulse, platform/background outside News Pulse, broad internal AI Worker routes, member routes, and org-scoped member routes are not migrated by Phase 4.17. No real provider calls, Stripe calls, live billing enablement, public billing changes, credit mutations, deploys, remote migrations, Cloudflare changes, GitHub settings changes, or unrelated route migrations were performed.
...
- No D1 schema except additive migrations `0049_add_admin_video_job_budget_metadata.sql` for sanitized job budget metadata, `0050_add_news_pulse_visual_budget_metadata.sql` for sanitized News Pulse visual budget metadata, `0051_add_admin_ai_usage_attempts.sql` for admin text/embeddings/music/compare/live-agent idempotency attempts, `0052_add_admin_runtime_budget_switches.sql` for app-level Admin AI budget switch state/history, and `0053_add_platform_budget_caps.sql` for the Phase 4.17 platform-admin-lab cap foundation. Phase 4.7, Phase 4.8, Phase 4.8.2, Phase 4.9, Phase 4.10, Phase 4.11, Phase 4.12, Phase 4.13, Phase 4.14, Phase 4.15, and Phase 4.16 add no migration.
- No Admin UI beyond the Phase 4.15.1 switch list/update panel and Phase 4.17 compact Platform Budget Caps panel. The panels cannot edit Cloudflare variables, call Stripe, or expose Cloudflare values or secrets.
