...
Status: Phase 4.10 Admin Compare budget enforcement on top of the Phase 4.2 contract/helper foundation, Phase 4.3 charged Admin BFL image-test hardening, Phase 4.4 read-only budget evidence reporting, Phase 4.5 admin async video job budget enforcement, Phase 4.6 OpenClaw/News Pulse visual budget controls, Phase 4.7 internal AI Worker caller-policy guard, Phase 4.8 admin text/embeddings budget metadata, Phase 4.8.1 durable idempotency, Phase 4.8.2 admin text/embeddings usage-attempt cleanup/inspection, and Phase 4.9 Admin Music budget enforcement. Phase 4.10 adds no migration and changes only `POST /api/admin/ai/compare`: required `Idempotency-Key`, sanitized `platform_admin_lab_budget` metadata, signed caller-policy propagation, and durable metadata-only `admin_ai_usage_attempts` duplicate suppression/conflict detection for compare fanout. Admin live-agent, sync video debug, unmetered admin image, Admin video beyond Phase 4.5, Admin text/embeddings beyond Phase 4.8/4.8.1/4.8.2, Admin Music beyond Phase 4.9, OpenClaw/News Pulse beyond Phase 4.6, platform/background AI, and broad internal AI Worker routes remain unmigrated or baseline-allowed. No real provider calls, Stripe calls, live billing enablement, public billing changes, credit mutations, deploys, remote migrations, or runtime route migrations outside this scope were performed.
...
- Admin live-agent, sync video debug, unmetered admin image, platform/background AI outside News Pulse visuals, and baseline-allowed internal AI Worker routes beyond covered caller paths as baselined gaps
...
12. Phase 4.11: Migrate one remaining Admin AI/internal caller path, such as Admin Live-Agent, sync video debug, the unmetered admin image branch, or another internal caller, without broad rewrites.
