...
- Sync video debug, unmetered admin image, platform/background AI outside News Pulse visuals, and broader internal routes remain tracked baseline gaps. Admin Live-Agent is now covered only by Phase 4.12 metadata-only stream-session enforcement; runtime env kill-switch enforcement, full stream replay, and live platform caps remain future work.
