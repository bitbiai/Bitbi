...
2. Resolve actor and billing scope
   ...
   - Phase 4.11 audits Admin Live-Agent only and records the Phase 4.12 target: `platform_admin_lab_budget`, `ENABLE_ADMIN_AI_LIVE_AGENT_BUDGET`, request-level idempotency, metadata-only stream-session attempts, signed caller-policy propagation, and no raw message/output persistence. It does not change Live-Agent runtime behavior.
...
- The implementation intentionally does not migrate admin debug video, broad platform budget enforcement, OpenClaw/News Pulse visuals before Phase 4.6, or internal AI Worker video task routes globally. Admin async video jobs are covered separately by Phase 4.5 and News Pulse visuals by Phase 4.6.
...
- Admin live-agent, sync video debug, unmetered admin image, platform/background AI outside News Pulse visuals, and broader internal routes remain tracked baseline gaps.
