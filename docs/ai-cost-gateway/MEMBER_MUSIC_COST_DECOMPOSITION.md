...
Status: Phase 3.7 hardens the already migrated member `/api/ai/generate-music` AI Cost Gateway path, and Phase 3.8 later migrates member video separately. Phase 4.9 Admin Music budget enforcement and Phase 4.10 Admin Compare budget enforcement are separate and do not change member music pricing, debit timing, replay behavior, or saved asset behavior. This document records the member music cost decomposition, the implemented parent-reservation policy, Phase 3.7 replay/cover/finalization/cleanup hardening, and remaining music-specific gaps. It does not document admin AI, platform/background AI, OpenClaw/News Pulse, or internal AI Worker route migration beyond noting that admin music is separate. It does not call real AI providers in tests, call Stripe, deploy, or prove production/live billing readiness.
...
10. After successful billing, `scheduleMemberMusicCoverGeneration()` runs background cover generation through `workers/auth/src/lib/member-music-cover.js`. Phase 3.6 policy includes that cover call in the parent bundled music reservation with no separate user-visible charge; Phase 3.7 writes safe cover status back to the parent attempt.
...
| --- | --- | --- |
| Lyrics success plus music failure | Parent reservation is active; music failure releases the reservation and charges nothing. | Raw generated lyrics are intentionally not stored in attempt metadata or replay responses. |
| Music success plus cover failure | Music save/debit can succeed; cover failure is logged and non-fatal. | Cover failure does not claw back or alter music credits. Phase 3.7 records safe `failed` cover status on the parent attempt when the background task runs. |
...
Phase 3.7 keeps the single parent music reservation/debit model and adds:

- replay-unavailable handling for completed attempts whose stored replay metadata is missing or expired;
- no automatic provider re-execution and no second debit for completed same-key attempts when replay is unavailable;
- replay responses that omit raw prompt and raw lyrics while returning bounded asset/audio metadata;
- parent-attempt cover status writeback with `pending`, `succeeded`, `failed`, and `skipped` states;
- terminal billing/finalization error handling when debit succeeds but attempt result metadata cannot be written;
- scheduled cleanup for expired/stuck member attempts and expired replay metadata without deleting saved media or mutating ledgers.

## Phase 3.7 Result

Phase 3.7 hardens only already migrated member image and member music generation. Phase 3.8 later adds member video gateway coverage in its own route. Phase 4.9 covers only Admin Music test generation using `platform_admin_lab_budget` metadata and `admin_ai_usage_attempts`; Phase 4.10 covers only Admin Compare using the same metadata-only attempt foundation; Phase 4.12 covers only Admin Live-Agent stream-session metadata/idempotency; Phase 4.14 classifies Admin Image branches without changing member music. None of those phases changes member music behavior. Sync video debug is retired/disabled-by-default, FLUX.2 Dev Admin Image is explicit-unmetered, unclassified Admin Image models are blocked before provider execution, and platform/background AI outside News Pulse visuals plus remaining broad internal AI Worker routes remain unmigrated or baselined. No real AI provider calls were made by Codex/tests, no Stripe calls were made, no remote migration was applied, and production/live billing remains BLOCKED.
