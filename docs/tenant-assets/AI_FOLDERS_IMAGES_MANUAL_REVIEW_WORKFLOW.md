...
Phase 6.21 adds legacy media reset dry-run/export planning only. Phase 6.22 adds `LEGACY_PERSONAL_MEDIA_RESET_EXECUTOR_DESIGN.md`. Phase 6.23 adds migration `0058_add_legacy_media_reset_actions.sql`, a dry-run-default reset executor endpoint, and read-only action/evidence endpoints. Phase 6.24 adds operator dry-run evidence docs and keeps the reset decision at `legacy_media_reset_dry_run_pending` because no live/main executor dry-run evidence file is committed. The executor remains limited to folders/images/derivatives/public refs, rejects video/music/text/profile/export/audit domains, and still does not approve ownership backfill, access-check switching, tenant isolation, production readiness, or live/main execution.
