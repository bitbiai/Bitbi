...
Recommended: `Phase 6.22 - Admin-approved Legacy Media Reset Executor Design`, after reviewing the Phase 6.21 read-only reset dry-run/export evidence.

Phase 6.21 adds legacy media reset planning only. A later idempotency-evidence completion phase may still be needed before any backfill-readiness report. All future work must still avoid backfill, access-check switching, source asset mutation, ownership metadata updates, review-row mutation unless separately approved, R2 actions, provider calls, Stripe calls, Cloudflare mutations, and billing/credit mutations.
