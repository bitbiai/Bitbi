...
- Phase 6.23 adds `0058_add_legacy_media_reset_actions.sql`, reset action/event tables, `POST /api/admin/tenant-assets/legacy-media-reset/execute`, and read-only action/evidence endpoints. The executor defaults to dry-run and is limited to first-pass folders/images/derivatives/public references. Codex/tests did not execute live/main deletion, run remote migrations, deploy, backfill ownership, switch access checks, mutate billing/credits, or list/mutate live R2.
...
| 6.23 | Admin-approved legacy media reset action tracking and executor; implemented as migration `0058`, dry-run-default POST execution endpoint, and read-only action/evidence endpoints. First-pass domains are folders/images/derivatives/public refs; no live/main execution by Codex/tests, ownership backfill, access switch, billing/credit mutation, or live R2 listing/mutation. |
| 6.24 | Legacy media reset operator dry-run evidence; implemented as decision/runbook/template docs only. Current status is `legacy_media_reset_dry_run_pending` because no live/main executor dry-run evidence file is committed; no runtime behavior changed. |
| 6.25 | Legacy media reset dry-run closure and confirmation gate; implemented as evidence recheck, pending decision update, and confirmation-gate checklist only. No runtime behavior changed and no executor call was made. |
