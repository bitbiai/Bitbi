...
| Irreversible deletion risk | Critical | Cleanup helpers can delete approved prefixes; broad owner-map is not proven. Phase 6.21 adds D1-only reset planning, Phase 6.22 designs the executor, and Phase 6.23 adds a dry-run-default executor path for first-pass folders/images domains. | `r2_cleanup_queue`, lifecycle cleanup, export cleanup. | Keep destructive live/main cleanup blocked until operator dry-run evidence, owner approval, and idempotent audited execution evidence exist. | Verify executor dry-runs emit no delete/move/backfill commands and reports keep `r2LiveListed=false`; confirmed execution must be separately approved and evidenced. | Phase 6.24 operator dry-run evidence |
