...
| Irreversible deletion risk | Critical | Cleanup helpers can delete approved prefixes; broad owner-map is not proven. Phase 6.21 adds D1-only reset planning, but no delete executor. | `r2_cleanup_queue`, lifecycle cleanup, export cleanup. | Keep destructive cleanup blocked until dry-run evidence, executor design, owner approval, and idempotent audited execution exist. | Verify dry-run emits no delete/move/backfill commands and reports `r2LiveListed=false`, `r2ObjectsMutated=false`. | 6.22 executor design only |
