...
Phase 6.1 adds risk evidence only. Phase 6.2 adds a focused owner-map dry run for `ai_folders` and `ai_images`. Phase 6.3 adds the schema/access impact plan. Phase 6.4 adds nullable ownership metadata columns for those two tables only. Phase 6.5 assigns metadata only for new personal folder/image writes. Phase 6.6 adds read-only simulated dual-read diagnostics. Phase 6.7 exposes those diagnostics through a bounded admin evidence report/export. Phase 6.8 adds the operator evidence runbook/template/checklist for collecting that report on main/live. Phase 6.9 adds a main-only evidence package directory. Phase 6.10 reviews the real main evidence summary; metadata missing, public unsafe, derivative risk, simulated dual-read unsafe, and manual-review counts remain nonzero, so access-switch/backfill stay blocked. Phase 6.11 designs manual review workflow, Phase 6.12 designs review-state schema, Phase 6.13 adds empty review-state tables/indexes only, Phase 6.14 adds local review-item import dry-run planning, Phase 6.15 adds an admin-approved import executor that writes review items/events only when confirmed, Phase 6.16 adds read-only queue/evidence APIs, Phase 6.17 adds admin-approved review status updates on review items/events only, Phase 6.18 adds Admin queue/status visibility, Phase 6.19 adds operator evidence collection docs, Phase 6.20 reviews live/main operator evidence with `operator_evidence_collected_needs_more_idempotency`, Phase 6.21 adds read-only legacy personal media reset dry-run/export planning, Phase 6.22 designs the reset executor, Phase 6.23 adds action tracking plus an admin-approved reset executor path, Phase 6.24 records pending operator dry-run evidence, and Phase 6.25 adds a confirmation gate while keeping dry-run evidence pending because no live/main executor dry-run file is committed. These phases do not backfill ownership, execute live/main media deletion by Codex/tests, delete media, assign organization ownership, update ownership metadata, move/list/delete live R2 objects, call providers, call Stripe, mutate Cloudflare, change access checks, change generation behavior, mutate billing/credits, or claim full tenant isolation.

| Risk | Severity | Evidence source | Affected tables/routes | Proposed mitigation | Safe dry-run signal | Future phase |
| --- | --- | --- | --- | --- | --- | --- |
| Ownership ambiguity | High | `ai_images.user_id`, `ai_text_assets.user_id`, `ai_folders.user_id` remain the active access owner signal. Phase 6.2 fixture dry-run shows user-only rows are only medium-confidence personal candidates. Phase 6.5 assigns high-confidence personal owner metadata only for new folder/image writes; Phase 6.6 flags old/null and conflicting rows; Phase 6.7 surfaces bounded admin evidence without changing access; Phase 6.10 real main evidence records 75 metadata-missing rows, 42 simulated dual-read unsafe rows, and 90 manual-review rows. Phase 6.13 adds empty review-state tables but imports no rows. Phase 6.14 dry-runs aggregate buckets from Markdown evidence and JSON item-level candidates. Phase 6.15 can import review items/events only when explicitly confirmed. Phase 6.16 can read/list/export those review rows only. Phase 6.17 can update review item statuses and append status events only. Phase 6.20 records live/main import and queue evidence but still needs idempotency replay/conflict completion. Phase 6.21 inventories reset candidates without deleting or mutating anything. Phase 6.22 designs the reset executor. Phase 6.23 adds action tracking and an admin-approved reset executor path. Phase 6.25 finds no committed live/main executor dry-run evidence and adds the confirmation gate, so reset confirmation remains blocked. | `ai_images`, `ai_text_assets`, `ai_folders`, private asset routes. | Prove owner-map or safe retirement path before backfill and keep read diagnostics clean before access changes. | Count rows lacking target owner classification, simulated dual-read conflicts, unresolved review statuses, reset candidate blockers, incomplete idempotency evidence, and reset dry-run evidence. | Phase 6.26 operator dry-run evidence |
...
| Irreversible deletion risk | Critical | Cleanup helpers can delete approved prefixes; broad owner-map is not proven. Phase 6.21 adds D1-only reset planning, Phase 6.22 designs the executor, Phase 6.23 adds a dry-run-default executor path for first-pass folders/images domains, and Phase 6.25 records missing live/main dry-run evidence plus a confirmation gate. | `r2_cleanup_queue`, lifecycle cleanup, export cleanup. | Keep destructive live/main cleanup blocked until operator dry-run evidence, owner approval, and idempotent audited execution evidence exist. | Verify executor dry-runs emit no delete/move/backfill commands and reports keep `r2LiveListed=false`; confirmed execution must be separately approved and evidenced. | Phase 6.26 operator dry-run evidence |
...
Phase 6.18 adds status operator evidence rollups and Admin Control Plane visibility:

- queue evidence includes status workflow availability, latest status update timestamps, and terminal approved/blocked status rollups
- the Admin Control Plane panel exposes safe summary cards, filters, JSON export, item detail, event history, and review-status controls for review-state rows only
- no migration, ownership backfill, access switch, source asset row update, ownership metadata update, R2 operation, provider call, Stripe call, Cloudflare mutation, credit/billing mutation, or tenant-isolation claim is added

## Phase 6.19 Result

Phase 6.19 adds manual-review status operator evidence collection docs only:
...
- current decision status is `operator_evidence_collected_needs_more_idempotency`: sanitized live/main import dry-run, confirmed import, final queue/status export, and one status-change rollup are present, but same-key replay/conflict evidence and a successful standalone status-update response remain pending
- no runtime behavior, migration, import execution, status update, source asset row update, ownership metadata update, ownership backfill, access switch, R2 operation, provider call, Stripe call, Cloudflare mutation, credit/billing mutation, or tenant-isolation claim is added

## Phase 6.22 Result

Phase 6.22 adds `LEGACY_PERSONAL_MEDIA_RESET_EXECUTOR_DESIGN.md` only. It defines future allowed/deferred reset domains, endpoint shape, deletion order, public/gallery handling, derivative/R2 safety, action tracking, audit/idempotency, partial-failure, and verification requirements. It adds no executor, endpoint, UI, migration, deletion, source mutation, ownership metadata update, review-row mutation, ownership backfill, access switch, R2 listing/mutation, provider call, Stripe call, Cloudflare mutation, credit/billing mutation, or tenant-isolation claim.

## Phase 6.23 Result

Phase 6.23 adds migration `0058_add_legacy_media_reset_actions.sql`, reset action/event tables, the admin-approved dry-run-default executor endpoint, and read-only action/evidence endpoints. First-pass execution is limited to `ai_images`, `ai_folders`, `ai_image_derivatives`, and `public_gallery_references`; video, music, text assets, profile avatars, data lifecycle exports, audit archives, unknown media tables, and manual-review supersession remain deferred. Codex/tests did not execute the reset against live/main data, run remote migrations, deploy, backfill ownership, switch access checks, mutate billing/credits, or list/mutate live R2.
