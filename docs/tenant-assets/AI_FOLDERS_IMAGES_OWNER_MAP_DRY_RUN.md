...
Phase 6.24 adds the operator dry-run evidence decision/runbook/template only and records `legacy_media_reset_dry_run_pending` because no live/main executor dry-run evidence file is committed. Phase 6.25 should be **Operator Runs Legacy Media Reset Dry-run** if the owner continues the reset path.
