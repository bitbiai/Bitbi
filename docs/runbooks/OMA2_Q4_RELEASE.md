...
Standard and Fast UI require Linux homepage core and macOS native core (macos-15, Node22.23.2, locked Playwright1.58.2). The 2026-09-09 decorative-preview policy replaces historical timing/stress release criteria: available media actually plays, failed or slow speculative media retains a visible preview/poster, navigation stays usable, suspension and manual pause retain their meaning. Each slot prepares at most one source for up to8 seconds; failure/cancellation holds the current content without automatic retries. Four-second source changes are not promised. Core negative tests still reject missing output, invalid identities, ignored pause and corrupt media.

`test:homepage-functional` and `test:homepage-webkit` select the same named core media scenarios in Chromium/macOS-WebKit. Explicit `test:homepage-functional:extended` / `test:homepage-webkit:extended` retain multi-loop, independent decoder comparisons and decorative transition stress in Full regression, not normal/Fast release gates. The separate bounded `diagnose:homepage-linux-media` keeps Linux-WebKit divergence visible. No old red result is certified retroactively. macOS WebKit is not a Safari/iPhone device certificate. Security, primary players, API and accounting coverage are unchanged.

This is Stefan's explicitly authorized 2026-09-08 acceptance-platform change. In e7ef7cbd's completed Linux runs, an ordinary original one-second MP4 stalls after its first loop even over real HTTP, outside the Hero. The historical failure remains recorded; a macOS pass does not repair or explain the Linux decoder. Revisit on a Playwright/WebKit/image/codec or media-serving change, or a matching supported-client report. An independent native frame-metadata/time/event control with the original and a second changing one-second fixture distinguishes observer and media boundaries without an FPS quota. The existing corrupt-output and frozen-slot countercontrols remain required. The 34312435904 macOS fourth-slot seek/output stall remains a recorded failure, not explained by a one-video pass. Independent four-video controls now use the same HTTP sources; progress polling avoids synchronous layout/decoder inspection. Each observed seek requires fresh subsequent output in its current identity. Detailed native metadata is captured separately on completion/failure. See [the regression register](REGRESSION_REGISTER.md) for evidence and the unresolved historical attribution.

`check:homepage-selection` checks actual discovery and exact Chromium/macOS native-scenario parity, plus retained Linux non-decoder coverage. The broad static job no longer repeats the dedicated native-video specs. The final Linux homepage and macOS native core groups have no fail-fast limit or retries and run every selected core scenario; extended discovery also rejects lost stress scenarios. Standard/full broad browser jobs wait for Release, Worker, Linux homepage and native macOS WebKit results; unexecuted downstream jobs remain open. There is no repeated overlapping native preflight. Acceptance and timing diagnostics use the built `_site` and refuse reused servers; source-based broad/Firefox tests remain complementary, not build acceptance. Artifacts upload even on failure. Native jobs test the built `_site`; macOS test processes use a private HOME, cleaned environment and inherited loopback-only OS sandbox. Image pull, installation and execution times remain separate; no future duration is guaranteed.
...
| Native WebKit core; playback, degraded previews, pause and navigation | `macos-15`, Node22.23.2, lockfile Playwright, fresh `_site`, private HOME/loopback sandbox | Required named core group, no fail-fast or retries; decoder stress uses explicit extended commands. Missing scenario/failed job blocks release. |
...
| Independent native Linux WebKit divergence | Same build/MP4s, dedicated bounded one-/four-video, two-source diagnostic | Not media acceptance; timeout/tooling status and artifacts remain explicit. Required Mac/Linux functional jobs stay independent. |
...
The release guard preserves cumulative Q4 base `8292a4926bb1bf24679db9dd2b87cd6882f4d4f7`. Push test selection uses its actual changed inputs; this workflow repair selects full coverage. Only an explicit validation dispatch forces full selection independently of the diff. A partial build may be tested but cannot be reused for Q4 publication. The candidate manifest identifies `decorative-core-v1`; native proof requires every named core scenario, not an old test count. Do not queue a second full manual run. The early job builds and uploads one `pages-candidate-<sha>-<run>-<attempt>` artifact. Standard Linux/macOS homepage jobs download and hash-check those exact bytes before/after their native tests; source-based broad tests remain complementary. Their separate immutable proof artifacts bind both OS results to the same build. Full-regression and Fast UI retain their own stated build scope and are not reusable sources for this protocol.

After that candidate finishes, dispatch the existing `static.yml` on its exact still-current main SHA with `candidate_run_id`, `candidate_run_attempt`, the original Q4 `release_plan_base_ref`, and the existing truthful dependency acknowledgement. The publish-only path checks the trusted repository, source attempt/commit, actually executed required jobs/steps, unexpired artifacts/digests, full-scope manifest and both tested build proofs. It rejects later unresolved required failures and a superseded main. Missing/wrong acknowledgement and uncategorized paths still fail the actual whole-diff guard immediately before publication. No new buildtoken or build is generated in this path. A completed validation with a failed Pages write may be reused only when the required validation jobs themselves all passed.

Only an eligible deploy job holds `group: pages`, shared with Fast UI. Both paths check current main under that lock before the official Pages action. A source request is rechecked immediately before the write; API/permission errors fail without custom waiting/reconciliation. Artifact download uses the official digest-failing action. See [download-artifact inputs](https://github.com/actions/download-artifact) and [run-attempt job metadata](https://docs.github.com/en/rest/actions/workflow-jobs#list-jobs-for-a-workflow-run-attempt). No-CI-Wait applies after push and later publish-only dispatch; a pending run is an exact continuation point, not permission to infer publication. Keep the Q4 baseline until its publication is confirmed; do not substitute a last-test-commit range.

After confirmed Q4 publication, retire its temporary default range before the next ordinary feature delivery, together with the Q4-only candidate protocol assumptions. Keep the existing change-aware selection and explicit full release acceptance distinct. Do not clear the pending range merely because CI is green or Pages has only started.
