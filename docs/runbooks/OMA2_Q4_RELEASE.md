...
Normal, full-regression and fast UI CI require both `homepage-validation` (Linux) and `homepage-webkit-media` (standard `macos-15`, Node 22.23.2, lockfile Playwright 1.58.2). Deploy jobs depend on both. Linux retains all Worker/workerd/D1/R2/API/security checks, Chromium video and WebKit non-decoder homepage functions, including mobile image loading. Both `homepage-hero-playback.spec.js` and `homepage-native-control.spec.js` native WebKit cases belong to mandatory macOS acceptance (the original 15 scenarios, native pause countercontrol and two four-video independent controls); Chromium retains both in Linux acceptance. `diagnose:homepage-linux-media` separately records the independent one-/four-video Linux WebKit controls, with finite play/seek/frame waits and a 60-second process-group limit. Timeout/incomplete output is a diagnostic finding, never a native pass; required test failures still block release. All three workflow paths retain this separate report. Browser-internal progress windows preserve observed output before a normal source change without weakening per-source/pause-epoch validation or increasing functional deadlines. macOS WebKit is not a real Safari/iPhone device certification.
...
`check:homepage-selection` checks actual discovery and exact Chromium/macOS native-scenario parity, plus retained Linux non-decoder coverage. The broad static job no longer repeats the dedicated native-video specs. The final Linux homepage and macOS native media groups have no fail-fast limit or retries and run every scenario. Standard/full broad browser jobs wait for Release, Worker, Linux homepage and native macOS WebKit results; unexecuted downstream jobs remain open. There is no repeated overlapping native preflight. Acceptance and timing diagnostics use the built `_site` and refuse reused servers; source-based broad/Firefox tests remain complementary, not build acceptance. Artifacts upload even on failure. Native jobs test the built `_site`; macOS test processes use a private HOME, cleaned environment and inherited loopback-only OS sandbox. Image pull, installation and execution times remain separate; no future duration is guaranteed.
...
| Native WebKit media; original scenarios, pause countercontrol and four-video controls | `macos-15`, Node22.23.2, lockfile Playwright, fresh `_site`, private HOME/loopback sandbox | Required complete group, no fail-fast or retries. Missing scenario/failed job blocks release. |
...
After confirmed Q4 publication, retire its temporary default range and full override before the next ordinary feature delivery, together with the Q4-only candidate protocol assumptions. Keep the existing change-aware selection and explicit full release acceptance distinct. Do not clear the pending range merely because CI is green or Pages has only started.
