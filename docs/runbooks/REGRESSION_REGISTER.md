...
| EN hidden-init/bfcache failure, run34642675559 / macOS job103406370743 | Original sample 12 invalidates right-bottom output at seek baseline 20; samples 13/14 still have 20 despite window-start output 9. Left-top remains seeking through samples 1–11. The delay's engine/scheduling/probe cause remains unresolved; no product or acceptance change. | `homepage-hero-state.spec.js` replays the exact projected 14-sample fixture, keeps it red, tests fresh-output and stale identity/pause/error countercases. Existing observer now reports effective seek baseline, last invalidation, per-sample cost/time and bounded native metadata; diagnostic fields never grant a pass. | Existing `playwright.homepage.config.js` / homepage functional selection executes the state tests in Chromium/WebKit; `playwright.homepage-webkit.config.js` retains native EN/DE lifecycle cases. | Local original EN and instrumented EN/DE/native countercases passed; this neither repairs nor re-certifies the failed CI run. Synthetic PageTransitionEvents are not real bfcache navigation. Private raw reports preserve exact inputs and the unresolved scheduling/seek hypotheses. |
