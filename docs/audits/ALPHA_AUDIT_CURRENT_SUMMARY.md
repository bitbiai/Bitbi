...
- Tenant-owned asset migration implementation and self-service privacy flows remain incomplete; Phase 6.7 evidence reports, Phase 6.8 collection docs, Phase 6.9 evidence package, Phase 6.10 main evidence decision, Phase 6.11 manual-review workflow, Phase 6.12 review-state schema design, Phase 6.13 review-state schema foundation, Phase 6.14 import dry-run planning, and Phase 6.15 review-item import executor do not migrate old ownership rows, assign organization ownership, change access checks, mutate source asset rows, or move/list/delete R2 objects.
...
Use `docs/tenant-assets/evidence/MAIN_FOLDERS_IMAGES_OWNER_MAP_DECISION.md` as the current Phase 6.10 decision, `docs/tenant-assets/AI_FOLDERS_IMAGES_MANUAL_REVIEW_WORKFLOW.md` for the Phase 6.11 review design, `docs/tenant-assets/AI_FOLDERS_IMAGES_MANUAL_REVIEW_STATE_SCHEMA_DESIGN.md` for the Phase 6.12/6.13 schema plan/foundation, `npm run tenant-assets:dry-run-review-import` for Phase 6.14 planning, and `POST /api/admin/tenant-assets/folders-images/manual-review/import` for Phase 6.15 review-item import. The next tenant-asset step should collect import operator evidence or design read/status workflow, with no broad backfill or access behavior change.
