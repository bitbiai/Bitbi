...
For tenant asset ownership evidence, use `docs/tenant-assets/TENANT_ASSET_OWNERSHIP_EVIDENCE_RUNBOOK.md`, `docs/tenant-assets/TENANT_ASSET_OWNERSHIP_EVIDENCE_TEMPLATE.md`, `docs/tenant-assets/evidence/`, `docs/tenant-assets/evidence/MAIN_FOLDERS_IMAGES_OWNER_MAP_DECISION.md`, `docs/tenant-assets/AI_FOLDERS_IMAGES_MANUAL_REVIEW_WORKFLOW.md`, and `docs/tenant-assets/AI_FOLDERS_IMAGES_MANUAL_REVIEW_STATE_SCHEMA_DESIGN.md`. Phase 6.10/6.11/6.12 decision, workflow, and schema-design records do not prove full tenant isolation, do not switch access checks, do not backfill old rows, do not create review rows, do not add the future review-state migration, and do not list or mutate R2. The current real main evidence decision requires manual review and blocks tenant-isolation/access-switch claims.
...
| Phase 6.12 manual review state schema design exists without migration, review rows, access switch, backfill, or R2 listing |  | BLOCKED |  |
...
- Phase 6.12 tenant asset manual-review state schema design record, with no migration/review-row/backfill/access-switch claim:
