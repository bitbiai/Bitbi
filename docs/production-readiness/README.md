...
Use `docs/tenant-assets/evidence/MAIN_FOLDERS_IMAGES_OWNER_MAP_DECISION.md` for the current AI folders/images owner-map evidence decision. As of Phase 6.10, the decision is `pending_main_evidence` because no real main evidence export is present in-repo. That pending status blocks ownership access-check switching, old-row backfill, and tenant-isolation claims. The review does not mutate D1/R2, list live R2, change access checks, or prove production readiness.
