...
Current audit/restart status is summarized in `docs/audits/ALPHA_AUDIT_CURRENT_SUMMARY.md`. DOC-1 keeps this file focused on evidence requirements rather than phase chronology. Operators must record budget switch, cap, reconciliation, repair, report/export, archive, tenant asset dry-run/schema/new-write evidence, and Admin Control Plane smoke evidence before enabling admin/platform provider-cost flags for a canary. Evidence workflows do not apply repairs automatically, mutate source attempts/jobs or usage rows, call providers, call Stripe, change member/org billing, backfill or enforce asset ownership, change access checks, move/delete R2 objects, or make production/live billing ready.
...
| Tenant asset ownership dry-run/schema/new-write evidence | Repo maintainer | Local repo/source inventory | None | Record `npm run dry-run:tenant-assets`, focused `npm run dry-run:tenant-assets:images`, migration `0056` status, Phase 6.5 new personal folder/image write metadata tests, and `docs/tenant-assets/AI_FOLDERS_IMAGES_SCHEMA_ACCESS_PLAN.md` summaries only; no live rows, R2 listings, ownership backfill, access changes, or raw object keys required. |
