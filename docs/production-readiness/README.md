...
Phase 4.8.2 adds admin-only sanitized inspection and bounded non-destructive cleanup for `admin_ai_usage_attempts` created by admin text/embeddings idempotency. Phase 4.9 adds no migration and extends that metadata-only idempotency foundation only to Admin Music test generation. It does not migrate Admin Compare/Live-Agent, sync video debug, unmetered admin image, member routes, or unrelated internal routes; it does not change public billing, call providers or Stripe, enable live billing, or make production/live billing ready. Operators should verify list/detail redaction, dry-run cleanup, execution of expired pending/running rows only, scheduled count-only cleanup logs, Admin Music duplicate-suppression/conflict behavior, and no credit/billing/provider side effects before using those endpoints operationally.
