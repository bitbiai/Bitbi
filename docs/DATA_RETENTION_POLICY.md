...
| AI usage attempts/reservations | Phase 2-D adds `ai_usage_attempts` for org-scoped image-generation idempotency, provider execution suppression, temporary result replay, and credit reservation/finalization state. Phase 2-E adds bounded scheduled/admin cleanup and admin-only sanitized inspection. | Short-to-medium operational and billing-dispute window to be defined; expired/stuck attempts should be cleaned through bounded batches. | Retain/anonymize according to billing/audit policy; do not export raw hashes by default. Cleanup releases stale reservations and expires replay metadata, but does not delete attempt, ledger, usage-event, or temporary replay-object rows/objects. | Stores status, request fingerprint/idempotency hashes, sanitized temporary save-reference metadata, and safe error codes only; no raw prompts, image bytes, provider secrets, or raw request bodies. Admin responses omit raw hashes/fingerprints/temp keys/save references. |
