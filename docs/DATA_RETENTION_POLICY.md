...
| Credit ledger/usage events | Phase 2-B adds idempotent credit grants, consumption, and usage events. | Retain for billing/cost/audit windows to be defined. | Retain/anonymize according to billing and abuse-review policy. | Helper/API foundation exists; lifecycle export/delete integration remains future work. |
...
Phase 1-H enforces request creation, planning, approval state, idempotency, route policy registration, admin-only access, fail-closed rate limiting, same-origin checks, and byte-limited bodies. Phase 1-I adds bounded export archive generation and authorized admin archive download for approved export plans. Phase 1-J adds bounded expired-archive R2 cleanup and a safe executor pilot for reversible auth-state cleanup. Phase 2-A adds organization/membership storage. Phase 2-B adds billing/entitlement/credit-ledger storage and admin credit-grant APIs. Org and billing data are not yet part of export/delete planning. The system still does not execute irreversible deletion or hard-delete user/media/audit records by default.
