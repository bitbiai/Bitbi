...
| Current update scope | Phase 3.5 member music cost decomposition/gateway prep only |
...
| Phase 3.5 Member music cost decomposition | Completed for design/check scope | Decompose member music into parent, optional lyrics, required audio, and generated cover/background provider-cost sub-operations; define target reservation/failure/replay semantics; keep the live route unmigrated. | `docs/ai-cost-gateway/MEMBER_MUSIC_COST_DECOMPOSITION.md`, `workers/auth/src/lib/ai-cost-operations.js`, `scripts/check-ai-cost-policy.mjs`, `scripts/test-ai-cost-policy.mjs`, `scripts/test-ai-cost-operations.mjs`, AI Cost Gateway docs, current-state docs. | Deterministic registry and policy tests cover music sub-operation entries, bundled metadata, report-only unmigrated status, member image pilot status, and no external calls. | Does not migrate music, require music idempotency, reserve music credits, change debits, add replay/cache, call providers, mutate billing, deploy, or prove production readiness. |
...
- Phase 3.5 decomposes member music and prepares the registry/check baseline only. It does not change live music route behavior, require music `Idempotency-Key`, reserve music credits, change credit debits, add provider replay/cache, call providers, mutate billing, or prove production/live billing readiness.
...
- Phase 3.4 provides the first runtime AI Cost Gateway pilot for member personal image generation, and Phase 3.5 documents/decomposes member music for a future migration, but member music/video, generated music covers, News Pulse visuals, most unmetered admin routes, and internal provider routes are not yet migrated behind one required-idempotency pre-provider reservation/replay AI Cost Gateway.
...
3. Phase 3.6 AI Cost Gateway route migration: migrate member music generation in a narrow PR, including parent reservation, lyrics/audio suppression, cover budget policy, replay, and failure/finalization safety without broad route rewrites.
