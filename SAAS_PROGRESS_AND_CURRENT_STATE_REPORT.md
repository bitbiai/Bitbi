...
| Current update scope | Phase 4.3 charged Admin BFL image-test budget enforcement hardening only |
...
1. Apply all auth migrations through `0048`.
...
- Apply auth migrations through `0048`.
...
1. Staging verification and deployment evidence phase: apply migrations through `0048` in staging, configure Testmode Stripe, configure live Stripe with `ENABLE_LIVE_STRIPE_CREDIT_PACKS=false` and `ENABLE_LIVE_STRIPE_SUBSCRIPTIONS=false` initially, enable canary flags only for bounded windows, verify all Phase 0 through BITBI Pro subscription/credit bucket flows plus the Phase 3.4 member personal image gateway pilot, run live health/header checks, and document results.
...
3. Phase 4.3 AI Cost Gateway continuation: verify member image/music/video evidence, preserve replay/result/cleanup invariants, and either migrate exactly one narrow admin/provider-cost flow or add a report-only budget evidence collector while keeping the Phase 3.9/4.1/4.2 known-gap guard current.
...
- Migrations `0034` through `0048` are additive/forward-only; do not attempt destructive rollback. Disable routes/UI instead if a problem occurs.
...
- Confirm latest auth migration is `0048_add_member_ai_usage_attempts.sql`.
