...
| Tenant assets | Folder/image ownership metadata exists for new personal writes only; legacy rows remain unresolved. Manual-review workflows exist. Admin Tenant Isolation Execution now exposes warning-gated Backfill dry-run/execution controls, Access-Switch shadow diagnostics, and Legacy Reset status/evidence controls. Post-cleanup evidence shows one exact safe `ai_images` ownership candidate prepared for operator execution; Access-Switch and Reset remain blocked. |
