...
| Current update scope | Phase 4.5 Admin async video job budget enforcement only |
...
1. Apply all auth migrations through `0049`.
...
- Apply auth migrations through `0049`.
...
1. Staging verification and deployment evidence phase: apply migrations through `0049` in staging, configure Testmode Stripe, configure live Stripe with `ENABLE_LIVE_STRIPE_CREDIT_PACKS=false` and `ENABLE_LIVE_STRIPE_SUBSCRIPTIONS=false` initially, enable canary flags only for bounded windows, verify all Phase 0 through BITBI Pro subscription/credit bucket flows plus the Phase 3.4 member personal image gateway pilot and Phase 4.5 admin video budget metadata/queue behavior, run live health/header checks, and document results.
...
3. Phase 4.6 AI Cost Gateway continuation: verify member image/music/video evidence, the Phase 4.4 budget evidence report, and Phase 4.5 admin video job budget metadata/queue behavior; preserve replay/result/cleanup invariants; and migrate exactly one narrow remaining admin/platform provider-cost flow, with OpenClaw/News Pulse visuals recommended next.
...
- Migrations `0034` through `0049` are additive/forward-only; do not attempt destructive rollback. Disable routes/UI instead if a problem occurs.
...
- Confirm latest auth migration is `0049_add_admin_video_job_budget_metadata.sql`.
