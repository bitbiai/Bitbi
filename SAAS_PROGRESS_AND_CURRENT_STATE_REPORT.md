...
Scope: documentation/audit reconciliation for the BITBI SaaS hardening roadmap. This report reconciles the current repository state after Phase 0 through Phase 3.3 AI Cost Operation Registry/report-only baseline. It is not a legal compliance certification, production deploy approval, live billing approval, AI cost readiness claim, or full tenant-isolation claim.
...
| Current update scope | Phase 3.3 AI Cost Operation Registry/report-only baseline only |
...
| Phase 3.2 AI Cost Gateway contract and tests | Completed for current scope | Add unused operation config normalization, request fingerprinting, scoped idempotency key helpers, pure plan/state classification, and deterministic test harness. | `workers/auth/src/lib/ai-cost-gateway.js`, `scripts/test-ai-cost-gateway.mjs`, package/preflight script updates. | `test:ai-cost-gateway` covers valid/invalid configs, deterministic fingerprints, volatile exclusions, prompt hashing, scoped idempotency, plan states, and no external calls. | Does not import the gateway into live routes, change existing AI route behavior, require Idempotency-Key on existing routes, reserve/debit credits, suppress provider calls, add replay/cache, call providers, or prove production readiness. |
| Phase 3.3 AI Cost Operation Registry | Completed for current scope | Add unused central operation registry, current enforcement metadata, route-policy/source-file baselines, registry summary helper, and report-only check/test coverage for 30 known AI cost operations. | `workers/auth/src/lib/ai-cost-operations.js`, `scripts/test-ai-cost-operations.mjs`, `scripts/check-ai-cost-policy.mjs`, package/preflight script updates. | `test:ai-cost-operations` validates unique ids, normalizer compatibility, high-risk coverage, deterministic summary counts, duplicate rejection, and no external calls. `check:ai-cost-policy` validates the registry and remains report-only by default. | Does not import the registry into live routes, change existing AI route behavior, require Idempotency-Key on existing routes, reserve/debit credits, suppress provider calls, add replay/cache, call providers, or prove production readiness. |
...
- Phase 3.2 adds unused AI Cost Gateway contract helpers and deterministic tests. It does not migrate routes, call providers, mutate billing, change existing Idempotency-Key requirements, reserve/debit credits, suppress provider calls, add replay/cache to live routes, or prove production/live billing readiness.
- Phase 3.3 adds an unused AI Cost Operation Registry and report-only baseline tests for 30 known AI provider-cost/current-policy operations. It does not import the registry into live routes, migrate routes, call providers, mutate billing, change existing Idempotency-Key requirements, reserve/debit credits, suppress provider calls, add replay/cache to live routes, or prove production/live billing readiness.
...
- Phase 3.2 provides an unused AI Cost Gateway contract foundation and Phase 3.3 provides an unused operation registry/report-only baseline, but member image personal mode, member music/video, generated music covers, News Pulse visuals, most unmetered admin routes, and internal provider routes are not yet migrated behind one required-idempotency pre-provider reservation/replay AI Cost Gateway.
...
3. Phase 3.4 AI Cost Gateway route migration: migrate exactly one low-risk provider-cost route, preferably member personal image generation, without broad route rewrites.
