...
| Current update scope | Phase 3.1 AI Cost Gateway design, route inventory, roadmap, and report-only policy-gap check only |
...
- Phase 3.1 documents the AI Cost Gateway target and route gaps, but member image personal mode, member music/video, generated music covers, News Pulse visuals, most unmetered admin routes, and internal provider routes are not yet unified behind one required-idempotency pre-provider reservation/replay AI Cost Gateway.
...
3. Phase 3.2 AI Cost Gateway contract/tests: add the gateway API/config/test harness without migrating routes or changing runtime charging behavior, then migrate one low-risk route in a separate phase.
