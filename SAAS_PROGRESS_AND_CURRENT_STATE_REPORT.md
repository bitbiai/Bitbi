...
| Current update scope | Phase 3.4 member personal image AI Cost Gateway pilot only |
...
| Phase 3.3 AI Cost Operation Registry | Completed for current scope | Add central operation registry, current enforcement metadata, route-policy/source-file baselines, registry summary helper, and report-only check/test coverage for 30 known AI cost operations. | `workers/auth/src/lib/ai-cost-operations.js`, `scripts/test-ai-cost-operations.mjs`, `scripts/check-ai-cost-policy.mjs`, package/preflight script updates. | `test:ai-cost-operations` validates unique ids, normalizer compatibility, high-risk coverage, deterministic summary counts, duplicate rejection, and no external calls. `check:ai-cost-policy` validates the registry and remains report-only by default. | Used by Phase 3.4 only for the `member.image.generate` operation. It does not migrate music/video/admin/platform/internal routes, call providers, or prove production readiness. |
...
- Phase 3.3 adds an AI Cost Operation Registry and report-only baseline tests for 30 known AI provider-cost/current-policy operations.
...
3. Phase 3.5 AI Cost Gateway route migration: migrate member music generation in a narrow PR, including its lyrics/music/cover sub-operation policy, without broad route rewrites.
