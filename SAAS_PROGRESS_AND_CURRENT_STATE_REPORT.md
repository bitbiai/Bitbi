...
| Current update scope | Phase 4.6 OpenClaw/News Pulse visual budget controls only |
...
- Phase 3.6 migrates only member music generation. Phase 3.7 hardens only the already migrated member image and member music gateway flows for replay-unavailable handling, safe replay metadata, music cover status writeback, terminal finalization behavior, and scheduled member-attempt cleanup. Phase 3.8 migrates only member video generation to the same member gateway foundation. Phase 4.5 covers only admin async video jobs, and Phase 4.6 covers only OpenClaw/News Pulse visual budget metadata/status controls. These phases do not migrate broad admin AI, Admin music/text/compare/live-agent, platform/background AI outside News Pulse visuals, or global internal AI Worker routes; they do not call real providers in tests, call Stripe, deploy, run remote migrations, or prove production/live billing readiness.
...
- Phase 3.4 provides runtime AI Cost Gateway coverage for member personal image generation, Phase 3.6 adds member music coverage with parent reservation/replay suppression, Phase 3.7 hardens replay-unavailable, cover-status, finalization, and cleanup behavior for those two migrated flows, Phase 3.8 adds member video coverage with parent reservation and durable saved-asset replay metadata, Phase 3.9 adds a default local guard/baseline so new provider-cost drift or member gateway regressions fail validation, Phase 4.1 defines target budget scopes for remaining admin/platform/internal/OpenClaw gaps, Phase 4.2 adds pure helper contracts/tests for future admin/platform budget enforcement, Phase 4.3 covers only charged Admin BFL image-test budget metadata, Phase 4.4 reports read-only evidence, Phase 4.5 covers only admin async video jobs with job/queue budget metadata and duplicate provider-task suppression, and Phase 4.6 covers only OpenClaw/News Pulse visuals with visual budget metadata/status controls. Broad Admin AI, Admin music/text/compare/live-agent, platform/background routes outside News Pulse visuals, and global internal provider routes remain explicit known gaps and are not yet migrated behind one required-idempotency pre-provider reservation/replay AI Cost Gateway.
...
3. Phase 4.7 AI Cost Gateway continuation: verify member image/music/video evidence, the Phase 4.4 budget evidence report, Phase 4.5 admin video job budget metadata/queue behavior, and Phase 4.6 News Pulse visual budget metadata/status behavior; preserve replay/result/cleanup invariants; and add internal AI Worker caller-policy guards without migrating broad admin/platform routes.
