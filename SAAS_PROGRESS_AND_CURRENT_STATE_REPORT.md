...
| Current update scope | Phase 3.8 member video AI Cost Gateway migration only |
...
| Phase 3.3 AI Cost Operation Registry | Completed for current scope | Add central operation registry, current enforcement metadata, route-policy/source-file baselines, registry summary helper, and report-only check/test coverage for known AI cost operations. | `workers/auth/src/lib/ai-cost-operations.js`, `scripts/test-ai-cost-operations.mjs`, `scripts/check-ai-cost-policy.mjs`, package/preflight script updates. | `test:ai-cost-operations` validates unique ids, normalizer compatibility, high-risk coverage, deterministic summary counts, duplicate rejection, and no external calls. `check:ai-cost-policy` validates the registry and remains report-only by default. | Used by member image/music/video gateway operations. It does not migrate admin/platform/internal routes, call providers, or prove production readiness. |
...
- Phase 3.4 provides runtime AI Cost Gateway coverage for member personal image generation, Phase 3.6 adds member music coverage with parent reservation/replay suppression, Phase 3.7 hardens replay-unavailable, cover-status, finalization, and cleanup behavior for those two migrated flows, and Phase 3.8 adds member video coverage with parent reservation and durable saved-asset replay metadata. News Pulse visuals, most unmetered admin routes, platform/background routes, admin video jobs, and internal provider routes are not yet migrated behind one required-idempotency pre-provider reservation/replay AI Cost Gateway.
...
3. Phase 3.9 AI Cost Gateway continuation: verify member image/music/video evidence, preserve replay/result/cleanup invariants, and add admin/platform AI cost telemetry plus a route-metadata guard without migrating unrelated runtime routes.
