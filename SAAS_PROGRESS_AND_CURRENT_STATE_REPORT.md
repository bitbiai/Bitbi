...
| Current update scope | Phase 3.6 member music AI Cost Gateway migration only |
...
- Phase 3.6 migrates only member music generation. It does not migrate member video, admin AI, platform/background AI, OpenClaw/News Pulse, or internal AI Worker routes; it does not call real providers in tests, call Stripe, deploy, run remote migrations, or prove production/live billing readiness.
...
- Personal/no-org image generation remains backward-compatible with the member credit balance path and charges after successful provider execution, but it is not yet covered by a pre-provider reservation/replay gateway.
...
- Member music/video routes check and debit member credits after successful provider execution, but they do not yet require idempotency, reserve credits before provider calls, suppress duplicate provider execution, or replay successful results through a unified gateway.
- Full paid model/catalog enforcement across every AI route is not complete. Phase 3.1 documents the target AI Cost Gateway and route gaps only.
...
- Phase 3.4 provides runtime AI Cost Gateway coverage for member personal image generation, and Phase 3.6 adds member music coverage with parent reservation/replay suppression. Member video, News Pulse visuals, most unmetered admin routes, platform/background routes, and internal provider routes are not yet migrated behind one required-idempotency pre-provider reservation/replay AI Cost Gateway.
...
3. Phase 3.7 AI Cost Gateway hardening: verify member image/music evidence, harden replay/result metadata and cover status writeback, then migrate exactly one remaining high-risk route such as member video in a narrow PR.
