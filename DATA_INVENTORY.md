...
- Organization, membership, billing, entitlement, credit ledger, usage-event, AI usage-attempt, billing-provider event, and Stripe Testmode checkout-session export/delete behavior is not yet integrated into lifecycle plans; Phase 2-A/2-B/2-C/2-D/2-E/2-F/2-H/2-I/2-J and Pricing only add schema, helpers, minimal APIs, org-scoped image/text generation debits, retry-safe attempt tracking, bounded attempt cleanup/inspection, bounded temporary replay cleanup, provider-neutral billing-event ingestion/inspection, Stripe Testmode checkout/session tracking, exact-once Testmode credit grants, and admin-only pricing UX.
...
- How Stripe Testmode checkout session records and future live checkout records should be represented in export/deletion/anonymization plans after legal/product review.
