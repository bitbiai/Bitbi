...
| Pricing page (`/pricing.html`, `js/pages/pricing/main.js`) | Free tier copy, product-facing `credits_5000`/`credits_10000` pack metadata, current admin's eligible organizations, org billing summary, checkout success/cancel state. | No new page-specific storage. Checkout creation writes to `billing_checkout_sessions`; verified webhooks write `billing_provider_events`, `billing_event_actions`, `credit_ledger`, and `usage_events` only through existing backend flows. | Visible only to authenticated admins during controlled rollout. Does not expose Stripe secrets, raw provider payloads, webhook signatures, ledger internals, or unsupported model availability. | Product/legal must approve public pricing copy, final pack amounts/currency, live billing terms, and whether pricing should be public. |
| Credits dashboard (`/account/credits.html`, `js/pages/credits/main.js`) | Organization balance summary, live pack catalog, checkout enabled/config status, recent live purchases, and recent ledger activity for platform admins or active organization owners. | No separate page-specific storage. Live checkout writes to `billing_checkout_sessions`; verified live webhooks write `billing_provider_events`, `billing_event_actions`, and `credit_ledger` through existing backend flows. | Hidden from logged-out users, normal users, org admins, members, and viewers. It does not expose Stripe secrets, raw provider payloads, webhook signatures, payment method data, raw idempotency hashes, raw request fingerprints, or SQL/debug metadata. Platform admins may see missing live config variable names only. | Legal/product must approve public billing terms, refund/chargeback operations, invoice/receipt policy, and broader user-facing rollout before expanding beyond this gated live canary surface. |
