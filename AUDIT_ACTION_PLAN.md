...
| --- | --- | --- | --- | --- |
| Merge readiness | Pass after Phase 2-B validation | Phase 2-B adds migration `0035_add_billing_entitlements.sql`, billing/entitlement helper, org/admin billing APIs, route-policy/release updates, and Worker tests. `npm run test:workers` passed 320/320, `npm run test:static` passed 155/155, `npm run release:preflight` passed, and `git diff --check` passed after the ledger ordering fix. Full final validation is recorded in `PHASE2B_BILLING_ENTITLEMENTS_REPORT.md` and the final response for this pass. | Future changes must rerun the relevant checks. All Phase 2-B code, migration, release, tests, and documentation files must be committed together. Production deploy still requires migrations through `0035`, live Cloudflare verification, and staging org billing/entitlement/credit verification. | Run Staff Security/Staff Engineer review for Phase 2-B, then commit all Phase 2-B files together if review passes. |
...
| Phase 0-A through 2-B security/operations posture | Reduced immediate risk | HMAC service auth, nonce replay protection, fail-closed limiters, body-size limits, durable MFA failed-attempt state, config validation, async admin video jobs, queue-safe provider task create/poll, R2 output ingest, poison-message persistence, default-disabled sync video debug route, admin poison/failed-job inspection APIs, quality-gate scripts/tests, purpose-specific auth secrets, high-risk auth-worker route policy registry/checks, public-safe AI/contact health probes, operational readiness checks, SLO/event docs, backup/restore plan, incident runbooks, signed activity cursors, indexed/redacted activity search projection, data inventory, retention baseline, admin lifecycle planning APIs, bounded private export archive generation, bounded expired archive cleanup, safe lifecycle executor pilot, additive organization/membership/RBAC foundation, and billing/entitlement/credit-ledger foundation are present. | This is not full SaaS maturity. Legacy `SESSION_SECRET` fallback remains during migration, MFA ciphertexts lack key IDs/lazy re-encryption, route policy is metadata/checking rather than full centralized enforcement, live checks skip by default in CI, dashboard controls/alerts are not repo-enforced, restore drills are not executed, historical activity projection backfill is not run, user self-service privacy flow and irreversible deletion execution remain deferred, existing assets remain user-owned, no live payment provider exists, AI routes are not yet wired to credit enforcement, and compliance/load-testing work remains open. | Complete Cloudflare staging verification, live health/header checks with `--require-live`, staging restore drill, alert/drift automation, archive cleanup/safe executor staging verification, Phase 2-A org-flow staging verification, Phase 2-B billing/entitlement staging verification, self-service privacy policy, tenant migration plan, and broader SaaS platform gaps. |

Phase 2-B is implemented as an additive billing/entitlement foundation. Future changes must keep related code, tests, release-policy, and documentation files together to avoid stale validation or route-policy drift.
...
| 16 | Add billing, plans, entitlements, and quota enforcement | Partially addressed | Phase 2-B adds migration `0035_add_billing_entitlements.sql`, seeded free plan/entitlements, `workers/auth/src/lib/billing.js`, user org entitlement/billing/usage reads, admin plan/org billing inspection, admin manual credit grants, route-policy/release updates, and Worker tests for membership checks, admin credit grants, idempotent credit grants/consumption, insufficient-credit denial, no negative balances, CSRF, body-size, limiter `429`, and limiter fail-closed `503`. | This is a foundation only. No live payment provider, checkout, invoices, webhooks, monthly grant scheduler, production billing activation, or broad AI route credit enforcement exists. Existing assets are not fully tenant-owned. | Stage and verify migration `0035`, then Phase 2-C should wire one low-risk AI route family to entitlement/credit checks or design payment-provider integration with webhook idempotency. |
...
22. Decide whether historical audit/activity records need a controlled projection backfill and whether user-facing self-service privacy flows are required before production launch.
23. Execute and record a staging D1/R2 restore drill.
24. Decide user self-service privacy flow and historical R2 owner-map/backfill policy.
25. Choose the next Phase 2 track: wire low-risk AI route families to billing entitlements/credits, design payment-provider integration, or continue domain-by-domain tenant ownership migration. Do not combine all three into one large rewrite.

## Next Phase: Phase 2-C Or Tenant Migration Track

Phase 2-B has implemented the billing/plans/entitlements/credit ledger foundation for the current scope. The next phase should be chosen deliberately:

- Option A: wire one low-risk AI route family to organization entitlements and credits while preserving the free-plan compatibility path.
...
Do not redo completed Phase 0, Phase 1, Phase 2-A, or Phase 2-B work. Treat the following as baseline capabilities unless a focused regression is found:
...
- Phase 2-B changes are committed together.
...
The latest Phase 2-B validation evidence is recorded in `PHASE2B_BILLING_ENTITLEMENTS_REPORT.md`. Relevant results from this update:
...
| `npm run check:route-policies` | PASS | 112 registered high-risk auth-worker route policies validate, including Phase 2-A org routes and Phase 2-B billing routes, and mutating dispatch markers are covered. |
...
| `npm run check:js` | PASS | Targeted syntax checks pass for 35 high-risk/new JS modules and scripts. |
| `npm run check:worker-body-parsers` | PASS | Worker body parser guard remains green. |
| `npm run test:workers` | PASS, 320/320 | Worker route/security regressions pass, including Phase 2-A org create/list/detail, Phase 2-B org billing/entitlement reads, admin credit grants, idempotency, CSRF, body-size rejection, limiter `429`, fail-closed limiter `503`, and credit consume helper behavior. |
| `npm run test:static` | PASS, 155/155 | Static/admin UI suite remains green after the admin activity search placeholder update. |
| `npm run test:release-compat` | PASS | Release compatibility contract includes migration `0035_add_billing_entitlements.sql` and the new billing route contracts. |
...
| `npm run release:preflight` | PASS | Aggregated preflight passed after Phase 2-B changes and the ledger ordering fix, including operational readiness checks, skipped-live checks, release compatibility, Cloudflare prereq repo validation, body-parser guard, admin activity query-shape guard, data lifecycle guard, Worker tests, and release plan. |
| `git diff --check` | PASS | No whitespace errors in the final diff. |

Checks not performed in the Phase 2-A implementation pass:
