...
| --- | --- | --- | --- | --- |
| Merge readiness | Pending final pre-merge review after validation | Phase 2-J and the Admin Control Plane are committed. The Pricing / Credit Purchase rollout adds an admin-only `/pricing.html` static page, admin-only desktop/mobile header link, product-facing `credits_5000` and `credits_10000` Stripe Testmode packs, checkout initiation through the existing org owner/admin route, success/cancel return states, and truthful Models overlay statuses. It adds no migration, dependency, live Stripe activation, subscription, invoice, customer portal, or production webhook trust. | Production deploy still requires migrations through `0038`, Stripe Testmode secrets/URLs/webhook endpoint, live Cloudflare verification, staging checkout/webhook/credit-grant verification, and staging verification for org, billing, AI usage, lifecycle, billing-event, control-plane UI, and pricing flows. | Run focused product/security/frontend review of the Pricing / Credit Purchase diff, then commit the page/header/catalog/test/report set together if validation is green. |
