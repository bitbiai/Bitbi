...
Last updated: 2026-05-15 after Phase 0-A, Phase 0-A+, Phase 0-B, Phase 1-A, Phase 1-B, Phase 1-C, Phase 1-D purpose-specific secret hardening, Phase 1-E route policy registry hardening, Phase 1-F operational readiness foundations, Phase 1-G audit/activity search scalability hardening, Phase 1-H data lifecycle foundation work, Phase 1-I export archive/delete-executor foundation work, Phase 1-J retention/executor pilot work, Phase 2-A organization/RBAC foundation work, Phase 2-B billing/entitlement foundation work, Phase 2-C org-scoped AI image usage enforcement, Phase 2-D org-scoped AI usage reservation hardening, Phase 2-E AI usage attempt cleanup/admin inspection, Phase 2-F AI replay object retention cleanup, Phase 2-G text-route inspection/no-op, Phase 2-H org-scoped member text generation API work, Phase 2-I billing event ingestion foundation work, Phase 2-J Stripe Testmode credit-pack checkout foundation work, Phase 2-K Admin-only Stripe Testmode Lockdown, Phase 2-L Live Stripe Credit Packs and Credits Dashboard, Phase 2-M Admin-only BFL image-test credit charging/live pack economics, Phase 2-N organization context and credit-debit visibility, the Admin Control Plane UI phase and corrective polish/data-accuracy pass, the admin-only Pricing / Credit Purchase Testmode rollout, the pricing asset-reference fix, the Stripe Testmode missing-config diagnostics fix, BITBI Pro member subscription/credit bucket foundations, and Alpha Audit documentation-currentness guardrails.
...
| --- | --- | --- | --- | --- |
| Merge readiness | Ready for review only after current Phase 0 validation | Phase 0 Alpha Audit hardening reconciles current-state docs to the release contract latest auth migration `0047_add_member_subscriptions_and_credit_buckets.sql`, adds an audit index/report, and adds a doc-currentness guard. | Production deploy still requires migrations through `0047`, Stripe Testmode/live credit-pack/BITBI Pro subscription config and webhook verification, live Cloudflare verification, checkout/webhook/credit-grant verification, organization-context verification, and verification for org, billing, AI usage, Admin AI Lab, lifecycle, billing-event, control-plane UI, pricing, Credits, Organization, and BITBI Pro flows. | Review the documentation/currentness diff and keep all historical audit reports preserved. |
| Production deploy readiness | Blocked | Repo config declares required auth/AI secrets, optional synthetic `BILLING_WEBHOOK_TEST_SECRET`, optional Stripe Testmode config, optional operator-controlled `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT`, optional live Stripe credit-pack config with `ENABLE_LIVE_STRIPE_CREDIT_PACKS`, optional BITBI Pro config with `ENABLE_LIVE_STRIPE_SUBSCRIPTIONS`, `SERVICE_AUTH_REPLAY`, auth migrations through `0047`, AI/video queues, `USER_IMAGES`, `AUDIT_ARCHIVE`, and `AI_LAB`. `ALLOW_SYNC_VIDEO_DEBUG` must remain absent/false unless a controlled emergency debug rollback is approved. | Live Cloudflare secrets/bindings/queues/R2/D1 migrations, Stripe Testmode endpoint/secrets, live Stripe endpoint/secrets, and BITBI Pro subscription webhook behavior were not verified by this implementation pass. Missing migration `0047` breaks BITBI Pro subscription state and separated member credit buckets; missing migration `0046` breaks user asset storage quota accounting; missing migration `0040` breaks persisted live checkout authorization/payment status tracking; missing earlier billing/attempt migrations break the flows that depend on them. Organization and billing tables do not mean full tenant isolation or full production billing readiness. | Provision/verify all Phase 0/1/2/BITBI Pro secrets/resources, apply migrations through `0047`, verify organization, billing/entitlement/credit grant, image/text/member usage, usage cleanup, replay cleanup, synthetic billing webhook, Stripe Testmode platform-admin-only checkout/webhook flows, live credit-pack flows, BITBI Pro subscription checkout/invoice/cancel/reactivate flows, and Pricing return states. Keep live enable flags absent/false except during bounded canary windows. |
| Phase 0-A through BITBI Pro security/operations posture | Reduced immediate risk | HMAC service auth, nonce replay protection, fail-closed limiters, body-size limits, durable MFA failed-attempt state, config validation, async admin video jobs, route-policy checks, operational readiness docs/checks, signed activity cursors, indexed/redacted activity search, data lifecycle foundations, organization/RBAC foundation, billing/entitlement/credit-ledger foundation, org-scoped image/text credit enforcement with usage attempts/replay/cleanup, provider-neutral billing event ingestion, Stripe Testmode checkout foundation, server-side platform-admin-only Testmode checkout lockdown, narrow live Stripe one-time credit-pack checkout, member credit buckets, guarded BITBI Pro subscription scaffolding, admin-only BFL image-test charging, and frontend Admin Control Plane/Credits surfaces are present. | This is not full SaaS maturity. Legacy `SESSION_SECRET` fallback remains during migration, route policy is metadata/checking rather than full centralized enforcement, live checks skip by default in CI, Cloudflare dashboard controls/alerts are not editable from the admin UI, restore drills are not executed, user self-service privacy flow and irreversible deletion execution remain deferred, existing assets remain user-owned, invoices/customer portal/tax/refund/chargeback/failed-payment/reconciliation workflows do not exist, member AI cost routing is not unified behind required reservations/replay, and compliance/load-testing work remains open. | Complete Cloudflare staging verification or a bounded operator-run canary, live health/header checks with `--require-live`, restore drill evidence, alert/drift automation, Phase 2-A through BITBI Pro verification, self-service privacy policy, refund/chargeback/provider hardening, tenant migration plan, and broader SaaS platform gaps. |

Phase 2-J is implemented as a Stripe Testmode credit-pack checkout foundation. Phase 2-K makes that checkout route server-side platform-admin-only and disabled unless `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true`. Phase 2-L adds separate live checkout routes for fixed live packs. Phase 2-M changes the live pack catalog to `live_credits_5000` / `live_credits_12000` and adds platform-admin-only organization-credit charging for supported BFL Admin AI image tests. Phase 2-N adds `/account/organization.html`, `GET /api/orgs/:id/organization-dashboard`, and shared active organization selection so Credits and Admin AI Lab use the same organization context. Phase 2-O updates `/pricing.html` to present Free, 5,000-credit, 12,000-credit, and later BITBI Pro positioning without visible Testmode positioning; paid Pricing CTAs route through backend-authoritative account billing routes. The later BITBI Pro work adds migration `0047` for subscription state and member credit buckets. Future changes must keep checkout creation, Stripe raw-body signature verification, billing event storage, admin/owner/member checkout grant requirements, subscription invoice top-up behavior, exactly-once credit grant behavior, admin image-test usage attempt/debit behavior, organization-context selection, tests, route policy metadata, release compatibility, pricing/credits/organization copy, and documentation files together to avoid stale validation or route-policy drift.
...
| 16 | Add billing, plans, entitlements, and quota enforcement | Reduced | Phase 2-B adds migration `0035_add_billing_entitlements.sql`, seeded free plan/entitlements, `workers/auth/src/lib/billing.js`, user org entitlement/billing/usage reads, admin plan/org billing inspection, admin manual credit grants, route-policy/release updates, and Worker tests for membership checks, admin credit grants, idempotent credit grants/consumption, insufficient-credit denial, no negative balances, CSRF, body-size, limiter `429`, and limiter fail-closed `503`. Phase 2-C wires `/api/ai/generate-image` to opt-in org-scoped entitlement and credit checks. Phase 2-D adds migration `0036_add_ai_usage_attempts.sql` and `workers/auth/src/lib/ai-usage-attempts.js` so org-scoped image requests reserve credits, suppress duplicate provider execution, replay temporary successful results, release reservations on provider failure, and fail safely on billing finalization failure. Phase 2-E/F add cleanup/admin inspection and temporary replay object cleanup. Phase 2-H adds org-scoped backend-only `/api/ai/generate-text` with `ai.text.generate` entitlement, credit reservation/finalization, same-key text replay, provider-failure no-charge behavior, and no admin/text-asset charging. Phase 2-I adds migration `0037_add_billing_event_ingestion.sql`, synthetic raw-body billing webhook verification, idempotent billing event storage, duplicate/mismatch handling, dry-run action planning, and sanitized admin billing event inspection. Phase 2-J adds migration `0038_add_stripe_credit_pack_checkout.sql`, Stripe Testmode credit-pack checkout tracking, Stripe raw-body signature verification, and exactly-once Testmode checkout credit grants. Phase 2-K makes Testmode checkout creation server-side platform-admin-only, requires `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true`, and prevents Testmode webhook credit grants unless the persisted checkout row was created by an active platform admin. The Pricing rollout adds migration `0039_raise_credit_balance_cap_for_pricing_packs.sql` and admin-only `/pricing.html` for 5000/10000 Testmode credit packs. Phase 2-L adds migration `0040_add_live_stripe_credit_pack_scope.sql`, live fixed credit packs, live checkout for platform admins and active org owners, live webhook verification, exactly-once live grants, and `/account/credits.html`. Phase 2-M updates live pack economics to 5,000 credits for 9.99 EUR and 12,000 credits for 19.99 EUR, blocks new `live_credits_10000` live checkout creation, and charges platform-admin-only BFL Admin AI image tests against selected organization credits using server-side cost calculation and usage-attempt idempotency. Phase 2-N adds `/account/organization.html`, shared active organization selection, `GET /api/orgs/:id/organization-dashboard`, and safe Admin AI Lab debit diagnostics so the selected credit-owning organization is visible and shared across Organization, Credits, and Admin AI Lab. Later member billing work adds migrations `0041` and `0042` for member credit ledger/live checkout, `0046` for asset storage quota, and `0047` for BITBI Pro subscription rows plus member credit buckets/events. | This is still not full SaaS billing. Production-ready subscriptions, invoices, customer portal, taxes, refund/chargeback automation, billing reconciliation/admin needs-review workflow, broad AI route credit enforcement beyond explicitly wired member image/text and admin BFL test paths, video credit enforcement, public production billing evidence, and full tenant-owned asset model remain open. Existing assets are not fully tenant-owned. | Verify migrations `0035` through `0047`; verify org-scoped image/text charge and retry behavior, synthetic billing webhook verification, platform-admin-only Stripe Testmode checkout creation, kill-switch failure, Stripe webhook signature verification, duplicate/mismatch handling, exactly-once credit grant behavior, non-admin-created checkout no-credit behavior, admin-only Pricing success/cancel flows, Phase 2-L live checkout/webhook/Credits dashboard behavior, Phase 2-M Admin AI Lab BFL credit charging/no-charge/idempotency behavior, Phase 2-N Organization page/context-sharing/debit visibility, and BITBI Pro subscription checkout/invoice/cancel/reactivate/bucket behavior; then choose refund/chargeback/reconciliation policy, AI Cost Gateway, or video route credit enforcement. |
...
7. Apply auth migrations through `0047` before deploying auth Worker code that depends on them.
...
33. Verify Phase 2-M Admin AI Lab BFL image-test credit charging in staging or a bounded operator canary, including org selector, Flux 1 schnell 1-credit default, Flux 2 klein 9B 10-credit default, missing org/idempotency denial, insufficient-credit denial before provider call, provider-failure no-charge behavior, billing-finalization safe failure, duplicate idempotency no-double-debit behavior, and Credits dashboard/ledger visibility.
...
35. Choose the next Phase 2 track: refund/chargeback/manual-review billing policy, video AI entitlement wiring, or domain-by-domain tenant ownership migration. Do not combine all tracks into one large rewrite.

## Next Phase: Billing Operations, Video AI Route, Or Tenant Migration Track

Phase 2-J has implemented Stripe Testmode credit-pack checkout and verified Testmode webhook credit grants, Phase 2-K has locked Testmode checkout creation to platform admins with an explicit operator kill switch, Phase 2-L has added narrow live one-time credit packs plus a gated Credits dashboard for platform admins and active org owners, and Phase 2-M has updated live pack economics plus admin-only BFL image-test credit charging. The next phase should be chosen deliberately:

- Option A: harden billing operations around refunds/chargebacks/manual review, support runbooks, receipts/tax/legal terms, and live canary evidence while keeping public billing disabled.
...
- Phase 2-M live pack economics update and platform-admin-only BFL Admin AI image-test credit charging with server-side model cost calculation, organization credit source, idempotency, provider-failure no-charge behavior, and sanitized ledger metadata.
...
- Latest auth migration number is documented as `0047_add_member_subscriptions_and_credit_buckets.sql`.
...
- Auth D1 migration `0047_add_member_subscriptions_and_credit_buckets.sql` is applied before enabling BITBI Pro subscription checkout, subscription invoice top-ups, member credit bucket separation, or Pro storage quota behavior.
...
- Staging or bounded operator canary verifies Phase 2-M Admin AI Lab BFL image-test charging against a real organization credit balance, including no public/member route exposure and no member-facing image pricing change.
...
| `npm run test:release-compat` | Pending final rerun | Release compatibility should validate migration `0047` after this phase. |
...
- Remote D1 migrations, including migrations through `0047`.
