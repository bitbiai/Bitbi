...
| --- | --- | --- | --- | --- |
| Merge readiness | Pending Phase 2-L full validation | Phase 2-L adds narrow live Stripe one-time credit packs and a gated Credits dashboard, while preserving Phase 2-K Testmode lockdown. Baseline preflight, `check:js`, route-policy check, Worker 352/352, and focused Credits static tests passed during implementation. | Full requested validation suite still must pass before merge. Production deploy still requires migrations through `0040`, Stripe Testmode and live Stripe config/webhook verification, live Cloudflare verification, checkout/webhook/credit-grant verification, and verification for org, billing, AI usage, lifecycle, billing-event, control-plane UI, pricing, and Credits flows. | Run full validation, review Phase 2-L diff, then commit Phase 2-L runtime/docs/tests together after approval. |
...
| Phase 0-A through 2-L security/operations posture | Reduced immediate risk | HMAC service auth, nonce replay protection, fail-closed limiters, body-size limits, durable MFA failed-attempt state, config validation, async admin video jobs, route-policy checks, operational readiness docs/checks, signed activity cursors, indexed/redacted activity search, data lifecycle foundations, organization/RBAC foundation, billing/entitlement/credit-ledger foundation, org-scoped image/text credit enforcement with usage attempts/replay/cleanup, provider-neutral billing event ingestion, Stripe Testmode checkout foundation, server-side platform-admin-only Testmode checkout lockdown, narrow live Stripe one-time credit-pack checkout for platform admins/org owners, and frontend Admin Control Plane/Credits surfaces are present. | This is not full SaaS maturity. Legacy `SESSION_SECRET` fallback remains during migration, route policy is metadata/checking rather than full centralized enforcement, live checks skip by default in CI, Cloudflare dashboard controls/alerts are not editable from the admin UI, restore drills are not executed, user self-service privacy flow and irreversible deletion execution remain deferred, existing assets remain user-owned, public billing/subscriptions/invoices/customer portal/refund automation do not exist, video AI is not credit-enforced, and compliance/load-testing work remains open. | Complete Cloudflare staging verification or a bounded operator-run canary, live health/header checks with `--require-live`, restore drill evidence, alert/drift automation, Phase 2-A through 2-L and Admin Control Plane verification, self-service privacy policy, refund/chargeback/provider hardening, tenant migration plan, and broader SaaS platform gaps. |

Phase 2-J is implemented as a Stripe Testmode credit-pack checkout foundation. Phase 2-K makes that checkout route server-side platform-admin-only and disabled unless `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true`. Phase 2-L adds a separate live checkout route for fixed live packs and allows only platform admins or active organization owners. The Pricing / Credit Purchase rollout exposes Testmode only to admins through `/pricing.html`; the new Credits dashboard exposes live packs only through `/account/credits.html` for eligible admins/owners. Future changes must keep checkout creation, Stripe raw-body signature verification, billing event storage, admin/owner checkout grant requirements, exactly-once credit grant behavior, tests, route policy metadata, release compatibility, pricing/credits copy, and documentation files together to avoid stale validation or route-policy drift.
...
Phase 2-J has implemented Stripe Testmode credit-pack checkout and verified Testmode webhook credit grants, Phase 2-K has locked Testmode checkout creation to platform admins with an explicit operator kill switch, and Phase 2-L has added narrow live one-time credit packs plus a gated Credits dashboard for platform admins and active org owners. The next phase should be chosen deliberately:
...
Do not redo completed Phase 0, Phase 1, Phase 2-A, Phase 2-B, Phase 2-C, Phase 2-D, Phase 2-E, Phase 2-F, Phase 2-G, Phase 2-H, Phase 2-I, Phase 2-J, Phase 2-K, Phase 2-L, or Admin Control Plane work. Treat the following as baseline capabilities unless a focused regression is found:
...
| `npm run check:route-policies` | PASS | Route-policy metadata covers Phase 2-L live checkout, live webhook, Credits dashboard, Phase 2-J/K Testmode checkout/webhook, and prior protected routes. |
...
| `npm run test:workers` | PASS, 352/352 | Full Worker route/security regression suite passed after Phase 2-L changes, including live checkout/webhook/Credits dashboard cases and existing billing/AI compatibility. |
