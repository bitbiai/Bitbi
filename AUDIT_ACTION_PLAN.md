...
| --- | --- | --- | --- | --- |
| Merge readiness | Ready for review | Phase 2-N adds a gated Organization page, shared active organization selection/defaulting, owner/platform-admin role visibility, and safe Admin AI Lab credit-debit diagnostics while preserving Phase 2-K Testmode lockdown, Phase 2-L live checkout/Credits behavior, and Phase 2-M BFL admin image-test charging. Validation results are recorded in the Phase 2-N report/final response. | Production deploy still requires migrations through `0040`, Stripe Testmode and live Stripe config/webhook verification, live Cloudflare verification, checkout/webhook/credit-grant verification, organization-context verification, and verification for org, billing, AI usage, Admin AI Lab, lifecycle, billing-event, control-plane UI, pricing, Credits, and Organization flows. | Review Phase 2-N diff, then commit Phase 2-N runtime/frontend/docs/tests together after approval. |
...
31. Verify the admin-only Pricing page in staging, including header visibility, direct access denial for non-admins, org selector behavior, checkout success/cancel return states, model-status truthfulness, and no live billing side effects.
