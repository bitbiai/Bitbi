...
| --- | --- | --- | --- | --- |
| Merge readiness | Pass for pre-merge review | Phase 2-H adds backend-only org-scoped `POST /api/ai/generate-text` with HMAC Auth-to-AI proxying, `ai.text.generate` entitlement/credit enforcement, `ai_usage_attempts` reservation/finalization, and bounded text replay metadata. Full local validation is recorded in `PHASE2H_MEMBER_TEXT_GENERATION_API_REPORT.md` and the Phase 2-H response. | Production deploy still requires migrations through `0036`, live Cloudflare verification, and staging org billing/entitlement/credit/AI image+text usage retry/reservation/cleanup/replay verification. Phase 2-H files must be committed together after review. | Run Staff Security/SRE review, then commit the complete Phase 2-H file set together if approved. |
| Production deploy readiness | Blocked | Repo config declares required auth/AI secrets, `SERVICE_AUTH_REPLAY`, auth migrations through `0036`, AI/video queues, `USER_IMAGES`, `AUDIT_ARCHIVE`, and `AI_LAB`. `ALLOW_SYNC_VIDEO_DEBUG` must remain absent/false unless a controlled emergency debug rollback is approved. | Live Cloudflare secrets/bindings/queues/R2/D1 migrations were not verified by this implementation pass. Missing migration `0036` breaks Phase 2-D org-scoped image retry/reservation behavior and Phase 2-H text replay/attempt behavior; missing migration `0035` breaks billing APIs; missing migration `0034` breaks organization APIs; missing migration `0033` breaks export archive metadata/status handling; missing migration `0032` breaks lifecycle APIs; missing migration `0031` breaks activity projection writes. Organization and billing tables do not mean full tenant isolation or production billing readiness. | Provision/verify all Phase 0/1/2 secrets/resources, apply migrations `0028`-`0036`, verify organization, billing/entitlement/credit grant, image usage, and text generation usage flows in staging, keep `ALLOW_SYNC_VIDEO_DEBUG` disabled, and run staging verification before production. |
| Phase 0-A through 2-H security/operations posture | Reduced immediate risk | HMAC service auth, nonce replay protection, fail-closed limiters, body-size limits, durable MFA failed-attempt state, config validation, async admin video jobs, route-policy checks, operational readiness docs/checks, signed activity cursors, indexed/redacted activity search, data lifecycle foundations, organization/RBAC foundation, billing/entitlement/credit-ledger foundation, org-scoped image credit enforcement with usage attempts/replay/cleanup, and backend-only org-scoped text generation enforcement/replay are present. | This is not full SaaS maturity. Legacy `SESSION_SECRET` fallback remains during migration, route policy is metadata/checking rather than full centralized enforcement, live checks skip by default in CI, dashboard controls/alerts are not repo-enforced, restore drills are not executed, user self-service privacy flow and irreversible deletion execution remain deferred, existing assets remain user-owned, no live payment provider exists, video AI is not credit-enforced, and compliance/load-testing work remains open. | Complete Cloudflare staging verification, live health/header checks with `--require-live`, staging restore drill, alert/drift automation, Phase 2-A/2-B/2-C/2-D/2-E/2-F/2-H staging verification, self-service privacy policy, tenant migration plan, payment-provider design, and broader SaaS platform gaps. |

Phase 2-H is implemented as a backend-only org-scoped member text generation API on top of the Phase 2-D/F AI usage-attempt and replay foundations. Future changes must keep related route code, attempt metadata behavior, tests, route policy metadata, release compatibility, and documentation files together to avoid stale validation or route-policy drift.
...
## Immediate Pre-Deploy / Phase 2-H Backlog

These are the highest-priority follow-ups after the Phase 2-H implementation. They are deployment/staging prerequisites plus the next roadmap entrypoint; they are not reasons to redo Phase 0, Phase 1, Phase 2-A, Phase 2-B, Phase 2-C, Phase 2-D, Phase 2-E, or Phase 2-F work.

1. Keep validation green; re-run `npm run release:preflight` after any further application/config/test changes.
2. Commit the Phase 2-H code, test, and documentation updates together after review.
...
26. Decide whether historical audit/activity records need a controlled projection backfill and whether user-facing self-service privacy flows are required before production launch.
27. Execute and record a staging D1/R2 restore drill.
28. Choose the next Phase 2 track: wire video AI to entitlements/credits, design payment-provider integration, or continue domain-by-domain tenant ownership migration. Do not combine all tracks into one large rewrite.

## Next Phase: Tenant/Payment Or Video AI Route Track

Phase 2-H has implemented backend-only org-scoped member text generation on the same AI usage-attempt foundation as org-scoped image generation. The next phase should be chosen deliberately:

- Option A: wire the existing async/member-safe video route family to organization entitlements and credits while preserving admin/debug boundaries.
...
Do not redo completed Phase 0, Phase 1, Phase 2-A, Phase 2-B, Phase 2-C, Phase 2-D, Phase 2-E, Phase 2-F, or Phase 2-H work. Treat the following as baseline capabilities unless a focused regression is found:
