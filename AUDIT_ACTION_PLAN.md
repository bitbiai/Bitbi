...
| --- | --- | --- | --- | --- |
| Merge readiness | Ready for review | Phase 2-M updates live credit-pack economics and charges selected BFL Admin AI image tests for platform admins only, while preserving Phase 2-K Testmode lockdown and Phase 2-L live checkout/Credits behavior. Baseline preflight, focused Phase 2-M/Phase 2-L Worker tests, focused Admin AI/Credits static tests, Worker 359/359, static 168/168, asset checks, static build, and final release preflight passed. | Production deploy still requires migrations through `0040`, Stripe Testmode and live Stripe config/webhook verification, live Cloudflare verification, checkout/webhook/credit-grant verification, and verification for org, billing, AI usage, Admin AI Lab, lifecycle, billing-event, control-plane UI, pricing, and Credits flows. | Review Phase 2-M diff, then commit Phase 2-M runtime/docs/tests together after approval. |
...
Phase 2-J is implemented as a Stripe Testmode credit-pack checkout foundation. Phase 2-K makes that checkout route server-side platform-admin-only and disabled unless `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true`. Phase 2-L adds a separate live checkout route for fixed live packs and allows only platform admins or active organization owners. Phase 2-M changes the live pack catalog to `live_credits_5000` / `live_credits_12000` and adds platform-admin-only organization-credit charging for supported BFL Admin AI image tests. The Pricing / Credit Purchase rollout exposes Testmode only to admins through `/pricing.html`; the new Credits dashboard exposes live packs only through `/account/credits.html` for eligible admins/owners. Future changes must keep checkout creation, Stripe raw-body signature verification, billing event storage, admin/owner checkout grant requirements, exactly-once credit grant behavior, admin image-test usage attempt/debit behavior, tests, route policy metadata, release compatibility, pricing/credits copy, and documentation files together to avoid stale validation or route-policy drift.
