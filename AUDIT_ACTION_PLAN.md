...
| --- | --- | --- | --- | --- |
| Merge readiness | Ready for review | Phase 2-O updates the admin-only Pricing page runtime UI to match the current live 5,000/12,000 credit-pack model, removes visible stale Testmode wording, routes paid Pricing CTAs to the gated Credits dashboard, and compacts eligible profile navigation around Wallet/Credits/Organization. Phase 2-N organization context and Phase 2-M Admin AI charging remain intact. Validation results are recorded in the Phase 2-O report/final response. | Production deploy still requires migrations through `0040`, Stripe Testmode and live Stripe config/webhook verification, live Cloudflare verification, checkout/webhook/credit-grant verification, organization-context verification, and verification for org, billing, AI usage, Admin AI Lab, lifecycle, billing-event, control-plane UI, pricing, Credits, and Organization flows. | Review Phase 2-O diff, then commit Phase 2-O runtime/frontend/docs/tests together after approval. |
| Production deploy readiness | Blocked | Repo config declares required auth/AI secrets, optional synthetic `BILLING_WEBHOOK_TEST_SECRET`, optional Stripe Testmode config, optional operator-controlled `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT`, optional live Stripe credit-pack config with `ENABLE_LIVE_STRIPE_CREDIT_PACKS`, `SERVICE_AUTH_REPLAY`, auth migrations through `0040`, AI/video queues, `USER_IMAGES`, `AUDIT_ARCHIVE`, and `AI_LAB`. `ALLOW_SYNC_VIDEO_DEBUG` must remain absent/false unless a controlled emergency debug rollback is approved. | Live Cloudflare secrets/bindings/queues/R2/D1 migrations, Stripe Testmode endpoint/secrets, and live Stripe endpoint/secrets were not verified by this implementation pass. Missing migration `0040` breaks persisted live checkout authorization/payment status tracking; missing migration `0039` can make 5000/10000-credit Testmode grants fail against the original balance cap; missing migration `0038` breaks Stripe checkout session tracking; missing migration `0037` breaks billing event ingestion; missing migration `0036` breaks Phase 2-D org-scoped image retry/reservation behavior and Phase 2-H text replay/attempt behavior; missing migration `0035` breaks billing APIs. Organization and billing tables do not mean full tenant isolation or full production billing readiness. | Provision/verify all Phase 0/1/2 secrets/resources, apply migrations `0028`-`0040`, verify organization, billing/entitlement/credit grant, image/text usage, usage cleanup, replay cleanup, synthetic billing webhook, Stripe Testmode platform-admin-only checkout/webhook flows, Pricing return states, and Phase 2-L live checkout/webhook/Credits dashboard behavior. Keep `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS` absent/false except during bounded canary windows. |
| Phase 0-A through 2-M security/operations posture | Reduced immediate risk | HMAC service auth, nonce replay protection, fail-closed limiters, body-size limits, durable MFA failed-attempt state, config validation, async admin video jobs, route-policy checks, operational readiness docs/checks, signed activity cursors, indexed/redacted activity search, data lifecycle foundations, organization/RBAC foundation, billing/entitlement/credit-ledger foundation, org-scoped image/text credit enforcement with usage attempts/replay/cleanup, provider-neutral billing event ingestion, Stripe Testmode checkout foundation, server-side platform-admin-only Testmode checkout lockdown, narrow live Stripe one-time credit-pack checkout for platform admins/org owners, admin-only BFL image-test charging, and frontend Admin Control Plane/Credits surfaces are present. | This is not full SaaS maturity. Legacy `SESSION_SECRET` fallback remains during migration, route policy is metadata/checking rather than full centralized enforcement, live checks skip by default in CI, Cloudflare dashboard controls/alerts are not editable from the admin UI, restore drills are not executed, user self-service privacy flow and irreversible deletion execution remain deferred, existing assets remain user-owned, public billing/subscriptions/invoices/customer portal/refund automation do not exist, video AI is not credit-enforced, member-facing image pricing is unchanged by Phase 2-M, and compliance/load-testing work remains open. | Complete Cloudflare staging verification or a bounded operator-run canary, live health/header checks with `--require-live`, restore drill evidence, alert/drift automation, Phase 2-A through 2-M and Admin Control Plane verification, self-service privacy policy, refund/chargeback/provider hardening, tenant migration plan, and broader SaaS platform gaps. |

Phase 2-J is implemented as a Stripe Testmode credit-pack checkout foundation. Phase 2-K makes that checkout route server-side platform-admin-only and disabled unless `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true`. Phase 2-L adds a separate live checkout route for fixed live packs and allows only platform admins or active organization owners. Phase 2-M changes the live pack catalog to `live_credits_5000` / `live_credits_12000` and adds platform-admin-only organization-credit charging for supported BFL Admin AI image tests. Phase 2-N adds `/account/organization.html`, `GET /api/orgs/:id/organization-dashboard`, and shared active organization selection so Credits and Admin AI Lab use the same organization context. Phase 2-O updates `/pricing.html` to present Free, 5,000-credit, and 12,000-credit tiers without visible Testmode positioning; paid Pricing CTAs route to `/account/credits.html`, where backend authorization remains authoritative. Future changes must keep checkout creation, Stripe raw-body signature verification, billing event storage, admin/owner checkout grant requirements, exactly-once credit grant behavior, admin image-test usage attempt/debit behavior, organization-context selection, tests, route policy metadata, release compatibility, pricing/credits/organization copy, and documentation files together to avoid stale validation or route-policy drift.
...
| 6 | Add fail-closed Worker config validation | Resolved for current critical paths | `workers/auth/src/lib/config.js` validates auth critical config including `SESSION_HASH_SECRET`, `PAGINATION_SIGNING_SECRET`, `ADMIN_MFA_ENCRYPTION_KEY`, `ADMIN_MFA_PROOF_SECRET`, `ADMIN_MFA_RECOVERY_HASH_SECRET`, `AI_SAVE_REFERENCE_SIGNING_SECRET`, legacy `SESSION_SECRET` while fallback is enabled, `DB`, and `AI_SERVICE_AUTH_SECRET` where needed. `workers/ai/src/lib/config.js` validates `AI_SERVICE_AUTH_SECRET` and `SERVICE_AUTH_REPLAY`. `workers/auth/src/lib/stripe-billing.js` fails closed with safe variable-name diagnostics for missing Stripe Testmode/live checkout config without exposing values, requires `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT=true` before Testmode checkout creation, and requires `ENABLE_LIVE_STRIPE_CREDIT_PACKS=true` plus live-like Stripe config before live checkout creation. `scripts/validate-cloudflare-deploy-prereqs.mjs` validates repo config for critical secrets/bindings and marks production blocked when live validation is skipped. Release compatibility now tracks auth migrations through `0040`. | Live Cloudflare config was not verified. Static headers, WAF, RUM, and some dashboard-managed resources are still not fully repo-enforced. Legacy fallback remains until operators disable it. Checkout enable flags are operator kill switches, not production readiness. | Provision all Phase 1-D, Phase 2-J/2-K Testmode, and Phase 2-L live Stripe config only when needed, apply migrations through `0040`, keep `ENABLE_ADMIN_STRIPE_TEST_CHECKOUT` and `ENABLE_LIVE_STRIPE_CREDIT_PACKS` false/absent except during canaries, run `npm run validate:cloudflare-prereqs -- --live` where credentials are available, then add IaC/live drift checks later. |
...
7. Apply auth migrations `0028` through `0040` before deploying auth Worker code that depends on them.
...
- Latest auth migration number is documented as `0040`.
...
- No new migration is required for Phase 2-M; it reuses `credit_ledger`, `usage_events`, and `ai_usage_attempts`.
...
| `npm run test:release-compat` | Pending final rerun | Release compatibility should validate migration `0040` after this phase. |
...
- Remote D1 migrations, including migrations through `0040`.
