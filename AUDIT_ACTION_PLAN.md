...
| P1 | Tenant ownership | New personal folder/image writes assigned; read diagnostics, admin evidence report, evidence collection runbook/template, Phase 6.9 package, Phase 6.10 real main evidence decision, and Phase 6.11 manual-review workflow design added; old rows unbackfilled | Use `docs/tenant-assets/`, `npm run dry-run:tenant-assets:images`, `/api/admin/tenant-assets/folders-images/evidence`, and `docs/tenant-assets/evidence/`; next design manual-review state schema, with no backfill or access change. |
