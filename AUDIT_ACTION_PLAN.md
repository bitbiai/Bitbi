...
Last reconciled: 2026-05-17 for Phase 6.6 read-only AI folders/images ownership metadata diagnostics.
...
| P1 | Tenant ownership | New personal folder/image writes assigned; read diagnostics added; old rows unbackfilled | Use `docs/tenant-assets/` and `npm run dry-run:tenant-assets:images`; next add admin/staging evidence only, with no backfill or access change. |
...
- Phase 6.1 tenant asset ownership design, inventory, risk matrix, Phase 6.2 `ai_folders`/`ai_images` owner-map dry-run/test scripts, Phase 6.3 schema/access impact plan, Phase 6.4 nullable ownership metadata schema, Phase 6.5 new personal folder/image write metadata assignment, and Phase 6.6 read-only dual-read diagnostics with no old-row backfill, R2, access-check, gallery, quota, lifecycle, or billing behavior mutation.
