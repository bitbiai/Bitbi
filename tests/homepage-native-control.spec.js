...
for (const source of ['original', 'changing']) {
  test(`independent native HTTP output: ${source} source loops, seeks and resumes`, async ({ page }, testInfo) => {
    ...
    const result = await page.evaluate(async source => {
      ...
      let callback, loops = 0, previous = null, restart = null, phase = 'loop';
      ...
      let rejection = null;
      try { await v.play(); } catch (error) { rejection = error.name; }
      const until = predicate => new Promise(resolve => {
        const end = performance.now() + 4500;
        function sample() {
          if (predicate() || v.error || performance.now() >= end) resolve();
      ...
      return { supported: true, source, identity, loopState, resumed, stayedPaused, seeked, rejection, error, frames, events };
    }, source);
    ...
    if (diagnostic) {
      ...
      console.log('LINUX WEBKIT MEDIA DIAGNOSTIC', JSON.stringify({ source, ...result.loopState, resumed: result.resumed, error: result.error }));
    } else {
