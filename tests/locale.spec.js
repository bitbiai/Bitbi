...
test.describe('Bilingual locale pages', () => {
  ...
  test('pricing pages render English and German copy', async ({ page }) => {
    ...
    await expect(page.locator('[data-pricing-auth-entry="register"]')).toContainText('Create account');
    ...
    await expect(page.locator('main')).not.toContainText('Set up the account path before checkout');
    ...
    await expect(page.locator('.pricing-hero__link')).toHaveCount(0);
    await expect(page.locator('.pricing-card__title')).toHaveText(['Free Account', 'BITBI Pro', 'Starter Credits', 'Creator Credits']);
    await expect(page.locator('[data-pricing-auth-entry="register"]')).toContainText('Konto erstellen');
    await expect(page.locator('#pricingDecision')).toHaveCount(0);
    await expect(page.locator('#pricingJourney')).toHaveCount(0);
    await expect(page.locator('#pricingContinuity')).toHaveCount(0);
    await expect(page.locator('#pricingAccountEntry')).toHaveCount(0);
    await expect(page.locator('main')).not.toContainText('Wählen Sie BITBI Pro für ein monatliches Kreativkontingent');
    await expect(page.locator('main')).not.toContainText('Jetzt erstellen');
    await expect(page.locator('main')).not.toContainText('Credits wählen');
    await expect(page.locator('main')).not.toContainText('Optionen vergleichen');
    await expect(page.locator('main')).not.toContainText('Wählen Sie die passende Option für heute');
    await expect(page.locator('main')).not.toContainText('Von Preisen in den Arbeitsbereich');
    await expect(page.locator('main')).not.toContainText('Von der Auswahl zum nächsten Prompt');
    await expect(page.locator('main')).not.toContainText('Kontopfad vor dem Checkout klären');
