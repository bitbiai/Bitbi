...
export function prepareBuild(artifactParent = os.tmpdir()) {
  ...
  assert.deepEqual(migrations.filter(row => Number(row.path.slice(0, 4)) > 83).map(row => row.path.slice(0, 4)), ['0084', '0085', '0086', '0087', '0088'], 'Native matrix includes the additive member-generation and Canvas processing migrations');
