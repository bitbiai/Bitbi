...
export function parseRuntimeArgs(args, env = {}) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    } else if (args[i] === '--suite' && !result.suite && ['member-generation','model-status','model-pricing','appearance','canvas','q4-stream'].includes(args[i+1])) {
      result.suite = args[++i];
    } else throw new Error('Usage: test-q2-runtime [--preflight] [--suite member-generation|model-status|model-pricing|appearance|canvas|q4-stream] [--artifacts <outside-repository-directory>]');
  }
  return result;
}
