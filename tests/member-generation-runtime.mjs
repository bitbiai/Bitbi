...
export async function runMemberGenerationTests(f) {
  ...
  for(const name of ['admin-lab-image','admin-lab-music','admin-lab-video','asset-naming-video','asset-naming-manual','asset-naming-image','asset-naming-music','asset-naming-image-manual','asset-naming-music-manual','clock-lease-expired','clock-credit-expired','clock-finalization-expired','closed-browser','execution-exhausted','poster-retry','stale-poster','insert-response-lost','provider-unknown','music-failed','image','music','music-cover-retry','debit-response-lost','unpublished-asset','finalization-response-lost','storage-restart',...[...grokCases,...catalogCases].map(c=>c.name)]) {
    await f.test(`member_generation_actual_queue_${name}`,async()=>{
      const response=await f.control('/member-generation',{name,...fixture,...([...grokCases,...catalogCases].find(c=>c.name===name)||{})});
      ...
      assert.equal(result.status,blocked?'billing_unverified':['provider-unknown','clock-credit-expired'].includes(name)?'outcome_unknown':['music-failed','execution-exhausted'].includes(name)?'failed':'succeeded');
      assert.deepEqual(await f.rows('PRAGMA foreign_key_check'),[]);
