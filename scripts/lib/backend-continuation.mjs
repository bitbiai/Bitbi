...
export function backendContinuationSupported(plan) {
  ...
  if(plan.workerDeploys.some(s=>s.worker==='ai') && !plan.changedFiles.every(f=>!f.startsWith('workers/ai/') || ['workers/ai/src/routes/text.js','workers/ai/src/lib/grok-chat.js','workers/ai/src/lib/invoke-ai.js'].includes(f)))return false;
