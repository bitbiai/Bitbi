// Auth, private FFmpeg and the reviewed existing AI text service are automated. Other Workers,
...
export function backendContinuationSupported(plan) {
  ...
  if(plan.workerDeploys.some(s=>s.worker==='ai') && !plan.changedFiles.every(f=>!f.startsWith('workers/ai/') || ['workers/ai/src/routes/text.js','workers/ai/src/lib/grok-chat.js'].includes(f)))return false;
