...
const FUTURE_PHASES = Object.freeze([
  ...
  {
    phase: "6.16",
    title: "Manual review queue read and evidence APIs",
    scope: "Implemented as read-only queue/item/event/evidence endpoints; no status updates, ownership backfill, or access switch.",
  },
  {
    phase: "6.17",
...
export function buildFoldersImagesOwnerMapDryRunReport(repoRoot = process.cwd(), options = {}) {
  ...
  const manualReviewQueueReadApiAdded = fileExists(repoRoot, manualReviewQueueHelper);
  ...
  const legacyMediaResetExecutorAdded = fileExists(repoRoot, legacyMediaResetExecutorHelper)
  ...
  return {
    ...
    manualReviewWorkflow: {
      ...
      adminUiAdded: manualReviewAdminVisibilityAdded,
    ...
    manualReviewQueueReadApi: {
      status: manualReviewQueueReadApiAdded ? "manual_review_queue_read_api_added" : "not_recorded",
      helper: manualReviewQueueReadApiAdded ? manualReviewQueueHelper : null,
      endpoints: manualReviewQueueReadApiAdded
        ? [
          "/api/admin/tenant-assets/folders-images/manual-review/items",
          "/api/admin/tenant-assets/folders-images/manual-review/items/:id",
          "/api/admin/tenant-assets/folders-images/manual-review/items/:id/events",
        ...
        : [],
      exportFormats: manualReviewQueueReadApiAdded ? ["json", "markdown"] : [],
      readOnly: true,
      routePolicyRequired: true,
      statusUpdatesAdded: false,
      backfillPerformed: false,
      accessChecksChanged: false,
      sourceAssetRowsMutated: false,
      r2LiveListed: false,
    ...
    manualReviewStatusWorkflow: {
      status: manualReviewStatusWorkflowAdded ? "manual_review_status_workflow_added" : "not_recorded",
      helper: manualReviewStatusWorkflowAdded ? manualReviewStatusHelper : null,
      endpoint: manualReviewStatusWorkflowAdded
      ...
      idempotencyRequired: true,
      writesReviewItemsAndEventsOnly: true,
      statusChangesReviewTableOnly: true,
      ownershipBackfillPerformed: false,
      accessChecksChanged: false,
      sourceAssetRowsMutated: false,
    ...
    manualReviewStatusOperatorEvidence: {
      status: manualReviewStatusOperatorEvidenceAdded
        ? "manual_review_status_operator_evidence_added"
        : "not_recorded",
      adminVisibilityAdded: manualReviewAdminVisibilityAdded,
      adminPanel: manualReviewAdminVisibilityAdded
        ? "admin/index.html#operations tenant asset manual-review queue"
        : null,
      clientWrappersAdded: manualReviewClientWrappersAdded,
      evidenceIncludesStatusRollups: manualReviewQueueReadApiAdded,
      evidenceIncludesLatestStatusUpdateTimestamp: readText(repoRoot, manualReviewQueueHelper).includes("latestStatusUpdateTimestamp"),
      statusControlsReviewStateOnly: manualReviewAdminVisibilityAdded,
      ownershipBackfillPerformed: false,
      accessChecksChanged: false,
      sourceAssetRowsMutated: false,
      ownershipMetadataUpdated: false,
    ...
    legacyMediaResetExecutorDesign: {
      ...
      executorImplemented: legacyMediaResetExecutorAdded,
      endpointAdded: legacyMediaResetExecutorAdded,
      uiAdded: false,
      migrationAdded: legacyMediaResetExecutorAdded,
      deletionPerformed: false,
      sourceAssetRowsMutated: false,
      ownershipMetadataUpdated: false,
      ownershipBackfillPerformed: false,
    ...
    legacyMediaResetExecutor: {
      status: legacyMediaResetExecutorAdded ? "legacy_media_reset_executor_added" : "not_recorded",
      helper: legacyMediaResetExecutorAdded ? legacyMediaResetExecutorHelper : null,
      migration: legacyMediaResetExecutorAdded ? legacyMediaResetActionMigrationFile : null,
      executeEndpoint: legacyMediaResetExecutorAdded
        ? "/api/admin/tenant-assets/legacy-media-reset/execute"
        : null,
      actionEndpoints: legacyMediaResetExecutorAdded
        ? [
          "GET /api/admin/tenant-assets/legacy-media-reset/actions",
          "GET /api/admin/tenant-assets/legacy-media-reset/actions/:id",
          "GET /api/admin/tenant-assets/legacy-media-reset/actions/:id/evidence",
      ...
      manualReviewSupersessionDeferred: true,
      liveExecutionPerformedByScript: false,
      deletionPerformedByScript: false,
      ownershipBackfillPerformed: false,
      accessChecksChanged: false,
      sourceMutationOutsideConfirmedExecutor: false,
      r2LiveListed: false,
...
export function renderFoldersImagesOwnerMapMarkdown(report) {
  const lines = [
    ...
    "",
    `- Status: ${report.manualReviewQueueReadApi?.status || "not_recorded"}`,
    `- Helper: ${report.manualReviewQueueReadApi?.helper || "not_recorded"}`,
    `- Endpoints: ${(report.manualReviewQueueReadApi?.endpoints || []).join(", ") || "none"}`,
    `- Export formats: ${(report.manualReviewQueueReadApi?.exportFormats || []).join(", ") || "none"}`,
    `- Read only: ${report.manualReviewQueueReadApi?.readOnly ? "yes" : "no"}`,
    ...
    "",
    `- Status: ${report.manualReviewStatusWorkflow?.status || "not_recorded"}`,
    `- Helper: ${report.manualReviewStatusWorkflow?.helper || "not_recorded"}`,
    `- Endpoint: ${report.manualReviewStatusWorkflow?.endpoint || "not_recorded"}`,
    `- Confirm required: ${report.manualReviewStatusWorkflow?.confirmRequired ? "yes" : "no"}`,
    `- Reason required: ${report.manualReviewStatusWorkflow?.reasonRequired ? "yes" : "no"}`,
    `- Idempotency required: ${report.manualReviewStatusWorkflow?.idempotencyRequired ? "yes" : "no"}`,
    `- Status changes review-table only: ${report.manualReviewStatusWorkflow?.statusChangesReviewTableOnly ? "yes" : "no"}`,
    ...
    "",
    `- Status: ${report.manualReviewStatusOperatorEvidence?.status || "not_recorded"}`,
    `- Admin visibility added: ${report.manualReviewStatusOperatorEvidence?.adminVisibilityAdded ? "yes" : "no"}`,
    `- Admin panel: ${report.manualReviewStatusOperatorEvidence?.adminPanel || "not_recorded"}`,
    `- Client wrappers added: ${report.manualReviewStatusOperatorEvidence?.clientWrappersAdded ? "yes" : "no"}`,
    ...
    "",
    `- Status: ${report.legacyMediaResetExecutor?.status || "not_recorded"}`,
    `- Helper: ${report.legacyMediaResetExecutor?.helper || "not_recorded"}`,
    `- Migration: ${report.legacyMediaResetExecutor?.migration || "not_recorded"}`,
