...

{
  ...
  assert.equal(skeleton.reportVersion, "current-baseline-billing-canary-evidence-v2");
  ...
  assert(skeleton.requiredEvidence.some((entry) => entry.id === "exactly_once_credit_grant_after_verified_webhook"));
  assert(skeleton.requiredEvidence.some((entry) => entry.id === "bitbi_pro_subscription_checkout"));
  assert(skeleton.requiredEvidence.some((entry) => entry.id === "redacted_admin_live_billing_export"));
  ...
  assert(billingMarkdown.includes("Exactly-once credit grant after verified webhook"));
  assert(billingMarkdown.includes("BITBI Pro subscription checkout evidence"));
  ...
  assert(billingMarkdown.includes("Redacted Admin Live Billing export attachment checklist"));
